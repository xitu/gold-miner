...
Kaylyn 在最近的一年里尝试通过实现各种算法来找乐子，可能对于你来说很奇怪，但对她而言尤其诡异。她在大学课堂里尤其讨厌算法。她的教授经常使用一些复杂的术语来授课，而且还拒绝解释一些『显然』的概念。结果就是，她只学到了一些能够帮助她找到工作的基本知识。

然而她的态度在当她开始使用 Go 来实现这些算法时就开始转变了。将那些由 C 或者 Java 编写的算法转换到 Go 身上令人意想不到的简单，于是她开始逐渐理解这些算法，并『重返』校园。
...
* 他们确实有一个两句话区域（以绿色框突出显示），解释了二进制查找树算法是什么。但它隐藏在一个几乎看不见的句子中，并称之为二元查找树『性质』，这对于初学者而言是非常令人困惑的术语。
...
本演讲的其余部分将介绍二叉查找树的内容。如果你是 Go 新手或算法新手，你会发现它很有用。而如果你不是，它仍然可以是一个很好的复习，你可以转发给其他对这些事情感到好奇的人。
...
这是一个『猜数游戏』，很多人儿时玩过的游戏。你问你的朋友来在一个范围内（比如 1 至 100）猜一个特定的书。然后你朋友可能会说『57』。一般情况下第一次猜会猜错，但是你会告诉他们猜测的数字是大了还是小了。然后他可以继续猜测知道最后猜中为止。

![](https://user-images.githubusercontent.com/9022011/44758764-7b777000-aaf3-11e8-92d4-ebb4e92c2832.png)

这个猜数游戏基本上就是一个二叉查找的过程了。如果你正确理解了这个猜数游戏，那么你也能够理解二叉查找树算法背后的原理。你朋友猜测的数字就是树种的数字，『高了』和『低了』决定了移动的方向：右节点或左节点。
