...
好问题！伪类 `:root` 与您使用 `html` 选择器时相同，不同之处在于 ':root' 伪类的特殊性更高。这意味着如果您在 `:root` 伪类中设置属性，它的优先级将大于 html 选择器。
...
例如，如果您想设置一个 `font-family`，并且需要指定一个以上的字体，该怎么办？ 回顾之前的提示，这实现起来应该很简单。我们只需要用逗号来写。所以代码应该是这样：
...
为什么我们要删除和添加 `.light` 类? 我们现在就回答这个问题。
