...
就在几周前，我正在和一个朋友谈话，他不经意间提到，“你永远都不会直接针对生产中的 Node 来运行应用程序”。我强烈点头，表示我**也**不会在生产中运行 Node，原因可能每个人都知道。但是我并不知道，我应该知道原因吗？我还能继续编程吗？
...
首先，让我们解决“永远不要直接针对生产中的 Node 运行应用程序”的说法。

### 永远不要直接针对生产中的 Node 运行

也许。但可能也不是。让我们来谈谈这个声明背后的原因。首先，让我们看看为什么不能。

假设我们有一个简单的 Express 服务器。我能想到的最简单的 Express 服务器。。。。。。
...
我们通常使用某种 Node 进程管理器 `supervisor` 或 `nodemon` 来解决这个问题。这些包会监听我们的项目，并会在我们进行更改时新启动服务器。我通常都会这样做。。。。。。
...
然后我运行 `npm run dev`。请注意，我在这里运行 `npx supervisor`，这使得我可以使用 `supervisor` 包而不用安装它。
...
这没什么大不了的，对吧？这是一个错误。它会发生。

不。这很重要。如果你返回终端，你将会看到应用程序完全关闭。

![](https://cdn-images-1.medium.com/max/2560/1*69LuEt53W2isIXP34vUlYA.png)

这意味着，如果你回到浏览器并尝试访问站点的根URL，你会得到相同的错误页面。一个方法中的一个错误导致**每个人**的应用程序关闭。

这是很糟糕的。这就是人们说 **“永远不要直接针对生产中的 Node 运行”**

好。如果我们无法在生产中运行 Node，那么在生产中运行 Node 的正确方法是什么？
...
pm2 是一个 Node 进程管理器，有很多有用的功能。就像其他的 “JavaScript” 库一样，你可以使用 `npm` 全局安装它 —— 或者你也可以再次使用 `npx`。我不想手把手教你。

使用 pm2 可以通过很多方法运行你的程序。最简单的方法就是在入口文件调用 `pm2 start`。
...
这是我们在 pm2 监控的后台运行的进程。如果你访问 `read` 端点并使应用程序崩溃，pm2 将自动重新启动它。你在终端不会看到任何内容，因为它在后台运行。如果你想看到 pm2 正在做什么，你可以运行 `pm2 log 0`。这个 `0` 使我们想要查看日志的进程的 ID。

![](https://cdn-images-1.medium.com/max/2560/1*AbR1eyySpr2IllYtA4wE-Q.png)

我们走吧！你可以看到，pm2 重启应用程序时，因为我们的未处理错误下降。

我们还可以提取开发命令，并为我们准备PM2监视文件，并在任何更改时重新启动。
...
如果要删除改进程引用，则需要运行 `pm2 delete`。你可以使用一个命令 `delete` 来停止和删除进程。
...
pm2 有很多配置选项，它们包含在一个“生态系统”文件中。可以通过运行 `pm2 init`来创建一个。你会得到这样的东西......
...
我会自动忽略本文中“部署”的部分，因为我不知道它的作用。

“应用程序”部分定义了希望 pm2 运行和监视的应用程序。你可以跑不止一个。许多这些配置设置可能是不言而喻的。这里我要关注的是**实例**设置。

pm2 可以运行您的应用程序的多个实例。你可以传入你想要运行的多个实例，pm2 会启动那么多实例。因此，如果我们想运行 4 个实例，我们可以拥有以下配置文件。
...
在单独的 CPU 上运行进程的好处是，如果有一个进程运行异常，占用了 100% 的 CPU，那么其他进程仍将工作。如果传入的实例多于内核数，pm2 将根据需要将CPU上的进程加倍。

你可以使用pm2进行更多操作，包括监视和以其他方式处理那些讨厌的 [environment variables](https://medium.freecodecamp.org/)。

另外一个注意事项：如果由于某种原因，你希望 pm2 运行 `npm start` 脚本。你可以通过运行 npm 作为进程并传递 `--start`。“start” 之前的空间在这里非常重要。

```
pm2 start npm -- start
```

在 [Azure AppService](https://docs.microsoft.com/en-us/azure/app-service/?WT.mc_id=medium-blog-buhollan) 中，默认在后台包含 pm2。如果要在 Azure 中使用 pm2，则无需将其包含在 `package.json` 文件中。你可以添加一个生态系统文件，然后就可以使用了。
...
事实证明，Azure 已经在做这件事了。如果我们不将 pm2 生态系统文件推送到 Azure，它将使用我们的 `package.json` 文件运行脚本来启动应用程序，我们可以直接针对Node运行。
...
在这种情况下，我们直接针对 Node 运行，没关系。如果应用程序崩溃，你会发现它又回来了。那是因为在 Azure 中，你的应用程序在一个容器中运行。Azure 正在安排运行应用程序的容器，并知道它何时更新。
...
Tierney 的最后一个建议就是像一个成年人一样处理错误和编写测试。但是谁有时间呢？
