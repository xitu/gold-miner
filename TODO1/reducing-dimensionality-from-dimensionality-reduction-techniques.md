...
在本文中，我将尽最大的努力来阐明降维技术中常用的三种降维方法：即 PCA、t-SNE 和自动编码器。由于这些方法多被当作黑盒来使用，因此有时会被误用。理解它们将可以帮助大家决定何时以及如何使用它们。
...
在处理实际问题和实际数据时，我们通常处理的数据维度是高达数百万的。
...
1.  你的系统使用余弦相似度来度量距离，但是你需要把它进行可视化，展示给一些非技术人员，他们可能根本不熟悉余弦相似度的概念——你会怎么做呢？
...
从现在开始，X 为我们要处理的数据矩阵，维度为 (n, p)，其中 n 表示样本数量，p 表示维度。

给定 X，以上这两种方法都试图以它们自己的方式，来处理和分解 X，之后我们可以将分解后的结果相乘，以在更小的维数中表示最多的信息。我知道这听起来很可怕，所以这里我将省去大部分的数学知识，只保留有助于理解这些方法的优缺点部分。
...
为什么要关心这个呢？因为这两个过程中有一些非常基本的东西告诉我们很多关于主成分分析（PCA）的知识。

正如你所看到的这样，这两种方法都是纯粹的线性代数方法，这基本上告诉我们，使用 PCA 方法就是将真实的数据从不同的角度进行观察——这是 PCA 方法独有的，而其它的方法则是将数据从低维进行随机表示，并且试图让它表现得像高维度数据一样。

其它值得注意的是，所有的操作都是线性的，所以使用 SVD 的方法速度非常快。
...
因此我们可以选择将维度减少到一定的数目，这将保留原数据大约给定维度数量的百分比例，我将在代码中演示这一点（比如在丢失最多 15% 的数据约束下，我们可以减少一定的维度）。
...
在第一个 “if” 判断语句中，我们将数据进行了规范化，并且检查了需要使用多少个奇异值，接下来基本上是从 `keep_info` 中求出 `n_dimensions` 的值。
...
举个例子，如果你是 CTO，并且有一些数据，你想要使用其余弦相似性来作为其距离度量，而你的 CEO 希望你现在来做一些展示来表现这些数据的特性，我认为你没有时间来向董事会解释余弦相似性以及如何操作集群数据，你只需要简单地绘制出余弦相似集群即可，这正如在 t-SNE 算法中使用的欧几里德距离集群一样——这就是我想说的，这已经很好了。
