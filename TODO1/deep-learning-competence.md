...
这意味着企业对高效的深度学习从者的需求量巨大。

问题是，一般的企业如何去鉴别这些从业者的好坏？

作为一名深度学习的从业者，你如何用最好的方式去证明你可以提供熟练的深度学习模型？

在这篇文章中，你将了解胜任深度学习的三级能力并且作为一个从业者，你最需要哪个。

在阅读本文之后你将了解：

*   深度学习能力的问题最好是通过项目组合去评估。
*   三个能力级别的层次结构可被使用去给从业者分类并且提供一个框架去鉴别他们的能力是否符合期望。
...
就是这样，结果胜过那些传统的能够代表能力的东西，诸如教育背景，工作历史和经验水平。

大部分开发者和他们的经理已经明白了这个道理。

但一些人还没有。

## 开发一个深度学习组合

毫无疑问这是最好的方法去回答一个从业者是否胜任深度学习的工作。

从业者必须提供证据去证明他们理解如何去提供深度学习的技术并且使用他们去发展高效的模型。

这意味着要使用开源仓库和可用的公共数据集去开发一个[公共的项目组合](https://machinelearningmastery.com/build-a-machine-learning-portfolio/)。
...
在实际项目中，通过一次真诚的讨论就能很快搞清楚实践者是否知道他们自己在做什么。

*   为了评估这个能力，雇主必须要求并且关注已完成工作的组合。
*   为了证明这个能力，深度学习的从业者必须开发和维护一个已完成的项目的组合。

作为一个从业者 ，问题变成了：什么是能力等级和每个等级的目标能力是什么？

## 深度学习能力的等级

不仅是为了开发，项目还可以用来去证明技术能力。

在本节中，我们将概述深度学习能力的等级以及作为一名从业者，您可以开发和实施的项目类型，以便学习、获取和展示每一级能力水平。
...
*   并不是所有的商业开发都要求或者可以完美使用等级 3 的从业者。
*   许多从业者在等级 3 中深潜，并且匆忙的想找出 1 级和 2 级。
*   等级 2 常常被忽视，但是我认为它是关键，（因为可以）证明有更深入的理解。

其他没有被讨论到的可能和能力相关的话题还有从零开始代码，处理大数据和数据流，GPU 编程，开发新的方法，等等。
...
但这并不意味着要你去有个更高级的证书或者你是什么项目主管。相反，这意味着你熟悉应用机器学习的基础知识和通过预测性建模项目端到端工作的过程。
...
图书馆能力意味着你知道如何去搭建一个开发环境并且使用最普遍的 API 层去定义，适应，并且使用神经网络模型去预测。

它也意味着你知道每种神经网络模型最基本的不同点和使用它的最佳时机。

这并不意味着你知道每种函数过程和每种参数。这也不意味着你知道具体技术的数学描述。

### 建模能力

建模能力意味着你知道如何使用神经网络模型去完成一个机器学习项目。
...
*   使用目标测试工具评估一套模型和模型架构。
*   选择和准备一个最终的模型并且使用它去预测一个新的数据。

这意味着你可以高效的驾驭神经网络使之用于新项目的开发和建立有效模型上。

这并不意味着你成为了一个专家，可以使用任何神经网络技术或者你可以取得最好的预测结果。这也并不意味着你对任何数据类型都相当熟悉。

### 项目

展示这种能力水平的项目应该是使用开源的深度学习库（比如 Keras），并在公共表格的机器学习数据集上显示应用机器学习过程的每一步。

这并不意味着要去实现对数据集的最佳预测结果，甚至不意味着使用神经网络是数据集的最佳可能模型。相反，要去证明使用神经网络的能力，最可能简化模型例如多层感知机。

一个不错的数据集资源是在 1900 年到 2000 年被广泛使用的小型内存数据集。可以用它去证明机器学习甚至是神经网络的表现。例如在 [UCI Machine Learning Repository](https://archive.ics.uci.edu/ml/index.php)列表中的一些。

数据集很小，很容易在内存中容纳，这意味着项目的范围也很小，允许使用强大的模型评估方案。如k-折叠交叉验证，并且可能需要仔细的模型设计，以避免过拟合。
...
假设能力等级为 1，然后展示你可以使用传统和现代的技术从深度学习神经网络模型中发挥最大的功效。

它演示了下列事情：

*   **学习能力**。你可以改进神经网络模型的建立过程。
*   **泛化能力**。你可以减少训练数据的过度拟合还有减少样本外数据的泛化误差。
*    **预测能力**.你可以降低最终预测模型的方差和提升模型的技能。

### 学习能力

学习能力表示为了让程序达到更好的表现你知道如何去配置和调整学习算法的超参数。
...
泛化能力意味着你知道如何去配置和调整一个模型去减少过拟合并且提升模型在样本外数据的表现。
...
证明这一能力水平的项目可能不太关注应用机器学习过程中的所有步骤，而是关注特定的问题和技术或旨在缓解问题的一系列技术。
...
再说一次，这并不意味着在一个具体的问题里实现最佳的表现，仅仅是显示技术的最好使用方法和它处理已确定问题的能力。

明确要研究的课题可能比数据集的选择更重要。

一些数据集自然会带来一些问题，例如，小的训练集和不平衡的数据集会带来过拟合的结果。

可以使用标准机器学习数据集。换言之，可以人为设计问题然后再去证明它，或者还可以使用数据集生成器。

## 等级 3：应用

这个能力等级高于能力等级 1 和 ,2，并且展示你可以在具体问题里使用深度学习神经网络技术。

这是深度学习技术的展示，它超越了表格形式的数据集。
...
数据处理能力意味着你可以获得，加载，使用和为模型准备数据。

这很可能证明处理数据的标准库和准备数据的标准技术的能力。

问题域和要处理的数据可能包括：

*   **时间序列预测**。将时间排序问题设定为监督学习问题的代码。
*   **计算机视觉**。用于加载图像和转换以调整像素大小（可能是标准大小的）的API。
...
这将很可能需要熟悉学术文献和/或行业中针对该领域一般类问题使用的常用方法。

问题域和针对具体问题的方法可能包括：

*   **预测时间序列**。使用序列预测模型像卷积和循环神经网络模型。
*   **计算机视觉**。使用深度卷积神经网络和使用特殊体系结构。
*   **自然语言处理**。使用深度循环神经网络模型和使用特殊体系结构。

### 项目

项目演示能力等级必须包括应用机器学习过程，包括关注模型调整（等级 1 和 2 的能力），并且必须注意具体领域的数据集。
...
它在多个域中高层展示能力可能是可取的，并且能够很好的在数据处理，模型技术和技能中进行转换。

在解决了最突出的问题和技术之后，也可能需要专门研究某个领域，并缩小对细微子问题的演示项目的范围。

因为这种类型的项目可能表现出深度学习的更广泛吸引力（例如，超越经典方法的能力），直接跳到这一水平是有危险的。

对于有经验的从业者来说，是有可能在本领域有着更深入的知识和其他机器学习方法的经验。

然而，这是非常困难的，因为你可能不得不学习并且必须一次驾驭和证明所有三个等级的能力。

这是迄今为止初学者犯的最大的错误。他们潜入特定领域的项目并遇到一个又一个的障碍。因为他们还不具备使用库的能力，通过项目的过程以及提高模型性能的过程，更不用说在域中使用特定的数据处理技术和建模技术。
...
在这篇文章中，你发现了深度学习能力的三个等级，作为一个实践者，你必须在每一个等级上证明什么
