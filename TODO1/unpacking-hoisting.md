...
（1）被否决，因为这种方法在该语言中没有先例。因此这对于 JavaScript 程序员并不直观。
...
即使 `func()` 声明在 `myVar` 声明前且使用了该变量，我们仍然可以调用 `func()`。但是我们必须等到 `myVar` 的暂时死区结束之后。
...
我们已经看到提前激活有一个陷阱，你可以在不使用它的情况下获得大部分好处。 因此，最好避免提前激活。但我对此感觉并不十分认同，如前所述，我经常使用函数声明，因为我喜欢它们的语法。
