...
我经常出差。我不太喜欢开车，所以当我有空的时候，我更喜欢在城市里散步或骑自行车。我出差去过的许多城市都有自行车租赁系统，可以让你租几个小时的自行车。这些系统中的大多数都有一个应用程序来帮助用户定位和租赁他们的自行车，但对于像我这样的用户来说，有一个单独的地方来获取城市中所有可供租赁的自行车的信息会更有帮助。
...
由此产生的共享单车应用程序集成了来自许多不同共享系统的数据，包括在纽约市的 [Citi Bike](https://www.citibikenyc.com/) 共享单车。它利用了 Citi Bike 系统提供的通用共享单车源，并使用其数据展示了一些可以使用 Redis 对地理空间数据进行索引的特性。Citi Bike 数据是根据 [Citi Bike 数据许可协议](https://www.citibikenyc.com/data-sharing-policy)提供的。

## 通用共享单车反馈规范

通用共享单车反馈规范（GBFS）是由[北美共享单车协会](http://nabsa.net/ )开发的[开源数据规范](https://github.com/NABSA/gbfs)，目的是让地图和运输应用程序更轻易地将共享单车系统添加到它们的平台中。目前世界上有 60 多个不同的共享系统在使用该规范。

反馈由包含有关系统状态信息的几个简单 [JSON](https：//www.json.org/) 数据文件组成。反馈从引用子反馈数据的 URL 的顶级 JSON 文件开始：
...
第一步是将来自 `system_information` 和 `station_information` 反馈的有关共享单车站点的信息的数据加载到 Redis 中。

 `system_information` 反馈提供系统 ID，这是一份为 Redis 键创建命名空间的简短代码。GBFS 规范没有指定系统 ID 的格式，但保证它是全局唯一的。对于系统 ID，许多共享单车反馈的都是使用简短的名称，如 coast_bike_share、boise_greenbike 或者 topeka_metro_bikes。另一些使用熟悉的地理缩写，如 NYC 或 BA，其中一个使用通用唯一标识符（UUID）。共享单车应用程序使用标识符作为前缀来构造给定系统的唯一密钥。

`Station_Information` 反馈提供组成系统的共享站点的静态信息。站点由带有多个字段的 JSON 对象表示。在站点对象中有几个强制字段，它们提供物理自行车站点的 ID、名称和位置。还有几个可选字段提供的有用信息，如最近的十字路口或公认的付款方法。这是这部分共享单车应用程序的主要信息来源。
...
仓库的 [systems.csv](https://github.com/NABSA/gbfs/blob/master/systems.csv) 文件为注册的共享单车系统提供了一个可用的 GBFS 反馈的发现 URL。发现 URL 是处理共享单车信息的起点。

 `load_station_data` 应用程序获取系统文件中发现的每个 URL，并使用它查询两个反馈 URL：系统信息和站点信息。系统信息反馈提供了一条关键信息：系统的唯一 ID。（**注意：systems.csv 文件也提供了系统 ID，但是该文件中的一些标识符与提要中的标识符不匹配，因此我总是从提要中获取标识符。**）系统的详细信息，比如共享单车 URL、电话号码和电子邮件。可以添加到应用程序的未来版本中。 因此使用键 `${system_id}:system_info` 将数据存储在 Redis 散列中。

## 加载站点数据

站点信息提供系统中每个站点的数据，包括系统的位置。`load_station_data` 应用程序迭代站点反馈中的每个站点，不能够使用形式为 `${system_id}:station:${station_id}` 的键将么个站点的数据存储到 Redis 散列中。使用 `GEOADD` 命令将每个站点的位置添加到共享单车的地理空间索引中。

## 更新数据

在随后的运行中，我不希望代码从 Redis中删除所有反馈数据并将其重新加载到一个空的 Redis 数据库中，因此我仔细考虑了如何处理数据的本地更新。

代码首先加载包含正在被处理到内存中的系统所有共享单车站点信息的数据集。当为站点加载信息时，从站点的内存集中删除站点(按键)。一旦加载了所有站点数据，我们就会得到一个包含了该系统必须删除的所有站点的数据集合。

应用程序迭代这组站点并创建一个事务来删除站点信息，从地理空间索引中删除站点键，并从系统的站点列表中删除站点。

## 代码注释

在[示例代码](https://gist.github.com/tague/5a82d96bcb09ce2a79943ad4c87f6e15)中有一些有趣的事情需要注意。首先，使用 `GEOADD` 命令将词条添加到地理空间索引中，但是用 `ZREM` 命令删除。由于地理空间类型基本实现使用排序集，因此使用 `ZREM` 删除词条。请注意，为了简洁，示例代码演示了如何使用单个 Redis 节点；需要对事物模块进行重构，以便在集群环境中运行。
...
Redis 提供两种用于存储数据的主要数据类型：散列和排序集。[散列类型](https://redis.io/topics/data-types#Hashes) 很好地映射到表示站点的 JSON 对象；由于 Redis 散列不强制执行关系模式，因此可以使用它们存储可变站点信息。
...
构建应用程序的下一步是确定用户的当前位置。大多数应用程序通过操作系统提供的内置服务来实现这一点。该 OS 可以为应用程序提供基于内置于设备中的 GPS 硬件或近似于设备的可用 WiFi 网络的位置。
...
最终， `GEORADIUSBYMEMBER` 命令类似于 `GEORADIUS` 命令，但该命令没有接受一组坐标，而是取索引的另一个成员的名称，并返回以该成员为中心所给定半径内的所有成员。要找到 Grand Army Plaza 和 Central Park 南1000 英尺范围内的所有车站，请输入以下内容：
...
虽然这个示例侧重于使用Python和Redis来解析数据并构建自行车共享系统位置的索引，但它可以很容易地推广到定位餐馆、公共交通或任何其他类型的地方，以帮助用户查找。
