...
好的，这里我们有你可以称之为 React 和 Vue 之间的交叉事件。使用 `use` 作为前缀是 React 的约定，所以如果你在 React 中查找 Hooks，你会发现 Hooks 的名称都会像 `useState`、`useEffect` 等。[更多信息可以查看这里。](https://reactjs.org/docs/hooks-overview.html#-state-hook)
...
但是当我们使用 Vue 风格的 Hook 时，我们会如何命名呢 —— 你猜对了 —— 比如：`useData`，`useComputed`等。
...
我们之前提到过，Hooks 和混入之间的主要区别之一是 Hooks 实际上可以互相传值。让我们看一下这个简单但有点不自然的例子。
...
你可以查看[这个仓库](https://github.com/yyx990803/vue-hooks)作为起步。Hooks 可能会成为混入的替代品，所以虽然这个功能还处于早期阶段，但是一个在此期间探索其概念是有好处的。
