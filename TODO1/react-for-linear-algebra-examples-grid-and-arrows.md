...
在本系列刚开始写作时，只有一个章节涉及了矢量的基本操作。因此，目前实现一个能渲染二维坐标网格并用箭头可视化矢量的组件就够用了。本文最后做出的效果如下图所示，你也可以在[此处](https://rodionchachura.github.io/linear-algebra/)进行体验。

![二维空间中的基本矢量操作](https://cdn-images-1.medium.com/max/3830/1*38t6SAlScgmBGjXQn9cTuA.gif)
...
为了在之后的示例中渲染矢量和其它东西，我们得制作一个好用的组件，这个组件需要有这么一种投影功能：将我们熟知的直角坐标系（原点在中间，y 轴正向朝上）投影到 **SVG** 坐标系（原点在左上角，y 轴正向朝下）中。
...
为了便于在不同的示例中使用这个网格组件，我们编写了一个 **GridExample** 组件，它可以接收两个参数：一个用于渲染信息（例如矢量的名称）的函数 `renderInformation`，以及一个用于在网格上呈现内容（如后面的箭头组件）的函数 `renderGridContent`。
...
这样就能在 view 中用这个组件了。下面以向量的加法为例测试一下：
