...
这篇文章，我希望能带领大家了解一下 [CSS animation property](https://css-tricks.com/almanac/properties/a/animation/)，以及详细地解释我的[个人网站](https://jesperekstrom.com/portfolio/malteser/)中的一个效果：让文字在移动的物体后出现。如果你想要看最后的成果，这里有一个[例子](https://codepen.io/jesper-ekstrom/pen/GPjGzy) 。
...
即使你对这个效果不是非常感兴趣，这仍将是一个可以扩展你 CSS 知识的好练习，你可以从这个效果开始创建属于你自己的动画效果。在我的例子中，在动画上进行深入的挖掘学习，让我对自己的 CSS 能力上有了更多的自信，并且让我更加有想象力，也让我对于整个前端开发都更加感兴趣了。
...
我们将使用一个靠谱的[转换小技巧](https://css-tricks.com/centering-percentage-widthheight-elements/)，来在的父元素中，将我们的 `position: absolute;` 的 div 水平垂直居中。因为我们希望我们的图片在文字之前，这里我们给它一个更大的 `z-index` 值。 
...
> 我们在整篇文章所有的例子中都没有保留浏览器前缀，但如果你在生产环境中使用这些效果，请务必考虑它。
...
> 我建议将所有的 `@keyframes` 添加到 CSS 文件的顶端，这样会有更好的结构，当然这只是我的个人喜好。
...
如果这里的 `cubic-bezier` 部分让让你感到头大，那就快看看 Michelle Barker 的[这个帖子](https://css-tricks.com/reversing-an-easing-curve/)。她深度的解释了这个话题。如果只是想要达到本文演示的目的，我觉得我这么说就够了：这是一个为物体的整个移动过程创建一个自定义动画曲线的方法。[cubic-bezier.com](http://cubic-bezier.com/#.5,.5,0,1) 网站是一个很好的可以帮助你生成这些值（而不是靠猜）的网站。
...
因为动画是基于固定的大小（像素），改变视口的宽度会造成元素们偏离，这对于想要通过位置来隐藏和揭示元素的我们来说，是非常糟糕的。我们可以在不同的 media queries 上创建多个动画来解决这个问题（这也是我最初的做法），但是一次处理这么多的动画可不是什么好玩的事。我们可以使用相同的动画，通过在特点的断点改变它的属性来解决这个问题。
...
我没有在之前的演示中使用这个方法，因为他不能很好地兼容 IE（你看，代码里有 -webkit 这个浏览器前缀）。在我们实际演示中使用的方法，如果把文字换成其他图像或者物件，仍然有效。

* * *

非常干净的小动画，是不是？它相当的细微，并且可以你的 UI 原色添色。例如，它可以用作揭示解释类文字甚至图片的标题。或者，在点击或者滑动事件上加一点 JavaScript，就可以触发动画，使得网页更加的具有交互性。
