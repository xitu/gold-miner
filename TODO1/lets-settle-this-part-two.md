...
嗨！欢迎来到让我们一起解决“this”难题的第二部分，我们试图揭开 JavaScript 中最难让人理解的一部分内容 - “this”关键字的神秘面纱。如果您还没有读过 [第一部分](https://github.com/xitu/gold-miner/blob/master/TODO1/lets-settle-this-part-one.md)，你需要先把它过一遍。在第一部分中，我们通过15个不同的示例讲解了默认绑定规则和隐式绑定规则。我们看到函数内的'this'是如何根据函数的调用方式的不同而改变。最后，我们也介绍了了箭头函数以及它们如何进行词法绑定。我希望你能记住这一切。

在这部分我们将讨论两个新规则，从 _new_ 绑定开始，我们将深入地分析一切是如何工作的。接下来，我们将介绍显式绑定以及如何通过 _call(...)_ ，_apply(...)_ 和 _bind(...)_ 方法将任意对象作为“this”绑定到函数上。

让我们接着上面的内容继续。你的任务还是一样，继续猜测控制台的输出内容是什么。还记得 WGL 吗？
...
我知道，现在你可能会想“发生了什么？为什么在这里将属性分配给函数？这不会导致错误吗？”好吧，首先，这不会导致错误。JavaScript 中的每个函数也都是一个对象。就像其他普通的对象一样，你也可以为函数指定属性！

接下来，让我们弄清楚控制台会输出什么。如果您注意下，你会发现隐式绑定在此处起作用。_c_ 的调用之前的对象是 _bar_ ，对吗？因此 _c_ 中的“this”指向的是 _bar_ ，因此 _bar_ 被输出到控制台中。
...
首先要注意，函数 **如何** 被调用。它不是一个独立调用，它的前面也没有对象引用。它有什么在呼叫前_new_。可以使用前面的_new_关键字调用JavaScript中的任何函数。当发生这种情况时，当用_new_调用一个函数时，大约会发生四个事件，其中两个是，
