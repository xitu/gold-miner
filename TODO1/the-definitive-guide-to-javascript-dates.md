...
尽管其被命名为 `Date` ，但其也操纵着 **具体的时间**（译者注：意为其也可以描述度分秒）。
...
从内部来看，Date 对象是表示距离 1970 年1月1日 (UTC) 所过去的毫秒数。这一天（1970 年1月1日）是非常重要的，因为就计算机而言，这是一切开始的地方。

你可能比较熟悉 UNIX 时间戳：它表示距离众人皆知的那天（1970 年1月1日）所过去的秒数。
...
如果我们传值为0，我们将会得到一个表示1970 年1月1日的 JavaScript Date 对象：
...
JavaScript 在没有找到任何关于时区的信息时，会把时区设置成UTC，同时也会自动地（对非当前时区的 Date 对象）进行到当前计算机时区的转换。
...
当你初始化一个 Date 对象时可以选择时区，这样做的话 Date 对象不会是默认的UTC时区，同时也会覆盖你所在的时区。
...
> `setDay` 和 `setMonth` 的取值范围从0开始，举个例子，三月的值为2。
