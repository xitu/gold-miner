...
我同时建议你使用 [命令行 / console 工具](https://www.davidbaumgold.com/tutorials/command-line/) 等等. 从现在起我统一叫它们终端. 这里可以看下它们的区别 [很明显](https://superuser.com/questions/144666/what-is-the-difference-between-shell-console-and-terminal).
...
首先我们需要一个文件夹来装我们的代码。 用一个你喜欢的方式随便建一个。 我把我新建的包叫做 **tiny** 因为它真的很小。 我为那些不熟悉命令行的人展示些新建相关的终端命令。
...
通过使用 **@bamblehorse/tiny** 代替 **tiny**, 我们会创建一个在我们用户吗 **scope** 下的一个包. 这个叫做 [**scoped package**](https://docs.npmjs.com/misc/scope). 它允许我们将已经被其他包使用的名称作为包名，比如说， [**tiny** package](https://www.npmjs.com/package/tiny) 已经在 npm 中存在。

你可能在一些著名的包中见过这种命名方法，比如来自 Google 的 [Angular framework](https://angular.io/)。 它们有几个 scoped packages ，比如 [@angular/core](https://www.npmjs.com/package/@angular/core) 和 [@angular/http](https://www.npmjs.com/package/@angular/http).
...
我们只需改变下指令来告诉 npm 我们想让每个人都可以使用这个模块 — 不要把它锁进 npm 的金库中。 所以我们执行如下指令:
...
[Version one](https://www.npmjs.com/package/@bamblehorse/tiny/v/1.0.0) 就躺在那呢!
...
GitHub 就是一个放代码的好地方。 先建一个 [new repository](https://github.com/new)。
...
这个名称肯定参考了 [James Bond reference](https://www.imdb.com/title/tt0097742/).

我实际上忘了添加许可证。

代码许可其实就是让别人知道在什么情况下才能使用你的代码。 这里有 [lots of different ones](https://choosealicense.com/) 供你选择。
