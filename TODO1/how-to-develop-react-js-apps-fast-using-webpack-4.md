...
**现在它应该能够启动一个本地服务器并使用您的应用程序自动打开浏览器选项卡。**
...
现在，让我们在本地为我们自己的新项目安装它。当然，此时 ESLint 会有很多设置。您可以在[官方网站](https://eslint.org/docs/about/)阅读更多相关信息。
...
最后一个命令将创建一个配置文件。系统将提示您选择以下三个选项：
...
您可以通过检查错误消息手动调试它，或者您可以使用它只需执行保存便自动修复问题的功能。
...
Prettier 是当今最流行的格式化程序之一，它已被编码社区广泛使用。它可以添加到 ESLint、[您的编辑器](https://prettier.io/docs/en/editors.html)，也可以被挂载在 git 的 pre-commit 钩子上。

![](https://cdn-images-1.medium.com/max/1600/1*l2Mh782tYEIhFY7nQrFr9Q.png)

我会在这里将它安装到我的 VS Code 中

安装后，您可以尝试再次检查代码。如果我们写一些奇怪的缩进并执行保存，它应该会自动格式化代码。

![](https://cdn-images-1.medium.com/max/1600/1*44ug1_PkUfZb07-H_mkvOg.gif)

但这还不够。为了使其与 ESLint 同步工作并且不会两次发出相同的错误，甚至发生规则冲突，您需要将它[与 ESLint 集成](https://prettier.io/docs/en/eslint.html)。

```
npm i --save-dev prettier eslint-plugin-prettier
```

在官方文档中，他们建议您使用 yarn，但 npm 现在同样也能安装。在你的 `.eslintrc.json` 文件中添加：
...
现在，当您执行保存时，您会看到代码自动格式化。那不是很漂亮（prettier）吗？双关语很有意思。
...
现在我们已经完成了很多工作，让我们快速回顾一下：ESLint 会监视代码中的错误，而 Prettier 是一种样式格式化工具。ESLint 有许多方法可以捕获错误，而 Prettier 可以很好地格式化您的代码。
...
如果您的 Prettier 插件在保存时没有自动格式化代码，您可以通过将下面的代码添加到 VS Code 设置来修复它：
...
您可以在[此处](https://eslint.org/docs/rules/no-unused-vars)和[此处](https://eslint.org/docs/user-guide/formatters/)更详细地了解这个配置。
...
现在您已经检查了代码，现在是时候向 React 应用添加更多组件了。到目前为止，你只有两个，但在大多数情况下，你会有几十个。
...
注意：您必须以正确的方式配置环境，否则 source maps 将不起作用。你可以在[这里](https://medium.com/@riittagirl/how-to-solve-webpack-problems-the-practical-case-79fb676417f4)阅读我的调试过程。下面我将为您梳理一个流程并解释我如何解决该问题。
