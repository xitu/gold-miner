...
列表推导式的批评者总是抱怨它们的可读性太差。他们是对的，很多推导式**都**很难读。**一些时候，这些推导式变的更易读的方法仅仅是多一点间隔**。
...
推导式是一种特殊用途的工具，用于在迭代器上循环，以便在修改每个元素的同时创建一个新列表，并/或向下过滤元素。`list` 构造函数是一个特定目的工具，用来遍历推导式并创建列表，同时不会改变任何的东西。
...
`list` 和 `dict` 的构造函数不是唯一的推导式替代工具。标准库和第三方库中包含了很多工具，他们在一些时候比推导式更适合于你的循环需要。
...
所以，除了的考虑“我是否可以从一个循环复制-粘贴到推导式”的检查之外，我们还需要考虑：我们的代码是否可以通过生成器表达式与接受迭代器的函数或者类的组合来增强？

那些可以**接受迭代器作为参数**的函数或者类，**可能与生成器表达式组合的**必要条件。
