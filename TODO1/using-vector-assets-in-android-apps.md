...
在之前的帖子中，我们研究了 Android 的 `VectorDrawable` 图像格式以及它能够实现的功能：
...
在这篇文章中，我们将会深入研究如何在你的 app 中应用这些矢量资源。 `VectorDrawable` 是在 Lollipop（API 21） 中引入的，也可以在 AndroidX 中使用（作为 `VectorDrawableCompat` ），可以向下支持到 API 14（这使其可以覆盖超过 [99％ 的设备](https://developer.android.com/about/dashboards/)）。本文将概述一些建议，以方便你决定是否需要在实际的应用中使用 `VectorDrawables` 。

## 首先是 AndroidX 

从 Lollipop 开始，你可以在任何需要使用其他可绘制类型的地方使用 `VectorDrawables` （使用标准的 `@drawable/foo` 语法引用它们），但是我建议**始终**使用 AndroidX 实现。

这显然增加了其使用平台的范围，不仅如此，它还支持将特性和 bug 修复程序向后移植到旧平台。例如，使用 AndroidX 中的 `VectorDrawableCompat` 可以:

- `nonZero` 和 `evenOdd` 路径 `fillTypes` —[定义形状“内部”](https://www.sitepoint.com/understanding-svg-fill-rule-property/)的两种常见方法，通常用于 SVGs （`evenOdd` 在 API 24 中被添加实现）
- 渐变（Gradient） & `ColorStateList` 填充 / 画笔 （在 API 24 中被添加实现）
- Bug修复

事实上， AndroidX 将使用 compat 实现，即使在一些存在本地实现的平台上（[当前是 api 21-23](https://android.googlesource.com/platform/frameworks/support/+/androidx-master-dev/appcompat/src/main/java/androidx/appcompat/widget/AppCompatDrawableManager.java#100) ）也可以实现上述优点。否则，它将委托给平台实现，因此仍然可以接收对新版本的任何改进（例如，为了提高性能， `VectorDrawable` 在 API 24 的 C 中重新实现)。

基于这些原因，你应该**始终**使用 AndroidX ，即使你很幸运的将你的 `minSdkVersion` 设置成 24 。这没什么不好的，如果/当 `VectorDrawable` 在未来扩展了新的功能，并且它们也被添加到 AndroidX 中，那么它们就可以直接使用，而不需要重新检查代码。
...
如果 `minSdkVersion` < 21，这意味着 Android Gradle 插件无法[生成矢量资源的 PNG 版本](https://developer.android.com/studio/write/vector-asset-studio#apilevel)——但如果我们使用 AndroidX 库的话就不用担心这个问题。
...
> `VectorDrawableCompat` 还提供了一个 `create` 方法。 我建议总是使用 `AppCompatResources`，因为这会增加一层缓存。
...
要解决这个问题，可以在代码中创建 `drawables` ；也就是说，使用 `AppCompatResources` 加载 向量，然后手动创建 `InsetDrawable` 格式的 drawable。

有一个例外是 AndroidX 最近添加了一个新功能（从 [`appcompat:1.0.0`](https://developer.android.com/jetpack/androidx/androidx-rn#1.0.0-new) 开始）——后移植 [`AnimatedStateListDrawables`](https://developer.android.com/reference/androidx/appcompat/graphics/drawable/AnimatedStateListDrawableCompat) 。这是 `StateListDrawable` 的一个版本，具有状态之间的动画转换(以 `AnimatedVectorDrawables` 的形式)。你不需要申明一个过渡。因此，如果你只需要一个可以使用 AndroidX 来扩充子向量的 `StateListDrawable` ，那么你可以使用：
...
> 有一种方法可以在嵌套的 drawable 中启用向量，通过使用 [AppCompatDelegate#setCompatVectorFromResourcesEnabled](https://developer.android.com/reference/android/support/v7/app/AppCompatDelegate.html#setCompatVectorFromResourcesEnable) ，但它有许多缺点。务必仔细阅读 javadoc。
...
请注意，除了api级别限定符之外，我们还需要在此处包含 `anydpi` 资源限定符。这是由于[资源限定符优先级](https://developer.android.com/guide/topics/resources/providing-resources#BestMatch)的工作方式导致的。任何在 `drawable- <whatever> dpi` 中的资源都被认为是比在 `drawable-v21` 更好的选择。
