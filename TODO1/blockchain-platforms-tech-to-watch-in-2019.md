...
*   [**Waves RIDE**](https://docs.wavesplatform.com/en/technical-details/ride-language.html)：一种图灵不完整（无循环或递归）的类 Haskell 式函数式程序语言，用于 Waves 区块链。它具有静态类型、延迟评估、模式匹配和用于确定是否允许事务完成的谓词表达式。图灵完整版本也在开发中。Waves 的智能合约支持目前正在主网上运行。我们应该能在2019年看到第一批 Waves dApp 出现。
