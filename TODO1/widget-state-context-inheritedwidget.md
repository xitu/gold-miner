...
当你需要直接或间接地构建与布局相关的任何内容时，你正在使用 **Widget**。
...
如果我们现在考虑这个基本示例，我们将获得以下 Widget 树结构（**限制代码中存在的 Widget 列表**）：
...
**Context** 仅仅是已创建的所有 Widget 树结构中 Widget 的位置参考。

> 简而言之，将 **context** 作为 Widget 树的一部分，其中 Widget 被附加到此树中。
...
**参数**可以是装饰、尺寸、甚至其他 widget 中的任何内容。唯一重要的是该配置一旦创建，在下一个构建过程之前都不会改变，除此之外都不重要。

> stateless widget 只有在 loaded/built 时才会绘制一次，这意味着无法基于任何事件或用户操作重绘该 Widget。

##### Stateless Widget 生命周期

以下是与 **Stateless Widget** 相关的典型代码结构。

如你所见，我们可以将一些额外的参数传递给它的构造函数。但请记住，这些参数在后续阶段将**不**改变（变化），并且必须**按照已有状态**使用。
...
> **极其重要的**:
>
> 作为 **State 对象** 与其相关的 **context**，这意味着该 **State 对象**是**不**（直接）访问**另一个 context** ！（我们将在稍后讨论该问题）。
...
在该方法中，**context** 是可以得到的，但是你**还不能**真正使用它，因为框架还没有完全将其与 state 关联。

一旦 **initState()** 方法执行完成，此时 State 对象被初始化并且 context 变为可用。
...
第二部分 “**_MyStatefulWidgetStat**” 是管理 Widget 生命周期中的**变化**，并强制每次应用修改时重建该 Widget 实例。名称开头的 ‘**\_**’ 字符使得该类对 .dart 文件是**私有的**。

如果你需要在 .dart 文件之外引用此类，请不要使用 ‘**_**’ 前缀。

**`_MyStatefulWidgetState`** 类可以通过使用 使用 **widget.{变量名称}** 来访问被存储在 **MyStatefulWidget** 中的任何变量。在该示例中为：**widget.color**。

* * *

## Widget 唯一标识 - Key

在 Fultter 中，每一个 Widget 都是被唯一标识的。这个唯一标识**在 build/rendering 时**由框架定义。
...
让我们考虑当用户点击按钮时显示 SnackBar 这样一个基本示例。由于 SnackBar 是 Scaffold 的子 Widget，它不能直接访问 Scaffold 内部任何其他子节点（**还记得 context 的概念以及其层次/树结构吗？**）。因此，访问它的唯一方法是通过 **ScaffoldState**，它暴露出一个公共方法来显示 SnackBar。
...
简而言之，**InheritedWidget** 允许在 **widget** 树中有效地传播（和共享）信息。
...
以上代码定义了一个名为 “**_MyInheritedWidget**” 的 Widget，目的在于“**共享**”所有 widget（子节点子树中的一部分）中的某些数据。
...
class MyChildWidget... {
   ...
   Widget build(BuildContext context){
      ...
      ///
      /// 此刻，该 widget 可通过调用：inheritedWidget.data 来使用 MyInheritedWidget 暴露的数据
      ///
      return new Container(
...
针对该场景，**InheritedWidget** 是唯一一个正确的 Widget！
...
*   **MyInheritedWidgetState** 暴露了一个 **itemsCount** getter 和一个 **addItem** 方法，以便它们可以被 widget 使用，这是**子** widget 树的一部分
...
7.  迭代整个**消费者**列表（这里是 Widget A 和 Widget B）并请求它们重建
...
#### 在继续访问 Inherited Widget 的同时组织某些 Widget 重建
...
在下一篇文章中我将介绍 **Notifiers / Listeners** 的概念，这在使用 **State** 和传递数据的环境中也非常有趣。
