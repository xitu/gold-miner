...
前端 Web 开发[不断增加的复杂性](https://blog.logrocket.com/the-increasing-nature-of-frontend-complexity-b73c784c09ae) 促进了前后端专业化和前后端分离的不断增长。
...
前端和 API 之间的接口已成为障碍、挑战和复杂性的共同点。

理想状态下，后端和前端将通过紧密的通信协调一致地演变，而后端提供的数据将完全匹配前端所需的数据。

实际上，应用程序的两个部分通常由完全不同的团队甚至不同的公司开发。 在设计部门和前端部门之中建立一个专门的团队并不罕见，而你自己的开发团队正在研究后端

### 典型解决方案

结果是一个通常看起来如下所示的流程：

1. 使用伪造的 'stubbed' 数据构建 UI，可以直接嵌入模板和代码中，也可以通过一个工具包加载。
2. 当 API 准备就绪时，迅速使用真实的 API 请求和数据替换每一处集成点。

这种方法存在的问题是双重的：

1. 数据集成通常分散在整个应用程序中，需要跟踪并重新编写大量代码。
2. 即使数据是相对独立的，前端期望与API最终提供的内容之间往往存在不匹配。

### 更好的解决方案：VueX

如果您使用 Vue.js 开发您的前端，那么一个更好的解决此问题的方法就在您的指尖。

深度集成到 Vue 中的 [VueX](https://vuex.vuejs.org/en/) 库提供了一种完美的解决方案，可以为您的数据创建一个**干净的**、**隔离的**接口，从而使残断数据（stubbed data）和实际API之间的转换变得轻而易举。
...
VueX将状态管理分为 3 个关键部分：**state**，**mutations** 和 **actions**。 当你实例化一个 VueX 存储时，你可以定义这三个对象：
...
Mutations 是**同步的**，并**直接**修改状态对象（比如与 Redux 相比，有一个同等的概念称为 reducer 并返回 **new** 对象）。
...
操作是**异步的**，并且**间接地**通过`commititting` mutations 来修改存储。 但是，由于它们是异步的，因此它们可以做的远不止这些。

异步允许 actions 处理诸如 API 调用，用户交互和整个 action 流程等事情。

作为一个简单的例子，一个 action 可能会进行一次 API 调用并记录结果：
...
Actions 可以返回 promises，允许视图或其他代码分派 actions ，等待它们完成并根据其结果做出反应。你应该使用 `dispatch` 来分派一个 action 。而不是使用 `commit`。 例如，我们的调用代码可能如下所示：
...
如果您正在开发一个后端和前端同时进行的项目，或者您是一个UI /前端团队，您甚至可能在后端存在之前就构建用户界面，那么您可能会在开发前端是经常面临这样的问题，即需要将后端的部分数据截取出来。

这种体现的常见方式是纯静态模板或内容，在前端模板中使用合适的占位符和文本。
...
VueX的优点在于，actions 提供了一种**完美的**方式来对前端和后端进行隔离和抽象，并且以这种方式进行操作，以便从残断数据（stubbed data）更新到真正的后端是无缝和简单的。
...
现在，我们的前端可以通过测试数据实现一种登录方式，这种登录方式在将来也是以**同样的**方式运行的，可以同时实现成功和失败的登录请求。 这种行为将**立即**发生，而不是通过API异步实现，但通过返回 promises ，现在任何调用者都可以像对待真正的API调用一样对待它。

当我们的 API 可用时，我们可以简单地修改这个 action 以使用它，并且我们的代码库中的其他所有内容都保持不变。

### 处理数据不匹配的问题

将我们的 API 调用与 VueX 隔离也为我们提供了一种漂亮而干净的方式来处理后端和前端之间的数据格式不匹配的问题。

继续使用我们的登录示例，或许我们假设 API 将返回我们在登录时需要的所有用户信息，但是我们需要在通过身份验证后从单独的端点获取首选项，即使此格式与我们预期的格式不同

我们可以将这种差异完全隔离在我们的 VueX 动作中，防止我们需要改变我们前端的其他任何地方。 由于 promises 可以链式调用和嵌套，我们可以在我们的行为被视为完成之前，通过一系列需要完成的 API 调用。
...
从我们的状态修改和分派我们的 `login` 动作的代码的角度来看，最终的结果是**完全**相同的。

借助 VueX，将新的或变化的后端API集成到我们的前端的挑战已经大大简化。
