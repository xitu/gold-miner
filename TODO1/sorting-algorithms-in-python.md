...
> * 校对者: [JalanJiang](https://github.com/JalanJiang)
...
与选择排序一样，该算法将列表分为已排序部分和未排序部分。它会通过迭代未排序的部分来将所查看的元素插入到排序列表中的正确位置。
...
这种分而治之的算法将一个列表分成两部分，并一直将剩下的列表分为一分为二直到列表中只剩下一个元素为止。

相邻元素成为排序对，然后合并排序对并和其它对进行排序。这个过程将一直持续到我们得到一个对未排序输入列表中所有元素排序的排序列表为止。
