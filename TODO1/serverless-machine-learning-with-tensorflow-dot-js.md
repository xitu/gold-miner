...
无服务的平台按需初始化运行环境用以处理调用。一旦运行环境被创建，他将会对[重新调用](https://mediumcom/openwhisk/squeezing-the-milliseconds-how-to-make-serverless-platforms-blazing-fast-aea0e9951bd0)有一些限制。
...
对冷激活和热激活（使用 256 MB 内存）调用 `classify` 动作1000次会产生以下结果。
