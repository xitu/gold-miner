...
这个函数将会在 `document` 树中查找以 `foo` 作为 ID 的节点。让我们更新我们的 `Node` 结构来获得更多的属性，然后为我们的树结构编写一个查询函数：
...
`hasClass` 获取 `Node` 结构的 classes 字段，通过空格字符来分割它们，然后再循环这个 classes 的切片并尝试查找到我们想要的类名。让我们来写几个测试来验证这个函数：
