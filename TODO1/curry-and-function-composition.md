...
随着主流 JavaScript 函数式编程戏剧般的兴起，在许多应用中柯里化函数变得普遍起来。理解它们是什么、如何运作和怎样有效地运用非常重要。

### 什么是柯里化函数？

柯里化函数是一种由需要接受多个参数的函数转化为**一次只接受一个**参数的函数。如果一个函数需要 3 个参数，那柯里化后的函数会接受一个参数并返回一个函数来接受下一个参数，第二次返回的函数传入第三个参数。最后一个函数会返回应用了所有参数的函数结果。
...
为了使用它，我们必须使用函数应用语法应用到每个函数上。在 JavaScript 中，函数后的括号 `()` 触发函数调用。当函数返回另一个函数，被返回的函数可以通过额外的括号被立即调用：
...
`add` 函数接受一个参数，然后使用在闭包作用域中固定的 `a` 返回自己的 **偏函数应用**。**闭包**指函数绑定其语法作用域。闭包在创建函数运行时被创建。固定意味着在闭包绑定的作用域内变量被赋值。
...
使用无点风格创建一个函数，该方法会把你传入的任何数字加一。记住，我们已经有一个叫 `add` 的函数，它需要一个数字，并且无论你传入了什么值都会返回一个第一个参数固定的偏函数。我们可以使用这种方法创建一个叫 `inc()` 的新函数。
...
作为一种归纳和专用机制，这很有趣。返回的函数不过是更加通用的 `add()` 函数的一种**专用版**。我们可以按需要使用 `add()` 来创建许多专用版本。
...
当我们调用 `add(1)` 来创建 `inc()` 时，`add()` 中的 `a` 参数被**固定**为 `1`，然后 `a` 在返回的函数中被 `inc` 使用。
...
我们可以随你意愿把任意多个函数组合成一个函数。换句换说，`compose()` 在函数中创建了一个管道，把一个函数的输出与下一个函数的输入连接起来。
...
但是柯里化函数的真正能力是它们可以简化函数组合。一个函数可以使用任意数量的输入，但是只返回一个输出。为了使函数可组合，输出类型必须与期望输入类型统一：
...
如果 `g` 上面的函数期望两个参数，`f` 的输出就会和 `g` 的输入不一致：
...
不过更好的方式是在开始就写出正确的函数。有时这种风格被称为“数据置后”，这意味着你需要首先专用化参数，并在最后传入参数执行函数。这里展示了原始的函数形式：
