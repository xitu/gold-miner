...
这么做的主要意图是用合理的新数据去扩展训练数据。换句话说，我们可以让模型看到更多样性的训练数据。举个例子，如果我们对一只猫进行水平的翻转，这个是有意义的，因为摄像头的拍摄角度可能是左边也可能是右边。但是做垂直翻转就没什么意义了，因为模型不太会接收到一个头上脚下的猫。

所以，我们应该明白，我们一定要根据训练数据和问题领域的相关上下文来慎重的选择应用于训练集的数据增强方法。此外，有一种比较有效的方法，就是在小的原型数据集上做一些独立的实验来度量增强后的模型是否在性能上有所提升。

现代的深度学习方法，像卷机神经网络或者CNN，都可以学习到图片特征种的位置不变性。数据增强可以帮助模型去学习这种性质并且可以使得模型对一些变化也不敏感，比如左到右和上到下的顺序、照片的光照变化等等。

这些图片数据的增强一般是应用于训练集而不是验证集和测试集。这些方法不同于预处理阶段的图片大小调整和像素值的缩放，他们需要在与模型交互的数据集上保持一致。
...
Keras 框架可以在训练模型时，自动的使用数据增强。

可以利用 [ImageDataGenerator 类](https://keras.io/preprocessing/image/) 达到这一目的。

首先，可以在类实例化时传入特定的参数到构造函数来配置对应的增强方法。

该类支持一系列的增强方法，包括像素值的缩放，但是我们只关注五种主要的图像数据增强方法，罗列如下：

*    图像平移通过 _`width_shift_range`_ 和 _`height_shift_range`_ 参数设置。
*    图像翻转通过 _rotation_range_ 参数设置。
*    图像亮度通过 _brightness_range_ 参数设置。
*    图像缩放通过 _zoom_range_ 参数设置。
...
_`steps_per_epoch`_ 参数需要设定为能包含整个数据集的批次数。举个例子，如果你的原始数据是 10000 张图片，同时你的 batch_size 设置为 32 ，那么当你训练一个基于增强数据的模型时，一个合理的 _`steps_per_epoch`_ 应该设置为 _ceil(10,000/32)_ ，或者说 313 批次。
...
数据集中的图片并没有被直接使用，而是将增广后的图片提供给模型。由于增强的图片表现是随机的，同时容许修改后的图片以及接近原图（例如，原图）的数据被生成并在训练中使用。
...
平移意味着将图片上的所有像素沿着某一个方向移动，可以是水平或者垂直，同时要保证维度没有变化。

这也意味着一些原有的像素点会被移出图片，那么就会有一个块区域的像素值需要重新指定。
...
执行这段代码，通过配置 _ImageDataGenerator_ 会生成一个图片增强实例，之后是创建一个迭代器。这个迭代器会在一个循环中被执行 9 次并画出每一的结果。
...
下面是类似的例子，通过调整 _`height_shift_range`_ 参数实现垂直平移，其中该参数被设置为0.5。
...
图片的翻转就是在水平翻转或者垂直翻转的情况下对应的反转行或者列的像素值。

翻转的参数是构造 _ImageDataGenerator_ 类时，分别通过 boolean 型的参数 _horizontal_flip_ 或者 _vertical_flip_ 来指定的。针对于之前提到的鸟的图片，水平翻转时有意义的，而垂直翻转时没什么意义的。
...
执行这个例子，会产生一个旋转图片，其中空白区域是通过最邻近法进行填充的。
...
*    图像数据增强时为了扩展训练数据集，从而提高模型的性能和泛化能力。
