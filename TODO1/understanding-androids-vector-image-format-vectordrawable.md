...
矢量图可以优雅的调整大小；这是因为它们将图像刻画在抽象大小的画布上，你可以放大或缩小画布，然后重新绘制对应尺寸的图像。然而，栅格资源在重新调整大小后会变得很糟糕。缩小栅格资源是可以的（意味着你在丢失一些信息），但是放大它们会因为插入丢失的像素，而变成模糊或条带这样的工件。
...
Android 选择最接近的较大密度并将其缩小（如果需要）。随着设备具有更高的屏幕密度的趋势,应用开发者必须不断创建、囊括、输送相同资源的更多版本。请注意，许多现代设备并不遵从精确的密度（例如，Piexl 3 XL 是 552 dpi，介于 xxhdpi 和 xxxhdpi 之间），因此资源通常会被缩放。
