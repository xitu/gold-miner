...
大多数的图像格式（png、jpeg、bmp、gif、webp 等等）都是栅格，这意味着它们用固定像素的网格来刻画图像。因此，它们是在特定分辨率下定义的，只了解每个像素的颜色，却不理解其中的内容。然而，矢量图是通过在抽象大小的画布上定义的一系列形状来描述图像。
...
矢量图可以优雅的调整大小；这是因为它们将图像刻画在抽象大小的画布上，你可以放大或缩小画布，然后重新绘制对应尺寸的图像。然而，栅格资源在重新调整大小后会变得很糟糕。缩小栅格资源是 OK 的（意味着你在丢失一些信息），但是放大它们会因为插入丢失的像素，而变成模糊或条带这样的工件。

![](https://cdn-images-1.medium.com/max/800/1*Z_ol_Ajp2SsMNx3DHKUgfQ.png)

来自（极端）放大光栅图像（左）与矢量图像（右）的伪像

这就是为什么在 Android 上我们需要为不同密度的屏幕提供多个版本的光栅资源：
...
Android选择最接近的较大密度并将其缩小（如果需要）。随着设备具有更高的屏幕密度的趋势,应用开发者必须不断创建、囊括、输送相同资源的更多版本。请注意，许多现代设备并不遵从精确的密度（例如，Piexl 3 XL 是 552 dpi，介于 xxhdpi 和 xxxhdpi 之间），因此资源通常会被缩放。
...
例如 [这里是一个改变](https://github.com/google/iosched/commit/78c5d25dfbb4bf8193c46c3fb8b73c9871c44ad6) [Google I/O app](https://play.google.com/store/apps/details?id=com.google.samples.apps.iosched) 通过将一些 PNG 图标从栅格转换成矢量节约了 482 KB。尽管听上去不是很多，但这仅仅是对小图像而言；更大的图片 (如插图) 会节省更多。
...
我们无法用 VectorDrawable 替换它，因为当时没有广泛支持渐变（现在可以），所以我们不得不发布一个光栅版本😔。如果我们能够使用矢量，那么这将是以 30% 的大小获得更好的结果:

*   Raster: Download Size = 53.9KB (Raw file size = 54.8KB)
*   Vector: Download Size = 3.7KB (Raw file size = 15.8KB)

> 请注意，虽然 [Android App Bundle’s](https://developer.android.com/platform/technology/app-bundle/) 的密度配置拆分仅通过向设备提供所需的密度资源而带来类似的好处，但 VectorDrawable 通常仍然会更小，并且无需继续创建更大的栅格资产。
...
正如前面所诉，矢量描述了它们的内容，因此它们需要在使用前进行 inflate 和 draw。
...
然而，SVG 包含一个 [路径规范](https://www.w3.org/TR/SVG/paths.html)，它定义了如何描述和绘制形状。使用此 API，您可以表达大多数矢量形状。这基本上是Android 支持的：SVG 的路径规范（加上一些补充）。
...
如上所述，VectorDrawable 支持 [SVG路径规范](https://www.w3.org/TR/SVG/paths.html)，允许您指定要绘制的一个或多个形状。它是作为 XML 文档创作的，如下所示：
...
请注意，您需要指定资源的内在大小，即通过 ImageView 的 wrap_content 设置它的大小。第二个视口大小定义虚拟画布，或者定义所有后续绘制命令的坐标空间。内在和视口尺寸可以不同（但应该以相同的比例） - 如果你需要，可以在 1*1 画布中定义矢量。
...
根`矢量`元素支持 `alpha` 属性 [0,1]。组没有 alpha 属性，但各个路径支持 `fillAlpha` 和 `strokeAlpha`。
