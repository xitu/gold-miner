...
例如，当我们在博客中阅读文章时，我们首先会看到标题，然后是副标题，然后才是副本块。这是否意味着副本块中的信息具有较低的重要性？其实不是这样，但通过这种方式用户就可以扫描标题和副标题，以了解文章是否对他们有用和有趣，而不用试图阅读所有文本。如果标题和副标题起的恰当，它们能够告知用户文章的结构和内容，这将是令人信服的阅读更多的因素。另一方面，如果用户看到又大又长的没有分块的文本，他们会感到很害怕，无法得知阅读这篇文章需要多长时间，以及是否值得投入时间和精力。
...
所有上文提到的眼动扫描模式显示，无论特定用户遵循哪种模式，扫描过程都会从网页的顶部水平区域开始。用它来展示交互和品牌的关键区域是很好的战略。这也是 UI / UX 设计师、内容管理者和营销专家认为[网站头部设计](https://uxplanet.org/best-practices-for-website-header-design-e0d55bf5f1e2)是一个基本问题的根本原因。
