...
# 使用 Go 语言的流模式来解析 DrugBank 的 XML（或者任何大的 XML 文件）
...
事实上，我想要的在 [Structure External Links](https://www.drugbank.ca/releases/latest#structures) 链接下载的 CSV 文件已经有了。使用这种方式解析似乎还有一些其它的用处，因为 XML 格式的 DrugBank 版本似乎比单独的 CSV 文件包含更多的信息。所以不管怎么样，这迫使我想出如何在 Go 中使用流模式来解析大型 XML 文件的方法，像  [XMLStarlet](http://xmlstar.sourceforge.net/) 这些旧的工具会在处理 DrugBank 文件阻塞好几分钟（也许是试图把文件的内容全部读入内存？），这让人在迭代开发周期中失去了任何想法。而且，Go 对流式解析 XML 的支持非常棒。

尽管 Go 对流式解析 XML 的支持很不错，但在文档里面并没有清晰的细节讲述如何使用流的方式来实现这个，还好 [David Singleton](http://twitter.com/dps) 的[这篇博客](http://blog.davidsingleton.org/parsing-huge-xml-files-with-go/) 拯救了我。基本上，你可以从他的文章来开始学习，但是我也想写一篇自己的博客来记录过程中想到的一些具体细节和特征。

## 想法：把 DrugBank 的 XML 解析为 TSV

简而言之，我们想要解析 DrugBank 的 XML 文件，这文件里包含了数据集中每种药物的大量分层信息，并只想提取其中的部分字段，然后把这些字段信息输出到由制表符分隔格式优美的（.tsv）文件中。

以下是在这个例子中针对每种药物我们想要提取的字段（基于上面提到的现实世界问题）：
...
DugBank 的 XML 格式在其最外层是最简单的：它基本上只包含了很多在 `<drugbank></drugbank>` 闭标签里的 `<drug></drug>` 元素。 `<drug>` 标签内的相对来说比较复杂。但因为 Go 使用标签来将 XML 解析为结构体，我们可以跳过大部分信息，而只关注感兴趣的部分。
...
下面你可以看到我定义的结构体构成（听起来是对的，对吗？）从中可以解析出我感兴趣的数据：
...
设置好结构体之后，我们就可以写 Go 代码了，这份代码会按照 David 的[博客](http://blog.davidsingleton.org/parsing-huge-xml-files-with-go/)以流的方式循环遍历读取一个 XML 文件，同时也会创建一个 TSV 写入器，这样我们就可以用流的方式将提取到的输出写入到一个新文件 drugbank_extracted.tsv （为简洁起见，导入和主函数都省略了）。
...
现在，我们可以使用 SciPipe （我正在开发的基于 Go 的工作流库）将它放到一个小工作流中，在这里我们会自动下载DrugBank数据，解压缩它之后再运行 XML 到 TSV 的 代码。 查看这个 [gist](https://gist.github.com/samuell/fc82fad39e7efda7987fc18173777f7f) 来了解完整的工作流代码。
...
*   确保你已经安装了 curl，在Ubuntu上：可以使用 `sudo apt-get install curl` 命令安装
...
我还在下面列出了完整的 SciPipe 工作流代码，会一直维护到 Github 关闭的那一天;）：
