...
Google Play 是怎么实现这种瘦身的呢？这一做法能够分发针对个别设备的优化包，这么一来，相应工具就能从安装包中移除所有不适用于设备的语言资源、分辨率资源以及本地库。比如，如果你的设备设置是 `fr-FR`（法语），分辨率是 `xxhdpi` ，CPU 是 `arm64-v8a` 架构的，下发的 APK 便只会包含必要的资源，而不会包含诸如针对西班牙语本地化的字符串之类的东西。当发现本地化字符串占用的空间有多大时，你一定会大吃一惊。
