...
我知道你在想什么，在已有那么多测试库的情况下再自己造轮子？其实不是，本文是关于如何能够**逆向工程**，以及理解背后发生的事。这么做的目的，是为了能够让你更广泛同时更深刻你所使用的库。
...
很多年前，在我作为一位软件开发人员的职业生涯开始的时候，我询问过一些高级开发人员他们如何变得更好。其中一个突出的答案是**逆向工程**，或者更确切地说是重建他们正在使用或者感兴趣的框架或库。

> 对我来说听起来像是在试图重新发明轮子。有什么好处，难道我们没有足够的库来做同样的事情吗？
...
这就是我们将要构建的东西 —— 让上述代码成功运行并且在构建过程中评论架构好坏，有可能的话，放进一个库里让它看起来漂亮：）
...
`expect()` 看起来像是接受一个 `boolean` 作为参数的函数，它返回一个对象，对象有一个 `toBe()` 方法，这样就可以将 `expect()` 的值以及传递给 `toBe()` 函数的值进行比较。让我们试着去写出大概：
...
另外，如果匹配成功或者失败，我们应该产生一些声明。因此需要更多代码：
...
类似这样比较两个值的函数例如 `toBe()` 被称为 `matcher`。让我们尝试添加另一个 matcher `toBeTruthy()`。**truthy** 匹配 JavaScript 中的很多值，所以我们可以不用 `toBe()` 去匹配所有东西。
...
如上所示，我们不仅捕获了错误并打印日志，我们还重新抛出错误以终止运行。再次，这样做的主要原因是因为我们认为报了错就没有必要继续测试了。你可以以合适的方式实现这个功能。
