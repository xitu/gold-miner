...
> **（伪）选择器可以为文档中不一定具体存在的结构指定样式，或者为某些元素（甚至是文档本身）的状态所指示的幻像类指定样式。**
> **— CSS 权威指南: Eric Meyer, Estelle Weyl**

这篇文章鼓励构造 UI 时使用更多普通的 CSS 和更少的 JS。熟悉所有的 CSS 是实现这个目标的一种方法 —— 另一种是实施最佳实践和尽可能的减少代码。
...
这个伪类选择器多用于使用 CSS 变量存储全局自定义属性，因为它应用于根元素。
...
这将选择 `innerDiv` 列表元素中包含的最后一个子元素，其类型为段落元素。
...
`innerDiv` 中最后一段的子元素 `p`（“Do the same”）将会被选中。
...
这将选中带有 `href` 属性且未点击访问过 `href` 指定的界面的所有的 `a` 锚点元素，选中的元素中的文字将会显示为橙色。
...
就这些了。这是全部内容。还有更多的伪选择器，但是为非标准的，因为我省略了它们。
