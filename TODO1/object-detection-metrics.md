...
在**目标检测评价问题**中，没有统一的评价不同工作或者实现的标准，这正是本项目的动机。尽管网上各类竞赛都使用它们自己的方式去评价目标检测任务的效果，但只有部分提供用于计算检测准确率的参考代码。

那些想要在不同的数据集上测试自己的方法而不是仅仅使用单一竞赛提供的测试数据集的研究者们需要自己实现相关的评价方法。而有时一个错误或者不同的实现方法会导致不同且有偏差的结果。为了获得值得信赖的能够应对不同的方法的标准，更理想的方式是提供一种灵活的实现方式，使得所有人都可以使用，无论使用的是什么数据集。

**本项目提供了易于使用的各类函数，这些函数实现的评价标准与目标检测领域最受欢迎的几大竞赛使用的评价标准相同**。我们的实现不需要你更改你的模型以适应各种复杂的输入格式，也避免了在 XML 和 JSON 文件之间的相互转换。我们对输入数据进行了简化（bbox 标签以及检测结果）并且将学术界和各类竞赛主流的评价标准集合在了一起。我们的实现与各类官方实现进行了仔细的对比，并且我们的结果与官方完全一致。
...
PASCAL VOC 的 MATLAB 版本评价代码从 XML 文件读取标签。当你需要应用到其它的数据集或者自定义的数据集时，就需要做出修改。尽管诸如 [Faster-RCNN](https://github.com/rbgirshick/py-faster-rcnn) 等工作实现了 PASCAL VOC 的评价标准，也需要把检测结果转化为特定的模式。 [Tensorflow](https://github.com/tensorflow/models/blob/master/research/object_detection/g3doc/evaluation_protocols.md) 框架也实现了相应版本的 PASCAL VOC 评价标准。

* **[COCO Detection Challenge](https://competitions.codalab.org/competitions/5181)** 使用不同的标准来衡量不同的目标检测方法的准确率。[这里](http://cocodataset.org/#detection-eval)你可以找到解释12种用于确定一个目标检测器在 COCO 上的性能的标准的文档。该竞赛提供了 Python 和 MATLAB 版本的代码，以便参赛者在提交结果前检验他们的得分。同样，将输出转化为特定的[格式](http://cocodataset.org/#format-results) 也是必要的。
...
精度×召回率曲线是评价目标检测算法一个很好的方式，因为在为每一类目标绘制该曲线的过程中置信度阈值是不断变化的。目标检测器在检测某一类物体时将会获得正面的评价如果随着该检测器召回率的上升，其精度始终保持很高的水平。因为这意味着无论你怎么改变置信度阈值，精度和召回率都很高。另一个判断检测器好的方法是该检测器只会检测正确的物体（高精度）同时能够找到所有的物体（高召回率）。  
...
自2010年开始，PASCAL VOC 计算 AP 的方法已经改变。目前， **PASCAl VOC 使用的插值方法使用了所有的数据点，而不是他们[文章](http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.157.5766&rep=rep1&type=pdf)所说的 11 个等间隔的点**。由于我们需要复现他们默认的标准，我们默认的代码遵循了他们最新的标准（使用所有的数据点）。然而，我们也提供了 11 个插值点的方法。 
...
该方法是求 11 个不同召回率（0, 0.1, ..., 1）对应的精度的平均值。各个点的精度插值对应的是召回率大于该处召回率的范围了最大的精度。具体如下图： 
...
两种方法的结果有些许不同：24.56%（完全插值） 和 26.84%（11 点插值）。  
