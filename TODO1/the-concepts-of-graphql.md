...
我认为 GraphQL 真正的核心在于应用数据图。在本文中，我将介绍应用数据图，讨论 GraphQL 如何在应用数据图上进行查询操作，以及如何利用 GraphQL 查询的树结构来缓存其查询结果。

**更新 2/7/2017: 现在你可以在下面这个视频里看到这篇文章的内容了（译者注：视频源自 Youtube 需要翻墙）**
...
要查看这个结果如何构造出一颗树，只需移动节点使其看起来更像一颗树：
...
要构建一个真正快速、流畅的应用，使用户不会浪费大量时间在等待加载的旋转圈上，我们希望使用缓存来减少客户端与服务器之间的请求。**事实证明，GraphQL 的树结构非常适合客户端缓存。**
...
稍后，页面的其他部分将再次请求这个相同的查询。除非我们完全需要最新的数据，否则第二个查询可以使用我们已有的数据来解决！这意味着缓存需要能够在不将查询发送到服务器的情况下解决查询，从而使我们的应用运行得更快。但是，仅仅是缓存我们之前获取的准确查询还不够，我们还可以做得更好。
...
如果在某些不成立的情况下（例如，当特定查询路径指向的信息真正频繁更改时），我们可以用**对象标识符**的概念来防止 Apollo Client 做出这样的假设，稍后我们将介绍这个概念。但是，一般来说，当涉及到缓存时，这是一个合理的假设。
...
这意味着对象标识符在整个应用中必须是唯一的。因此，您不能直接使用SQL ID，因为这样一来，作者的 SQL ID 可能是 5，图书的 SQL ID 也可能是 5。但这很容易解决：要生成唯一的对象标识符，只需将 GraphQL 返回的 **__typename** 附加到后端生成的 ID 即可。因此，一个 SQL ID为 **5** 的作者可以有一个 **Author:5** 或类似的对象标识符。
...
有时，对于您的应用来说，把所有的内容都使用对象标识符是不合理的，或者您可能不想直接在代码中处理它们，但仍需要更新缓存中的特定信息。我们提供方便而强大的 API 可以解决这个问题，例如 **updateQueries** 或 **fetchMore**，可以使您通过非常精细的控制将新信息合并到这些查询树中。
...
2. Apollo Client缓存了**查询结果树**。为此，它应用了两个假设：
