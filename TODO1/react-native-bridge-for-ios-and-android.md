...
> * 译者：[MeandNi](https://github.com/MeandNi)
...
_Bulb_ 模块需要主队列设置，因为它覆盖 `init`但 没有实现 `requiresMainQueueSetup`。在以后的版本中，React Native 将默认初始化后台线程上的所有原生模块，除非明确选择不需要。
...
打开 Android Studio 并单击 **打开现有的 _Android Studio_** 项目，然后在 LightApp 中选择 _android_ 文件夹。下载所有 gradle 依赖项后，创建一个 Java 类 Bulb.java，如下所示：
...
下一步是注册模块，如果模块未注册，则无法从 JavaScript 获得。通过单击菜单"文件” ->“新建” -> “Java 类”并将文件名设置为 _BulbPackage_ 来创建文件，然后单击“确定”。然后将以下代码添加到 _BulbPackage.java_
...
需要在 _MainApplication.java_ 文件的 _getPackages_ 方法中提供 _BulbPackage_ 包。此文件存在于 react-native 应用程序目录中的 android 文件夹下。在 _android/app/src/main/java/com/LightApp/MainApplication.java_ 中更新以下代码
