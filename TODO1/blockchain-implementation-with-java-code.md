...
区块链的应用领域尚未探索完毕。它也有可能会破坏企业自动化。关于区块链的工作原理，有很多可用的信息。我们有一个深入区块链的[免费白皮书](https://keyholesoftware.com/wp-content/uploads/Blockchain-For-The-Enterprise-Keyhole-White-Paper.pdf)（无需注册）。
...
首先我们简要总结下区块链。块包含一些头信息和任何类型数据的一组或一组交易。该链从第一个（初始）区块开始。随着交易被添加/扩展，将基于区块中可以存储多少交易来创建新区块。

当超过块阀值大小时，将创建一个新的交易区块。新区块与前一个区块连接，因此称为区块链。
...
注意，chain 属性维护了一个类型为 `Tx` 的区块列表。此外，`无参构造器` 构造器会在创建初始链表时初始化“初始”区块。下面是 `newBlock()` 方法源码。
...
你可以看到，试图以任何方式伪造交易数据或任何其他属性是非常困难的。而且，随着链的增长，它会继续变得非常、非常、非常困难，基本上是不可能的 —— 除非量子计算机可用！
...
另外，如果你是 C# 程序员，这个（我不会告诉任何人），我们也有用 C# 写的示例。下面是 C# 区块链实现的[示例](https://github.com/in-the-keyhole/khs-blockchain-csharp-example)。
