...
这篇文章中，我会介绍一些使你成为更好的 React 开发者的方法。我会从工具到代码风格等一系列地介绍，这些可以帮助你提升 React 相关技能。 💪

* * *

### 代码检查

写出更好代码很重要的一件事就是使用好的代码检查工具。如果代码编辑器配置好了一套代码检查工具，就能帮我们捕捉任何可能导致代码出问题的东西。

但除了捕捉问题，[ES Lint](https://eslint.org/) 也会不断让你认识到 React 代码的最佳实现。
...
```
props (react/prop-types) 验证没有 'hello'
```
...
- [**prop-types**：用于 React props 和类似对象的运行时进行类型检查的工具](https://www.npmjs.com/package/prop-types)
...
#### Stateless Functional Component（无状态功能组件）
...
无状态功能组件是一种很常见的组件类型。他为我们提供了一种非常简洁的方式来创建不使用任何 [**state**](https://reactjs.org/docs/faq-state.html)、[**refs**](https://hackernoon.com/refs-in-react-all-you-need-to-know-fb9c9e2aeb81) 或 [**生命周期方法**](https://reactjs.org/docs/state-and-lifecycle.html) 的组件。

无状态功能组件的特点是没有状态并且只有一个功能。所以你可以把组件定义为一个返回一些数据的常量函数。

简单来说，无状态功能组件就是返回 JSX 的函数。
