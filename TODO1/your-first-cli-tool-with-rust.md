...
在精彩的编程世界里，你可能听说过这种名为 Rust 的新语言。它是一种开源的系统级编程语言。它专注于速度、内存安全和并行性。你可以像 C/C++ 那样用它编写底层应用程序。
...
好吧，让我把事情说清楚。我可以用任何其他语言或框架来完成命令行工具。我可以选 C，Go，Ruby 等等。甚至，我可以使用经典的 bash。
...
我的个人建议是，如果你用的是 Windows 10，我建议你使用 [WSL](https://docs.microsoft.com/en-us/windows/wsl/install-win10) 来完成安装。以上就是安装所需的步骤，我们现在可以去创建我们的第一个 Rust 应用程序了！
...
如果我们 `cd` 到该目录中，我们将看到一个目录结构。而且，方便的是，这个项目已经默认初始化了 git。真是太好了！
...
在开发模式下，你可以通过调用 `cargo run`（用 `cargo run --- my_arg` 来传递命令行参数）来执行二进制文件。
...
正如我之前在文章中所说的，我们正在尝试构建一个精简版的 `cat` 命令。我们的目标是模拟 `cat` 的行为并在终端输出文件内容，通过启动 `kt myfile.txt` 即可。
...
在这个文件中，你将看到有一些信息已经填充好了，比如作者、版本等等。我们要做的只是在 `[dependencies]` 下添加依赖项。
...
保存文件后，我们需要重新构建项目，以便能够使用依赖库。不要担心 `cargo` 会下载除了 `clap` 以外的东西，这是由于 `clap` 也有其所需的依赖关系。
...
打开 `main.rs` 文件。我们必须明确地声明我们要使用 Clap 库。
...
这里我们声明的是我们想要使用 `Arg` 和 `App` 模块。我们希望我们的应用程序有一个 `FILE` 参数，它将包含一个文件路径。Clap 可以帮助我们快速实现该功能，用一种方法链接的方式，这是一种令人非常愉悦的方式。
...
再次编译并执行，除了变量 `matches` 上的编译警告(对于 Ruby 开发者来说，可以在变量前面加上 `_`，它会告诉编译器该变量是可选的)，它应该不会在输出中给出太多其他信息。
...
同样，我们将使用一个标准库来读取名为 `File` 的文件。我们将使用 `open` 方法读取文件的内容，然后将其写入一个字符串对象，该对象将在 `stdout` 中显示。
...
构建并再次运行此代码。恭喜你！我们现在有一个功能完整的工具了！🍾
...
我们完成了！我们通过约 45 行代码就完成了我们的简化版 `cat` 命令 🤡，而且它表现得很好！
