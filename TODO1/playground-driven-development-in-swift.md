...
作为移动开发者，我们的目标是为终端用户提供最佳的用户体验，通过专门的应用程序能让生活更具吸引力，更轻松。任务之一是确保用户所看到的 UI 界面看起来不错并且正确。 大多数情况下，可以说应用是数据的优化器。我们从后端获取 JSON 数据，将其解析为模型，然后使用 `UIView`，主要是 `UITableView` 或 `UICollectionView` 呈现出来。

对于 iOS，我们需要根据设计来不断调整用户界面，使其能够适合小尺寸的手持设备。这个过程涉及到更改代码、编辑、等待、检查、然后又更改代码等等……像 [Flawless App](https://flawlessapp.io/) 这样的工具可以帮助你轻松地比对 iOS 应用和 Sketch 设计的结果。但真正痛苦的是[编译](https://medium.com/@johnsundell/improving-swift-compile-times-ee1d52fb9bd)部分，这个过程需要花大量的时间，而对于 [Swift](https://github.com/fastred/Optimizing-Swift-Build-Times) 来说，情况就更加糟糕了。它会降低我们快速迭代的效率。看起来像是编译器在假装编译时暗中在挖掘比特币。😅
...
起初我并不十分相信，并且我看到很多关于Playground 反应换面或无反应的抱怨。但当我看到 [Kickstarter iOS 应用](https://github.com/kickstarter/ios-oss)使用 Playground 来加速其样式和开发流程后，它给我留下了深刻的印象。所以我开始在一些应用中也成功使用了它。它不像 [React Native](https://facebook.github.io/react-native/) 或 [Injection App](http://johnholdsworth.com/injection.html) 那样能够立即重新渲染，但希望它以后会越来越好。 😇

或者至少它取决于开发社区。Playground 的使用场景是我们一次只设计一个屏幕或组件。这迫使我们仔细考虑依赖关系，所以我能导入一个特定的屏幕，然后在 Playground 中进行迭代。

### Playground 中的自定义框架

Xcode 9 允许开发者[在 Playground 中导入自定义框架](https://help.apple.com/xcode/mac/9.0/#/devc9b33111c)，只要框架和 Playground 在同一工作区内。我们可以使用 [Carthage](https://github.com/Carthage/Carthage) 来获取并构建自定义框架。但如果你使用的是 CocoPods，那么也是没有问题的。
...
对于该演示，因为我们想要一些有趣的东西，我们将使用 CocoaPods 来获取一个名为 [Cheers](https://github.com/hyperoslo/Cheers) 的依赖项。如果你想祝贺用户达成一些成就时，`Cheers` 可以显示花哨的五彩纸屑效果。
...
运行 `pod install` 后，CocoaPods 会生成一个包含 2 个项目的 workspace 文件。一个是我们的应用程序项目，另一个是有全部 pod 的项目。 现在的话只有 `Cheers`。关闭你现在的工程，改为打开生成的 workspace 文件。
...
iOS 中 的 `ViewController` 主要位于 UI 层，因此它应该只获取解析过的数据并使用 UI 组件呈现出来。如果当中有一些逻辑，则可能会涉及如缓存、网络……等其他部分。这需要你添加更多的文件到 AppFramework。小而独立的框架显得更合理，因为可以让我们快速迭代。
...
如果你需要查看如何使用桥街头文件，可以阅读[同一项目中的 Swift 和 Objective-C](https://developer.apple.com/library/content/documentation/Swift/Conceptual/BuildingCocoaApps/MixandMatch.html)。
...
为此，你需要将 `Bridging-Header.h` 添加到框架中，并且声明为 `public`。在 SO 搜索一下，你会发现[这些](https://stackoverflow.com/questions/7439192/xcode-copy-headers-public-vs-private-vs-project)：
