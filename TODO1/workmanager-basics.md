...
假设你有一个图片编辑应用，可让你对图像进行过滤并将其上传到网络让全世界看到。你希望创建一系列后台任务，这些任务用于过滤器，压缩图像和之后的上传。在每个环节，都有一个需要检查的约束——过滤图像时要有足够的电量，压缩图像时要有足够的存储空间，以及上传图像时要有网络连接。
...
*   需要是 **确保** 能够运行，无论应用程序是否退出，因为如果过滤后的图像永远没能与世界共享，你的用户会非常不满意！

这些特点使我们的图像过滤和上传任务成为 WorkManager 的完美用例。

### 添加 WorkManager 依赖

本文使用 Kotlin 书写代码，使用KTX库（KoTlin eXtensions）。The KTX version of the library provides [extension functions](https://developer.android.com/reference/kotlin/androidx/work/package-summary#extension-functions-summary) for more concise and idiomatic Kotlin. 你可以添加如下依赖使用 KTX 版本的WorkManager：
...
在我们将多个任务连接在一起之前，让我们关注如何执行一项工作。我将会细分上传任务。首先，你需要创建自己的 [`Worker`](https://developer.android.com/reference/androidx/work/Worker)  实现类。我将会把我们的类命名为 `UploadWorker`，然后重写 [`doWork()`](https://developer.android.com/reference/androidx/work/Worker.html#doWork%28%29) 方法。
...
当 `Worker` 定义工作的**作用**时，[`WorkRequest`](https://developer.android.com/reference/androidx/work/WorkRequest) 定义**应该如何以及何时运行工作**。
...
假设 `UploadWork` 不应该立即运行——它应该只在设备有网络连接时运行。你可以通过添加 [`Constraints`](https://developer.android.com/reference/androidx/work/Constraints.Builder) 对象来完成此操作。你可以创建这样的约束：
...
你首先需要获取 [`WorkManager`](https://developer.android.com/reference/androidx/work/WorkManager) 的实例，这是一个负责执行你的工作的单例。调用 [`enqueue`](https://developer.android.com/reference/androidx/work/WorkManager#enqueue%28androidx.work.WorkRequest%29) 是启动 `WorkManager` 跟踪和调度工作的整个过程的原因。

### 在幕后——工作是怎么运行的

那么，`WorkManager` 能为您做些什么呢?默认情况下，`WorkManager` 会这些：

*    **从主线程**运行你的工作（假设你正在继承 `Worker` 类，如上面的 `UploadWorker` 所示）。
...
让我们探讨一下 WorkManager 如何确保你的工作从主线程运行并保证执行。在幕后，WorkManager 包括以下部分：
...
2.  稍后，当满足 `WorkRequest` 的 `Constraints` 时（可以立即），Internal TaskExecutor 会告诉 `WorkerFactory` 创建一个 `Worker`。
3.  之后，默认的 `Executor` 调用你的 `Worker` 的 `doWork()` 方法 **脱离主线程**。

通过这种方式，默认情况下，你的工作都可以保证执行并从主线程运行。

现在，如果你想使用除默认 `Executor` 之外的一些其他机制来运行你的工作，你可以做到！对协同程序（`CoroutineWorker`）和 RxJava（[`RxWorker`](https://developer.android.com/reference/androidx/work/RxWorker)）的开箱即用支持作为工作的手段。
...
WorkManager 无法将所有工作信息保存到数据库中，这使得它成了非常适合需要保障执行的任务。这也是使得 WorkManager 对于不需要保障且只需要在后台线程上执行的任务的过度杀伤的原因。例如，假设你已经下载了图像，并且希望根据该图像更改 UI 部分的颜色。这是应该在主线程运行的工作，但是，因为它与UI直接相关，所以如果关闭应用程序则不需要继续。所以在这样的情况下，不要使用WorkManager——坚持使用像 [Kotlin 协同程序](https://codelabs.developers.google.com/codelabs/kotlin-coroutines/#0)那样轻量的东西或创建自己的 `Executor`。

###  使用链进行依赖性工作

 我们的过滤器示例包含的不仅仅是一个任务——我们想要过滤多个图像，然后压缩并上传。如果要一个接一个地或并行地运行一系列 `WorkRequests`，则可以使用链。示例图显示了一个链，其中有三个并行运行的过滤器任务，后面是压缩任务和上传任务，按顺序运行：
...
三个图片过滤器 `WorkRequests` 并行执行。一旦完成所有过滤器 WorkRequests （并且只有完成所有三个），就会发生 `compressWorkRequest`，然后是 `uploadWorkRequest`。

链的另一个优点是：一个 `WorkRequest` 的输出作为下一个 `WorkRequest` 的输入。因此，假设你正确设置了输入和输出数据，就像我上面的 `UploadWorker` 示例所做的那样，这些值将自动传递。

为了处理并行的三个过滤器工作请求的输出，可以使用 [`InputMerger`](https://developer.android.com/reference/androidx/work/InputMerger)，特别是 [`ArrayCreatingInputMerger`](https://developer.android.com/reference/androidx/work/ArrayCreatingInputMerger)。代码如下：
...
请注意，`InputMerger` 是添加到 `compressWorkRequest` 中的，而不是并行的三个过滤请求。

假设每个过滤器工作请求的输出是映射到图像 URI 的键“KEY_IMAGE_URI”。添加 `ArrayCreatingInputMerger` 的工作是并行请求的输出，当这些输出具有匹配的**键**时，它会创建一个包含所有输出值的数组，映射到单个键。可视化图表如下：

![](https://cdn-images-1.medium.com/max/800/1*Ft-4gVTZVjVtz6xIaEtNOg.png)

`ArrayCreatingInputMerger` 功能可视化

 因此，`compressWorkRequest` 的输入将最终成为映射到过滤图像 URI 数组的 “KEY_IMAGE_URI” 对。
...
*    `WorkInfo` 更改时进行观察并通知的能力是 `LiveData` 提供的功能。
...
*   调度 `WorkRequest`。
*   默认情况下，了解 `WorkManager` 在线程和保障运行方面的工作。
