...
**注意**：**值得注意的是，尽管这些对齐属性始于 Flexbox 规范，但 Box Alignment 会最终覆盖 Flexbox 规范的相关内容，如 [flexbox 规范](https://www.w3.org/TR/css-flexbox-1/#alignment)中所述。**
...
当你在 flex 布局中看到有一个属性是以 `align-` 开头的，那个就是交叉轴的对齐方式，`align-items` 属性规定子项目在沿着交叉轴方向上的对齐方式，它的其他的值包括：
...
这些子项目(flex item)同样可以收缩。 `flex-shrink` 的值默认被设置成了1。这就意味着，如果我们的 flex 容器非常小，那么其中的子元素在溢出容器之前就会自动的缩小以适应容器大小。这是一个非常灵活的属性。总的说就是，子项目在容器没有足够空间去排列的情况下依然能保持在容器之内，并且不会溢出。

为了在默认情况下获得最好的展示效果，`flex-basis` 属性的默认值是 `auto` ，我们会在这个系列的其他文章中好好了解这个值代表什么。现在，你只需要将 `auto` 理解为『大到足够适应容器』就行了。当你的 flex 容器中有一些子项目，其中的一个子项目相较于其他包含更多的内容，那么它会被分配更多的空间。
...
上面这个例子就是弹性布局灵活性的体现。`flex-basis` 属性的默认值是 `auto`，并且子项目（flex item）没有设置尺寸，那么它就会有一个 `max-content` 的基础尺寸。这就是它们延伸之后没有任何填充的尺寸。然后，子项目按照比例来占据空间，详见以下 [flexbox 规范](https://www.w3.org/TR/css-flexbox-1/#flex-flex-shrink-factor) 中的说明。
...
弹性布局正在试图帮助我们获得一个合理的最终显示结果，而不需要写 CSS 的人来定义。对于一个每行有几个单词的子项目，它会为这个项目分配更多的空间来展示，而不是平均的减少空间，从而变成一个非常高的元素。这种表现正是如何正确使用弹性布局的关键。弹性布局最适用于用于沿着一条轴线排列的元素，以一种灵活和感知内容的方式。这里我简单介绍了一点细节，但在接下来的系列文章中我们会更加深入的了解这些算法。

### 总结

在这篇文章中，我用弹性布局的初始值来介绍当你设置 `display: flex` 的时候，究竟发生了什么。令人惊讶的是，当你逐步分解之后，发现它原来有这么多内容，并且这些内容就包含了弹性布局的核心特点。
