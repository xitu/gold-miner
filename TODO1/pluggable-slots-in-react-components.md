...
咱们得先共同了解这样一个事实：React 能够让你通过在 JSX 标签之间嵌入 **children** 的方式来向组件传递 **children**。这些元素 （零个、一个或多个）在那个组件里将以名为 `children` 的 prop 的形式存在。 React 的名为 `children` 的 prop 其实相当于 Angular 的 transclusion 还有 Vue 的 `<slot>`。
