...
如果页面信息被分组形成逻辑单元，用户会更容易看懂。请把把外边距（Padding）设置成 120 像素到 180 像素， 并且通过背景色把不同文本单元分开。
...
不同组的相关信息间没有设置边距，而且这个设计需要用色块把页面划分出逻辑单元。所以，目前这些信息让人很难看懂，而且文字的单元分组也很不清晰。  

边距足够大，而且不同单元通过背景色分开，这让什么单元包含什么信息变得一目了然。

**2. 页面元素的空白间距不相等**  

请对页面的逻辑单元设置相等的空白间距。否则，你的页面会看起来很凌乱，用户不会把注意力均分到每一个单元上。
...
不同位置的空白间距不均匀，并让人产生了公司信息与标题相连的印象，尽管每个单元的结构是一样的。  

标题和内容上下的空白边距相等，让人意识到每个逻辑单元的结构是一样的。

**3. 边距太小，意味着用户无法把内容分解成不同逻辑单元**  
...
使用了窄边距，构成站点的单元彼此粘在了一起。这让页面很拥挤，而且十分费解 —— 网站访问者会以为这是一段纯文字，而不是含义不同的单元。 

边距足够大，因此这两个单元之间的差异立即可见。

**4. 避免底图上文字与下面的低对比度**

文字和背景之间应有足够的对比度。为了让文本更显眼，在底图上放一层（增加）对比度的滤波器。黑色比较流行，但你也可以使用鲜艳的颜色混合搭配。
...
请不要突出带色块的狭窄页面元素。它就是很难看。例如，标题已经通过它们的文字大小、样式和间距变得很显眼了。你想突出页面上的某一点吗？请在整个单元上使用背景色，包括相关的标题和文本。
...
标题和相关文本使用了相同的背景。这表明它们属于同一逻辑单元。

**7. 在窄栏中包含了太多文字**  

当窄栏中含有大量文字时，阅读很费劲，因为页面访问者不得不一行一行地跳转视线。另外，它就是很难看！最好减少列数，缩短文本长度，否则没人会看的。
...
如果需要包含大量文本，请使用具有可折叠文本功能的单元（在 Tilda 中，它们是 TX12、TX16N 或者是按钮 BF703)。  
...
这个标题挡住了女人的面容。（文字下面）有那么多细枝末节，使文本难以阅读。  

图片和文字都容易阅读，构图很好。

**10. 误用视觉层次结构**  

对于在页面上清晰可见的信息层次，封面上的标题字体应该大于其余标题，或者至少是相同大小。举个例子，如果标题很长，尤其要这么处理。  
...
封面上的标题小于下面的标题，不合比例，这很让人费解。为什么？它让第二个标题更显眼了。  

封面上的标题比下面单元中的标题大，因此整个页面看起来协调一致。

同样的原理适用于逻辑单元内的视觉层次结构。标题应该是页面上最大的设计元素，其次是较小的、不太突出的子标题。接下来，内容标题应明显小于主标题，但使用相同的粗细程度。最小的字体应该用于内容描述。  
...
一个跟在文本之后，占据全屏的图像或图片画廊，显得跟独立的单元很像。如果你在图片画廊周围添加边距，由于背景一致，文字和图像看起来是一个逻辑单元整体。 
...
全屏的图片画廊看起来与上面的标题脱节，像一个独立的单元。

图片画廊和它上面的标题背景一致，这使整个构图看着紧密。

**12. 标题太大太长**  

对短句子来说，超大字体是完美适配的。如果标题较长，请使用小号字体。这将将利于阅读，并给其它的设计元素在页面上留出足够的空间。
...
这个页面排版不错，所有的设计元素之间都是协调的，文字也利于阅读。
...
一种颜色作为基调，在此之上衍生出颜色多样性，这样就不干扰页面内容了。
...
在排版中，字体大小对比是用来划分不同视觉层次的文本，并建立严谨结构的。主标题应该是网页中最突出的，子标题应该小不少，但也要清晰可见。
...
如果不同单元在页面中同等重要，那它们应有相同的界面外观，并且位置间距离应该相等。
...
设计样式（如黑体）在使用很少的时候表现良好。使用太多，就会妨碍页面阅读。
...
标题是一个单独的设计元素。它不应该离下面的图片太近。对于一个成功的标题图片组合单元，请设置元素的间距不小于 60 像素，并添加子标题 —— 它将展开页面内容，把正确的重点放在你需要的地方。
...
在这里，标题与用图片用子标题分开，它引领了整个单元，而不仅是图片
