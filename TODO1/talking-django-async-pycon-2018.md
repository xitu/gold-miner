...
那不仅仅是一个 request 和 response

00:01:57,869 --> 00:02:00,390

像 HTTP 那样，而是一个持续的 socket 嵌套字
...
总体来说非常好and it was generally pretty good it
...
大的 worker servers 的池子，

00:04:36,900 --> 00:04:39,150

一旦你有一个 WebSocket 的事件，
...
一个 Django worker，运行一些代码，

00:04:43,710 --> 00:04:45,390

等待完成然后返回 twisted 服务器
...
最重要的事，它没有任何异步 IO 的 支持

00:05:18,150 --> 00:05:21,210

这段时间， Python 3 非常司空见惯了
...
拥抱 Python 3吧，不要在 Python 2 上工作了，
...
需要我花些时间去做的事情me a while to do this and I had to sort
...
和如何通过它的表面价值those changes and how taking up a sink
...
你可以看看我的一个博客文章 you can look at that covers the
...
然后你来到了视图then you come to a view that's business

00:11:33,930 --> 00:11:35,670

这里业务逻辑和表现logic in presentation and then finally
...
通过堆栈复制了很多that replicates a lot of that behavior
...
还是仍然需要调用同步 Django，left there is still need to call into
...
是这两个方法，是and be to functions that were the result
...
在线程中运行，所以我们会run in threads so we are going to run

00:14:33,149 --> 00:14:34,680

运行超过一个同步方法more than one synchronous function we
...
然后给我返回结果me the result back the simplified code
...
基本流程是类似的basic flow is similar you find your
...
像过去那样使用同步代码，那你in synchronous code before you just
