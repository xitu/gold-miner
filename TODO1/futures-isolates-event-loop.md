...
我认为通过一篇文章来解释**异步**、**并行**处理这些主题并消除其中任何含糊不清的内容是非常有用的。
...
当你启动一个 **Flutter**（或任何 **Dart**）应用时，将创建并启动一个新的进程**线程**（在 **Dart** 中为 「**Isolate**」）。该**线程**将是你在整个应用中唯一需要关注的。
...
在线程的整个生命周期中，一个被称为**事件循环**的**单一**且隐藏的进程将决定你代码的执行方式及顺序（取决于 **MicroTask** 和 **Event** 队列）。

**事件循环**是一种**无限**循环（由一个内部时钟构成），在每个**时钟周期内**，**如果没有其他 Dart 代码执行**，则执行以下操作：
...
正如我们看到的，**MicroTask** 队列 优先于 **Event** 队列，那这 2 个对列的作用是什么呢？
...
事实是通过在 **methodC()** 中定义 **Future** 的地方简单地添加 **await** 来改变整个行为。
...
我想向你演示的最后一个例子如下。
运行 **method1** 和 **_method2** 的输出是什么？它们会是一样的吗？
...
##### 1.3. 第三步：销毁新的 Isolate 实例

当你不再需要新的 Isolate 实例时，最好通过以下方法释放它：
...
> 平台—信道通信**仅仅**由**主 isolate 支持**。该**主 isolate** 对应于应用启动时创建的 **isolate**。

也就是说，通过编程创建的 **isolate** 实例，无法实现**平台—信道**通信……
...
你的应用可以满足以上所有因素，但如果用户在一些处理过程中遇到了**滞后**，这极有可能对你不利。
...
同样重要的是要谨记 **Flutter**（**Dart**）是**单线程**的，因此，为了取悦用户，开发者必须确保应用运行尽可能流程。**Future** 和 **Isolate** 是非常强大地工具，它们可以帮助你实现这一目标。
