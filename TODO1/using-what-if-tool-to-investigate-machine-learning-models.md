...
为了说明 What-If 工具的功能，PAIR 团队已经使用预先训练过的模型发布了一组[示例](https://pair-code.github.io/what-if-tool/index.html#demos)。你可以在笔记本中运行演示程序或者直接通过网络运行它。
...
> 核心思想是，先训练一个模型，然后使用 what-if 工具对测试数据进行可视化处理，然后将经过训练的分类器的结果可视化。

### Tensorboard 结合 WIT 使用

你需要通过 [TensorFlow 模型服务器](https://www.tensorflow.org/serving)为你模型提供的服务才能在 TensorBoard 中使用 WIT，而要分析的数据必须作为 [TFRecords](https://medium.com/mostly-ai/tensorflow-records-what-they-are-and-how-to-use-them-c46bc4bbb564) 文件。在 TensorBoard 使用 WIT 的更多细节，可以参阅[文档](https://github.com/tensorflow/tensorboard/tree/master/tensorboard/plugins/interactive_inference#what-do-i-need-to-use-it-in-tensorboard)。

### 在笔记本上使用 WIT

为了能在笔记本中访问 WIT，你需要一个 WitConfigBuilder 对象，该对象指定要分析的数据和模型。这个[文档](https://github.com/tensorflow/tensorboard/tree/master/tensorboard/plugins/interactive_inference#notebook-mode-details)为在笔记本中使用 WIT，提供了一个慢慢学习的教程。

![](https://cdn-images-1.medium.com/max/2000/0*bUExfjdJSB0BqCpt.png)

***

为了确保工作的进行，你可以使用[笔记本示例](https://colab.research.google.com/github/pair-code/what-if-tool/blob/master/WIT_Model_Comparison.ipynb)，来编辑代码以此来容纳数据集。
...
我么首先对数据集进行研究。这里有一个用于后续演示的[链接](https://pair-code.github.io/what-if-tool/uci.html)。

What-if 工具有两个主面板。**右面板**包含你加载的数据集中各个数据点的可视化内容。

![](https://cdn-images-1.medium.com/max/2210/1*Fpeb_UkmNv53Wo55nQ9O0A.png)

在这种情况下，**蓝点**是模型推断收入**小于 50k** 的人，**红点**是模型推断出收入**超过 50k 的人**。默认情况下，WIT 使用值为 0.5  的[正分类阀值](https://developers.google.com/machine-learning/crash-course/classification/thresholding)。这意味着，如果推理分数是 0.5 或者更高，数据点就会被认为是一个积极的类别，即高收入。
...
- [**Google FACETS 的可视化机器学习数据集**。来自 Google 的开源工具，可以轻松地从大量数据中学习模式](https://towardsdatascience.com/visualising-machine-learning-datasets-with-googles-facets-462d923251b3)
...
通过简单改变年龄，模型现在将预测这个人属于高收入类别。对于此数据点来说，早期的正得分为 0.473，负得分为 0.529。然而随着年龄的改变，正得分达到了 0.503。

* **寻找最近的反事实**

理解模型的另一种方式是，看看什么样的小范围更改会导致模型翻转其决策，即所谓的**反事实**。只要点击一下，我们就可以看到与我们所选定的数据点最相似的反事实（用绿色高亮）。在数据点编辑器选项卡中，我们还看到了与原始数据点的特征值相反的特征值。绿色文本表示两个数据点不同的特征。WIT 使用 [L1 and L2](https://www.kaggle.com/residentmario/l1-norms-versus-l2-norms) 距离来计算数据点之间的相似。

![](https://cdn-images-1.medium.com/max/2008/1*H7S9oSQgPP7H56NSFXg_hg.png)

在这种情况下，最近的反事实是稍微老些，有一个不同的职业和资本收益，但在其他方面是相同的。

我们还可以使用**显示与所选数据点相似**的按钮来查看所选点与其他点之间的相似性。WIT 测量从选定的点到其他每一个数据点的距离。让我们改变我们的 X 轴散射，来显示到选定数据点的 L1 距离。

![](https://cdn-images-1.medium.com/max/2000/1*lSHybyMux8FdsWlO7HasgA.png)

* **分析部分依赖图**

部分依赖图（PDP 或 PD 图）显示了一个或两个特征对机器学习模型预测结果的边缘效应( [J. H. Friedman 2001](https://statweb.stanford.edu/~jhf/ftp/trebst.pdf))。
...
* 该模型学习了年龄和收入之间的正相关
* 更深度的内容让此模型对高收入更有信心
* 高收益是高收入的一个非常有力的指标，比任何其他单一特征都要强得多。
...
我们可以看到，在默认的阀值水平 0.5时，大约有 15% 的时间，我们的模型是不正确的，大约 5% 的时间是假阳性，大约有 10% 的时间是假阴性。更改阀值来查看其对模型精度的影响。

可以进行调整的还有设置“**成本比率**”和“**优化阀值**”的按钮。

* **机器学习公平性**

机器学习中的公平性与建模和预测结果一样重要。训练数据中的任何偏差都回反映在经过训练的模型中，如果采用这种模式，所产生的产出也会有偏差。WIT 可以通过集中不同的方式帮组调查公平问题。我们可以设置一个输入特征（或一组特征）来对数据进行切片。例如，让我们看看性别对模型性能的影响。
...
特征选项卡，提供了数据集中每个特征的汇总统计信息，包括直方图、分位数图、条形图等。该选项卡还允许查看数据集中每个功能的值分布。例如，允许我们探讨性别、资本收益和种族特征。
