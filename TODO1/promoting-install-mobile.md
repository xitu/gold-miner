...
为什么你希望用户在他们的主页安装你的应用？和你希望用户从任何应用商店安装你的应用原因是一样的。安装了应用的用户是你最需关注的群体。安装了 PWA 的用户比一般的访客拥有更好的参与度指标，包括更频繁的访问次数，在页面上更久的停留时间以及更高的用户转化率，通常与移动设备上的本机应用用户相同。
...
1. 将推广与你的用户操作流程相分离。举例来说，在 PWA 的登录页，将请求操作放在登陆表单和提交按钮下方。混乱的使用推广模式会降低你 PWA 的可用性，并且会对你的参与度指标造成负面影响。
2. 需包含关闭或减少推广的能力。如果他们这样做，请记住用户的偏好，并且只有在用户与你的应用内容的关系发生变化时才会重新提示，例如他们已登录或完成购买。
3. 可以在你 PWA 应用的不同部分里结合使用多种技术，但请注意不要在安装推广时过多的或打扰到你的用户。记住第一条规则 #1！
...
当你的 PWA 通过 Android 上的[可安装性检查清单](https://developers.google.com/web/fundamentals/app-install-banners/?hl=zh-cn)时，浏览器已经告诉用户使用迷你信息栏的 PWA 的可安装性。这个迷你信息栏只是一个辅助，未来它会将被移除。

**注意：**从 Chrome 76 版本开始，你可以通过在 `beforeinstallprompt` 事件上调用 `preventDefault` [阻止迷你信息栏出现](https://developers.google.com/web/updates/2019/05/mini-infobar-update?hl=zh-cn)。如果你不调用 `preventDefault`，该条目会在用户第一次访问你的站点时被显示出来，并且这是符合安卓的[可安装标准](https://developers.google.com/web/fundamentals/app-install-banners/?hl=zh-cn)的，然后在大约 90 天以后会再次显示。

## 应用 UI 推广模式

应用 UI 推广模式几乎适用于任何类型的 PWA 应用并表现在 UI 层面上，诸如网站导航，横幅等等。和使用任何其他类型的推广模式一样，很重要的一点是关照到用户的使用环境，尽最小程度的干扰用户操作流程。
...
如果使用得当，在你的站点标题处推行 PWA 安装是一种很好的方式，因为这可以让你的大多数忠实用户更轻松地回归到你的体验上来。PWA 标题里的像素非常有价值，所以请确保你的安装请求操作具有合适的大小，并且比其他任何可能的标题内容更重要，还不那么显眼。

你必须确保：

* 评估用户已安装用例的价值。考虑选择性定位的向那些可能从中受益的用户展示这些推广信息。
* 有效使用宝贵的标题空间。考虑在标题中可以为你的用户提供哪些其他帮助，并权衡安装推广相对于其他选项的优先级。
...
导航菜单是推广安装应用的好地方，因为打开菜单的用户会发出与你的体验相关的信息。

请你确保：

* 避免破坏重要的导航内容。把 PWA 安装推广放在其他菜单项下面。
* 提供一个简短的相关说明，告诉用户安装 PWA 带来好处的原因。

### <a name="landing"></a>着陆页面

着陆页的目的是宣传你的产品和服务，所以这里是一个适合大规模推广安装你们 PWA 应用好处的地方。

![](https://developers.google.com/web/fundamentals/app-install-banners/images/install-promo/landing.png?hl=zh-cn)

首先，介绍你们网站的价值观，然后让访问者了解他们能从安装中得到什么。

请你确保：

* 着重突出对你的访客最重要的功能，并强调可能将他们带到你着陆页的关键字。（译者注：也许是这些关键字让用户访问的着陆页）
* 这是一个着陆页。在明确了你的价值主张之后，立即进行安装推广和号召性用语！
* 考虑在用户花费大量时间的应用上添加安装推广。

### <a name="banner"></a>安装横幅

页面顶部可关闭的横幅。

![](https://developers.google.com/web/fundamentals/app-install-banners/images/install-promo/banner.png?hl=zh-cn)

大多数用户在手机体验中遇到过安装横幅，并且熟悉横幅提供的交互功能。应谨慎使用横幅，因为它们会对用户体验造成极大的破坏。

请你确保：

* 在显示横幅之前，请等待用户对你的站点表现出足够的兴趣。如果你的用户关闭了横幅，请不要再显示，除非用户触发了转化事件，表明和你的内容有更高的参与度，例如在电子商务网站上的消费行为或者注册成为一个账号。
* 提供有关在横幅中安装 PWA 的价值的简要说明。例如，你可以通过提及它的使用几乎不占用用户设备存储空间或者不会跳转到应用商店而是立即安装的方式，告诉用户安装一个 PWA 和本机应用的区别。

## 内在推广模式

内在推广技术将推广和站点内容交织在一起。这通常比应用 UI 上的推广更微妙一些，UI 具有权衡性。你希望你的推广能够足够突出，感兴趣的用户会注意到它，但比起降低了的用户体验，这并不太值得。
