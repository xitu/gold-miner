...
在稀有事件问题中，我们面对一个不平衡的数据集。这代表着，相较于负样本，我们只有很少的正样本标签。典型的稀有事件问题中，正样本在全部数据中占比大概在 5-10% 之间。在极端稀有事件问题中，我们只有少于 1% 的正样本数据。比如，在我们使用的数据集中，正样本只有 0.6%。

这种极端的稀有事件在现实世界中是十分普遍的。举个例子，工厂中的纸张断裂和机器故障，在线销售行业中的点击或者购买。

分类这些稀有事件是十分具有挑战的。最近，深度学习被广泛的应用于分类问题。然而，**少量的正样本限制了深度学习的应用**。不管数据量有多大，正样本数量都会限制深度学习的效果。
### 为什么还要绞尽脑汁使用深度学习？

这是一个合理的问题。我们为什么不去考虑使用其他的机器学习方法呢？

答案是主观的。我们可以使用机器学习方法。为了使它工作，我们可以降采样负样本数据使得数据接近平衡。由于正样本数据只有 0.6%，降采样后的数据集大概只有原始数据集大小的 1%。一些机器学习方法，如：SVM、随机森林等，都可以在这个数据量上工作。然而，它的准确率会受到限制。这是因为我们不会使用剩下的 99% 的数据。

如果数据充足，深度学习会表现的更好。它还可以通过使用不同的结构来灵活的改进模型。因此，我们准备尝试使用深度学习方法。

***

在本推文中，**我们将要学习如何使用稠密层自编码器来搭建一个稀有事件分类器**。推文的目是为了展示一个极端稀有事件分类器的自编码器实现。我们将探索不同自编码器结构和配置的工作留给读者。如果有什么有趣的发现，请分享给我们。
...
* 自编码器由编码器和译码器组成。
* 译码器用来学习进程的潜在特征。这些特征通常是由少量的维度表出。
* 译码器可以从潜在的特征中重构出原始的数据。

![Figure 1. 自动编码器的插图。 [[Source](http://i-systems.github.io/HSE545/machine%20learning%20all/Workshop/CAE/06_CAE_Autoencoder.html): Autoencoder by Prof. Seungchul Lee
iSystems Design Lab]](https://cdn-images-1.medium.com/max/2000/1*S_OcBPkRTpKJo0iz5IaNbQ.png)

### 怎么使用自编码器来分类稀有事件？

* 我们首先将数据分为两个部分：正样本标签和负样本标签。
* 负样本标签被约定为进程的**正常**状态。**正常**状态是无事件的进程。
* 我们将忽视正样本数据，同时在负样本上训练这个自编码器。
* 现在，这个自编码器学习了所有**正常**进程的特征。
* 一个充分训练的自编码器可以预测任何来自**正常**状态的进程（因为他们有同样的模式和分布）。
* 因此，重构的误差会比较小。
* 然而，如果我们重构一个来自稀有事件的数据，那么自编码器会遇到困难。
* T这会导致重构稀有事件时，会有一个很高的重构误差。
* 我们可以抓住这些高重构误差同时标记它们为稀有事件
* 这个过程类似于异常检测。
...
这个稀有事件问题的目的是在发生断裂前预测它。我们尝试提前 4 分钟预测出断裂。为了建立这个模型，我们把数据标签提前 2 行（对应于 4 分钟）。通过这行代码实现 `df.y=df.y.shift(-2)`。然而，在这个问题中，我们想做的是：如果行**n**被标记为正样本，
...
现在，我们将数据分为训练集、验证集和测试集。然后我们将使用只有 0 的数据子集来训练自动编码器。
...
对于自动编码器，通常最好使用标准化数据(转换为高斯、均值 0 和方差 1)。
...
首先，我们将初始化自动编码器框架。我们只构建一个简单的自动编码器。更多复杂的结构和配置留给读者去探索。
...
这是一个(多元)时间序列数据。我们没有考虑数据中的时间信息/模式。在[下一篇推文](https://medium.com/@cran2367/lstm-autoencoder-for-extreme-rare-event-classification-in-keras-ce209a224cfb)，我们将要探索是否可以结合 RNN。我们将尝试 [LSTM autoencoder](https://medium.com/@cran2367/lstm-autoencoder-for-extreme-rare-event-classification-in-keras-ce209a224cfb)。
