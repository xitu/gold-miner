...
当给类、变量和方法命名时，我们很容易冲动地给我们的方法命名。特别是当你觉得这一切都很合理时。试着几个月之后再回头看看那些代码，看看它们是否依旧合理。如果是，那么很有可能是你当时明智地命名了你的变量和方法。
...
确保方法和变量名称都是清晰明了的，且准确地描述了正在发生的事情。想象一下，如果你指引给一个游客错误的方向，他/她会多么生气和困惑。你正在为下一个前来阅读您的代码的程序员指引道路。

## 2. 最小化全局变量

不管使用哪种语言，你可能在编程中经常听到这种说法。人们只会说使用全局变量不好，而不去解释为什么不好。那么让我来告诉你为什么应该尽可能地最小化和避免全局变量。

> 全局变量会造成困惑，因为程序中的任何地方都可以访问到它们。

如果全局变量同时也是可变的，则会扩大人们的困惑。如果你声明了一个变量，那么很可能你只是想在自己的代码中去使用它。你猜猜接下来会发生什么？
