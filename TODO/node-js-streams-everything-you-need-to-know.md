...
让我们来看看真实的情况吧。我们启动服务器，发起连接，并记录下内存使用情况。
...
在这个变换流中，我们只实现了 `transform()` 方法。在该方法中，我们把 `chunk` 转换为大写然后通过 `push` 方法传递给下游。

#### 流对象模式 ####

默认情况下，流期望 Buffer/String 值。我们可以通过设置 `objectMode` 参数使得流可以接受任何 JavaScript 对象。
