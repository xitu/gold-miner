...
我们最近发布了 react-beautiful-dnd 的第四个版本 [`version 4`](https://github.com/atlassian/react-beautiful-dnd/releases/tag/v4.0.0) 其中包含了**大量的性能改进**。

![](https://cdn-images-1.medium.com/max/800/1*cn48EAW1k9TcDpfTtkySog.png)

基于具有500个可拖动项目的配置。使用开发版本和启用仪器进行录制 - 两者都会降低速度。 但是，录制也是在一个相当强大的开发机器上完成的。 确切的改进将取决于数据集的大小，设备性能等。

您仔细地阅读，**我们看到有 99% 的性能提升** 🤘。由于这个库已经 [extremely optimised](https://github.com/atlassian/react-beautiful-dnd#performance)，所以这些改进更加令人印象深刻。你可以自己试一下性能上的提升在我们的 [large list example](https://react-beautiful-dnd.netlify.com/iframe.html?selectedKind=single%20vertical%20list&selectedStory=large%20data%20set) 或者 [large board example](https://react-beautiful-dnd.netlify.com/iframe.html?selectedKind=board&selectedStory=large%20data%20set) 😎。
...
为了执行拖动，我们拍摄了所有相关组件的尺寸（坐标，大小，边距等）的快照，并将它们放入我们的 **state** 和拖动的开始处。然后，我们会在拖动过程中使用这些信息来计算需要移动的内容。 我们来看看我们如何完成这个初始快照：
...
为了解决这些问题，我们创建了一个新角色来管理尺寸收集流程：`dimension marshal`。 以下是新尺寸现在发布的工作方式：
...
2.  `dimension marshal` 接收 `request` 并直接向所需维度发布者请求关键维度（拖动项目及其容器）以便开始拖动。 这些发布到 store 就可以开始拖动。
...
因为 `dimension marshal` 知道系统中的所有 `ID` 和  `index`，所以它可以直接请求任何维度 `O（1）`。这也使其能够决定如何以及何时收集和发布维度。 以前我们有一条消息 `shouldPublish` ，应该一次性回应所有。`dimension marshal` 在调整这部分生命周期的性能方面给了我们很大的灵活性。如果需要，我们甚至可以根据设备性能实施不同的收集算法。
...
**如果你感兴趣，你可以看看我们怎么** [**actually do it**](https://github.com/atlassian/react-beautiful-dnd/blob/0fb4dc75ea9b625f64cac48602635ac2822f26ec/src/view/style-marshal/style-marshal.js)。
...
使用这种技术，我们还能够优化拖放生命周期中的其他阶段。 我们现在可以更新样式项目，而无需 `render` 它们。

**注意：您可以通过创建预置样式规则集，然后更改 `body`上的 `class` 来激活不同的规则集来实现类似的技术。 然而，通过使用我们的动态方法，我们可以避免需要在_body_ _上添加_class_es，并允许我们随着时间的推移使用具有不同值的规则集，而不仅仅是固定的。 然而，通过使用我们的动态方法，我们可以避免在 `body` 上添加 `class`es。并允许我们随着时间的推移使用具有不同值的规则集，而不仅仅是固定的。**
...
我们现在只移动对用户来说部分可见的东西，而不是移动用户看不到的项目。 因此完全不可见的项目不会移动。这大大减少了我们在进入大列表时需要做的工作量，因为我们只需要 `render` 可见的可拖动项目。

当检测可见的内容时，我们需要考虑当前的浏览器视口以及滚动容器（带有自己滚动条的元素）。一旦用户滚动，我们会根据现在可见的内容更新位移。在用户滚动时，确保这种位移看起来正确，有一些复杂。他们不应该知道我们没有移动那些看不见的项目。以下是我们提出的一些规则，以创建在用户看起来是正确的体验。

*   如果项目需要移动并且可见：移动项目并为其运动添加动画
*   如果一个项目需要移动但它不可见：不要移动它
*   如果一个项目需要移动并且可见，但是它之前的项目需要移动但不可见：请移动它，但不要使其产生动画。

因此我们只移动可见项目，所以不管当前的列表有多大，从性能的角度看移动都没有问题，因为我们只移动了用户可见的项目。
...
当用户拖动 `Droppable` 列表时，我们通过更新 `isDraggingOver`  属性让用户知道。但是，这样做会导致 `Droppable` 的 `render` - 这反过来会导致其所有子项 `render` - 可能是 100 个 `Draggable` 项目！
...
在 react-beautiful-dnd 中我们经常使用数组去存储有序的数据。但是，我们也希望快速查找此数据以检索条目，或查看条目是否存在。通常你需要做一个 `array.prototype.find` 或类似的方法来从列表中获取条目。 如果你这样做了很多，对大型数组而言可能性能很差。
...
我希望你发现这个博客很有用，可以考虑一些可以应用于自己的库和应用程序的优化。看看 [react-beautiful-dnd](https://github.com/atlassian/react-beautiful-dnd)，并与 [our examples](https://react-beautiful-dnd.netlify.com) 一起玩。
