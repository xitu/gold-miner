...
既然这种过程实际上更改了目前的 Session 实例，我让它永远不使用从我的 `getEntitiesSession()` 返回的「共享的」Session 实例。如果我在一个 reducer 里需要使用，无论如何我总是创建一个新的 Session。如果我在一个形实替换程序（thunk）中需要使用，我用另一个选择器来为这个任务创建一个单独的 Session 实例：
