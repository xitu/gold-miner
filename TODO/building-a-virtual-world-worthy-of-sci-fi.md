...
两种解决方案都描述了一种生成用户通过 VR 头盔就可以体验的 3D 环境的设计，使用不同级别的云计算和云存储来给客户端提供虚拟地球的数据，并且实时计算用户与之交互时对世界环境的改变。

**Reto 的方案**专注于使用数百万个无人机获取实时传感器数据，创建一个真实世界的虚拟克隆。他的虚拟空间本质上是和现实世界联系在一起的，包括几何形状和当前的天气条件。
...
模仿这个过程，让我们来生成一个非常相似的东西。我们可以使用卫星数据，LIDAR（激光雷达）输入，还有来自各个角度和来源的无人机图片，并把他们放到一个 GCS bucket 里面。

除了这些，我们生成工作信息并将 work token 推送到 pub/sub。我们有一批抢占式虚拟机，负责收集这些 pub/sub 请求，并开始制作 3D 网格和纹理图集。最终的结果也被推送到 GCS bucket 里面。
...
当我们所有的网格和纹理数据都处理完毕的时候，结果将是数以 TB 的虚拟环境数据。很明显，我们不能一次将所有内容都传输给每个客户，相反，我们会根据地理边界捆绑模型数据。
...
为了优化客户端的加载时间，给他们经常访问的区域添加本地缓存是非常有意义的，避免他们每次进入一块新的区域就需要下载大量数据。

![](https://cdn-images-1.medium.com/max/800/1*6b2I4tUiPyn3pVSw7aPwfg.png)

为了图表清晰起见，我们可以将整个过程封装起来作为一个离线系统，给他起个名字叫做『自动内容生成器（ACG）』。

随着时间的推移，本地缓存将会失效，或者需要将更新推送给用户。为此，我们制定了更新和分期流程，客户可以在登录时或在他们重新进入他们最近访问的区域时接收更新的环境数据。
...
> 或者我们可以使用 app engine flex，这将允许我们提供我们自己的图像，但规模也一样。或者我们可以使用 app engine 标准，它有自己的部署和扩展。
...
随着你的用户移动并且和虚拟环境交互，他们所做出的任何改变都需要和其他的周边数据同步，并分享给其他用户。

你需要一些复合组件来确保用户操作不违反任何物理规则，然后是用于将这些信息存储或者广播给其他玩家的系统。
...
所有需要分发到多个其他客户端的持久性数据将存储在云端 Spanner 中，这将确保区域比较接近的客户端在发生这种情况时尽快共享信息。
...
由于我们的代码需要经常改变，我们需要增加我们的更新和临时服务器以将代码分发到我们的 world-shards。为了实现这一点，我们允许在暂存代码中执行计算级分段，并将图像推送到 Google Container Registry，以便根据需要支持各种 world shards 和游戏服务器。

![](https://cdn-images-1.medium.com/max/800/1*V0jjfEVbgTpBA1T91L1W1A.png)

### 绘制你的虚拟世界

除非您戴上 VR 头盔，否则虚拟世界就不是一个有意义的虚拟世界。 为此，你可以利用 Google VR 和 Android Daydream 平台在完全身临其境的 VR 体验中呈现我们巨大的虚拟世界。然而，Daydream 本身并不是一个适当的渲染引擎，因此你需要利用像 UNITY 这样的工具来帮我们绘制所有模型，并代表我们与 Daydream 系统进行交互。
...
> 其次，它内置了 IAM 规则和配置，因此我们可以写更少的代码就能获得我们所需的安全保证和登录系统。
