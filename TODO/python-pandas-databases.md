...
在我们开始之前，让我们先快速检阅下我们之后要处理的数据。我们看到的是航空公司的航班数据，它包含了有关航空公司的信息，机场名称和往来各个机场的航线名称。每一条航线会显示出一个重复的航班，它是往返与目的地和始发地的航空公司。
...
对于现实世界中的数据科学来说，最难处理的部分就是那些经常几乎每秒都不停变换着的数据。拿 `aireline` 这个例子来说，我们可能决定在 `airelines` 表中新增加一个 `airplanes` 的属性，它显示出每一个航空公司拥有多少架飞机。幸运的是，在 `SQLite` 中有一个方式能修改表并且添加这些列:
