...
当这个审查制度被有效使用时，它通常能发现这个程序 **60%** 的问题（不管是程序缺陷还是错误）。根据 McConnell 的研究，相比于不怎么流程化的代码审查，这个制度平均每 1000 行代码能减少 20% 到 30% 的错误。
...
McConnell 的研究发现了下面这些有关于小型代码审查的事实：
...
总的来说，从 1 分（**非常不认可**）到 10 分（**非常认可**），Swift 开发团队给代码审查认可度打了 9.5 分的高分。Ruby 开发团队第二，打出了 9.2 的高分。
...
> 我们有一个开发人员只是盲目的给每个 PR 一个赞然后都不怎么留评论。我可以告诉你这是真的因为他们 1 分钟能看 5 到 6 个 PR。
...
> 长长的 PR 总是花费大量的时间，然后这个 PR 还对将来的特性有重要的影响。
