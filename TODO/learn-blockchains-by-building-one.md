...
记得区块链是**不可变的，有序的**记录的链，记录也叫做块。块可以包含交易，文件或者任何你能想到的数据。不过至关重要的是，它们由**哈希值****串**在一起。
...
用你喜欢的编辑器或者 IDE，新建 `blockchain.py` 文件，我个人比较喜欢 [PyCharm](https://www.jetbrains.com/pycharm/)。本文全文都使用这一个文件，但是如果你搞丢了，可以参考参考[源代码](https://github.com/dvf/blockchain)。

### 表示区块链

创建 `Blockchain` 类，其构造函数会创建初始为空的列表（用来存储区块链），其他用来存储交易信息。这个类的设计如下：
...
`Blockchain` 类负责管理链。它用来存储交易信息，也有一些帮助方法用来将块添加到链中。我们接着来实现一些方法。

### 块是什么样子的？

每个块都有其**索引**，**时间戳**（Unix 时间），**交易列表**，**证明**（稍后解释），以及**前序块的哈希值**。
...
到这里**链**的概念就介绍清楚了，每个新块都包含于其中，也就是前一个块的哈希。**这一重要概念使得区块链的不可变性成为可能：**如果攻击者篡改了链中的前序块，**所有**的后续块的哈希都是错的。

理解了吗？如果没有想明白，花点时间思考一下，这是区块链的核心思想。

### 在块中添加交易信息

还需要在块中添加交易信息的方法。用 `new_transaction()` 方法来做这件事吧，代码写出来非常直观：
...
`new_transaction()` 在列表中添加新交易之后，会返回该交易被加到的块的**索引**，也就是指向**下一个要挖的块**。稍后我们会发现索引对提交交易的用户比较有意义。

## 创建新块

实例化 `Blockchain` 类之后，需要新建一个**起源**块，它没有任何前序块。此外还要在起源块中加入**证明**，证明来自开采（或者工作量证明）。稍后再来讨论开采这件事。

除了要在构造函数中创建**起源**块，我们还要实现  `new_block()`，`new_transaction()` 和 `hash()`。
...
上面的代码还是比较直观的，我还加了一些注释和文档字符串做进一步解释。这样就差不多可以表示区块链了。但是到了这一步，你一定好奇新块是怎么创建，锻造或者开采的。

### 理解工作量证明

工作量证明算法（PoW）表述了区块链中的新块是如何创作或者开采的。PoW 的目的是发现可以解决问题的数字。用计算机术语讲，对网络中的任何人来说，该数字必须**难于寻找，易于证明**。这是工作量证明算法背后的核心思想。

下面给出一个非常简单的例子来帮助理解。

不妨规定某整数 `x` 乘以另一个 `y` 的哈希必须以 `0`结尾。也就是 `hash(x * y) = ac23dc...0`。就这个例子而言，不妨将令 `x = 5`。用 Python 实现如下：
...
比特币的工作量算法叫做 [`Hashcash`](https://en.wikipedia.org/wiki/Hashcash)。它和上面给出例子也没有多大的不同。矿工们争相求解这个算法以便创建新块。总体而言，难度大小取决于要在字符串中寻找多少个字符。矿工成功求解就会得到一个币作为奖励。
...
> 寻找数字 `p`，当它和前一个块的解一起求哈希时，该哈希开头是四个 `0`。
...
要调整算法的难度，直接修改要求的零的个数就行了。不过 4 个零足够了。你会发现哪怕只加一个零都会让求解难度倍增。

类写得差不多了，可以用 HTTP 请求与之交互了。

## 第二步：将区块链用作 API

可以使用 Python Flask 框架。Flask 是一个微框架，易于将网络入口映射到 Python 函数。由此可以轻易地借助 HTTP 请求通过网络和区块链交互。

需要创建三个方法：

* `/transactions/new` 在块中新增交易
* `/mine` 通知服务器开采新节点
* `/chain` 返回完整的区块链

### 开始 Flask 吧

这个服务器会构成区块链网络中的单一节点。下面是一些模板代码：
...
* **第 24–26 行:** 创建 `/mine` 入口，使用 `GET` 方法。
* **第 28–30 行:** 创建 `/transactions/new` 入口，使用 `POST` 方法，因为要给它发数据。
* **第 32–38 行:** 创建 `/chain` 入口，它会返回整个区块链。
...
因为已经有类方法将交易加到块中，剩下的就很简单了。写一个添加交易的函数：
...
开采入口是见证魔法的地方，而且非常简单。只需要做三件事：

1. 计算工作量
2. 奖励矿工（这里就是我们），加一次交易就赚一个币
3. 将块加入链就可以锻造新块
...
注意，开采出的块的接受人就是节点的地址。这里做的事情基本上就是和 Blockchain 类的方法打交道。代码写到这里就差不多搞定了，下面可以和区块链进行交互了。

## 第三步：和区块链交互

可以用简陋又古老的 cURL 或者 Postman 来通过网络用 API 和区块链交互。
...
通过 `GET` 请求 `http://localhost:5000/mine` 尝试开采一个块。
...
重启了服务器，又开采了两个块，现在总共就有了三个块。通过请求 `http://localhost:5000/chain` 来查看全部区块链：
...
非常酷对不对？我们已经构建了基本的区块链，不仅支持交易，还可以开采新块。但是区块链的核心是**去中心化**。但是如果要去中心化，怎么知道每个块都在同一个链中呢？这就是**共识**问题，如果网络中不只这一个节点，必须实现共识算法。

### 注册新节点

在实现共识算法之前：我们需要让节点知道其在网络中的邻居节点的存在。网络中的每一个节点都应该保存网络中其他节点的信息。所以要写几个新入口：

1. `/nodes/register` 接受新节点的列表，形式就是 URL。
2. `/nodes/resolve` 来实现共识算法，解决所有冲突，确保节点的链是正确的
...
上面提过，冲突就是一个节点的链和其他节点的不同。要解决冲突，我们要制定一个规则：**最长有效链即权威**。也就是说，网络中最长的链就是**事实上**正确的链。利用这个算法，就可以在网络中的多个节点中实现**共识**。
...
第一个方法 `valid_chain()` 会检查链的有效性，主要是通过遍历每个块，验证哈希和工作量证明。

`resolve_conflicts()` 方法会遍历所有邻居节点，**下载**它们的链，用上面的方法来验证。**如果找到了有效链，而且长度比本地的要长，就替换掉本地的链**。

接下来将这两个入口注册到 API 中，一个用来新增邻居节点，另一个来解决冲突：
...
我在 2 号节点开采了新块，保证 2 号节点的链更长。然后用 `GET` 调用 1 号节点的 `/nodes/resolve`，可以发现链被通过共识算法替换了：
