...
即时是在行内设置，CSS 变量也会生效。在 JavaScript 中，我们控制了文档的根节点，然后就可以在行内给 CSS 变量设置新的值了。
...
这个 transform 简写用了两个不同的函数。一个是角度，另一个是沿着 Y 轴旋转。
...
激活三维空间后，元素具有了 x，y，z 平面。`rotateY` 就是元素沿着 `Y` 平面旋转。
...
那么，你可以在生产环境使用 CSS 变量吗？当然可以！但是你自己还是要检查一下普及率。
