...
在 JavaScript 语言的国际标准 ECMAScript 的 ES6 版本中，我们得到了新的异步原生对象 [Promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise)。这是一个非常强大的概念，它使我们可以避免臭名昭著的 [回调地狱](http://callbackhell.com/)。例如，几个异步操作很容易形成下面这样的代码：
...
正如你所看到的，我们嵌套了几个回调，如果想要改变一些回调的顺序，或者想同时执行一些回调，我们将很难管理这些代码。通过 Promise，我们可以将其重构为更可读的版本：

```
// 我们不再需要回调了 – 只需要使用 then 方法
// 处理 Promise 的返回结果
function updateUser() {
  return fetchData()
    .then(updateUserData)
...
这样代码不仅更简洁，可读性更强，而且可以轻松切换回调的顺序，同时执行回调或删除不必要的回调（或者在回调链中间新增一个回调）。

> 使用 Promise 链式写法的一个缺点是我们无法共享访问每个回调函数的作用域（或者其中未返回的的变量），你可以阅读 Alex Rauschmayer 博士这篇 [伟大的文章](http://2ality.com/2017/08/promise-callback-data-flow.html) 来解决这个问题。

但是，我发现了 [这个问题](https://stackoverflow.com/questions/30233302/promise-is-it-possible-to-force-cancel-a-promise)，你不能取消 Promise，这是一个真正的问题。有时你**需要**取消 Promise，你要构建变通的方法 — 工作量取决于你多长时间使用一次这个功能。
...
这比以前的解决方案稍微冗长一点，但是它完全一样，如果你没有 Bluebird（或者不想在 Promise 中使用非标准的方法），这是一个可行的解决方案。正如你所看到的，我们改变了签名 - 现在我们返回对象而不是一个 Promise，但实际上我们可以传递一个参数给函数，一个对象，并附上`cancel`方法（或者 Promise 的 monkey-patch 实例，但它也会在以后给你造成问题）。如果你只在几个地方有这个要求，这是一个很好的解决方案。

## 切换到 generators

Generators 是 ES6 另一个新特性，但由于某些原因，它们并不流行。想想看，在采用它们之前 - 你是作为新手会感到困惑呢？还是可以应付呢？而且，它还存在于其他一些语言中，如 [Python](https://wiki.python.org/moin/Generators)，所以作为团队使用这个解决方案可能很容易。
...
这是一个相当长的函数，但基本上它（除了检查，当然这是一个非常初级的实现） - 代码本身将保持完全相同，我们将从字面上获取 cancel 方法！让我们来看看在我们的例子中它将如何：
...
正如你所看到的，表面保持不变，但是现在我们可以选择在执行过程中取消任何基于 Generator 的函数，只需将其包装到合适的运行器中即可。缺点是一致性 - 如果它只是在你的代码中的几个地方，那么其他人会发现你在代码中使用所有可能的异步方法，这又是一个折衷。
...
在 [ES2017](https://tc39.github.io/ecma262/2017/#sec-async-function-definitions) 版本提供了 async/await，你可以在 Node.js 中没有任何标志的情况下使用它们从 [版本7.6](https://www.infoq.com/news/2017/02/node-76-async-await)。不幸的是，没有任何东西可以支持取消 Promise，而且由于 async 函数隐含地返回 Promise，所以我们不能真正感觉到它（附加一个属性或返回其他东西），只有 resolved/rejected 状态的值。这意味着为了使我们的函数可以被取消，我们需要传递一个对象，并将每个调用包装在我们著名的包装器方法中：
...
这是非常相似的解决方案，但是因为我们不直接在`cancel`方法中调用 reject，所以可能会使读者感到困惑。另一方面，它是现在语言的一个标准功能，它具有非常方便的语法，它允许你在后面使用前面调用的结果（所以在这里解决了 Promise 链式调用的问题），并且具有非常简明和直观的通过`try / catch`的错误处理。所以，如果取消不再困扰你（或者你可以用这种方式来取消某些东西），那么这个特性绝对是在现代 JavaScript 中编写异步代码的最好方式。

## 使用 streams (就像 RxJS)

Streams 是完全不同的概念，但它实际上是广泛的 [不仅在 JavaScript ](http://reactivex.io/)，所以你可以将其视为独立于平台的模式。Streams 是比 Promise/Generator 更好或者更糟糕的。如果你已经接触过它，并且使用它来处理一些（或者所有的）异步逻辑，那么会更好，如果你没接触过，那就糟了，因为它是完全不同的方法。

我不是一个使用 Streams 的大专家，但是使用过一些，我可以说你应该使用它们来处理所有的异步事件，或者完全不使用它们。所以，如果你已经在使用它们，这个问题对你来说应该不是一件难事，因为这是 Streams 库的一个长期众所周知的特性。

正如我所提到的，我没有足够的经验来提供使用它们的解决方案，所以我只是放几个关于 Streams 实现取消的链接：
...
事情朝着正确的方向发展 - fetch 将会新增 [abort](https://github.com/whatwg/fetch/issues/447) 方法，如何取消 Promise 正在长时间的热烈讨论中。会形成某种取消的实现吗？可能会可能不会。而且，取消 Promise 对于许多应用程序来说不是至关重要的 - 是的，你可以提出一些额外的请求，但有一个以上的请求结果是非常罕见的。另外，如果发生一次或两次，则可以从一开始就使用扩展示例来解决这些特定函数。但是，如果你的应用程序中有很多这样的情况，请考虑一下上面列出的内容。
