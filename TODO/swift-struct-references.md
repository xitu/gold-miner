...
最近我打算为 Swift 的最新的 keypath 找一个好的使用场景，这篇文章介绍了我意外获得的一个使用示例。这是我刚研究出来的，但还没实际应用在生产代码上的成果。也就是说，我只是觉得这个成果非常酷并想把它展示出来。

思考一个简单的通讯录应用，这个应用包含一个展示联系人列表的 tableView 和展示联系人对象详细信息的 viewController。如果把 `Person` 定义成一个类的话，大概是这个样子：
...
- 当地址发生变化时，收到通知就更难了。观察对象的嵌套属性则是最困难的。
- 如果我们需要给 `Person` 创建一个独立的本地 copy，我们就需要实现一些像 `NSCopying` 这样的东西， 这需要不少的工作量。甚至当我们决定这么做时，我们还需要考虑是想要深拷贝（比如：地址也会背拷贝）还是浅拷贝（地址数组是独立的，但是里面的地址仍然指向相同的对象）呢？
...
- 每个结构体都是独立的拷贝。这是有用的，因为我们知道它总是一致的，不会在我们手底下改变。然而，当我们在详情 view controller 中对 `Person` 做了修改时。我们就需要一个方法来将这些改变反馈给 tableView（或者说通讯录列表）。而对于对象，这种情况会自动发生（通过修改 `Person` ）。
- 我们可以观察通讯录结构体的根地址，从而知道通讯录发生的任何变化。但是，如果我们想观察这个也并不是那么容易（比如：观察第一个人的名字）。

我现在提出的解决方案结合了两个方案的最大优势：

- 我们有可变的共享指针
- 因为底层数据是结构体，所以我们可以随时得到我们自己的独立拷贝
- 我们可以观察任何部分：无论在根级别，还是去观察某个人的属性（例如第一个人的名字）

我接下来会演示这个方案怎么使用，如何工作，最后再说说方案的局限性和遇到的问题。
...
我们用一个初始化的空数组来创建一个新的 `addressBook`。然后添加一个 `Person` 。接下来就是最酷的地方：通过使用下角标我们可以获得指向第一个人的 **指针** ，接着是一个指向他们名字的 **指针** 。我们可以将指针指向的内容改为 `“New Name"` 来验证我们是否更改了原始的通讯录。
...
我们可以监视任何 `Ref` 。就像 reactive 库一样，我们得到了一个可以控制观察者生命周期的一次性调用：
...
我们也可以监视 `nameOfFirstPerson` 。在目前的实现中，无论什么时候做了什么更改都会触发通讯录的改变，但以后的实现会有更多的功能。
...
让我们返回我们的 `PersonVC` 。我们可以使用 `Ref` 作为他的实现。 这样 viewController 就可以收到每一次更改。在实际的编程中，信号通常是只读类型的（你只会收到发生了变化的信息），这时你需要其他方式进行回传消息进行交流。 在 `Ref` 方案中，我们可以使用 `person.value` 进行回写：
...
我们现在可以添加一个初始值，这样我们就有了可以观察单个结构体的值。它创建了一个观察者和变量对应的字典。这样无论变量什么时候被修改了，所有的观察者都会被通知到。它使用初始化方法定义，并传递给 `get`, `set`, 和 `addObserver`:
...
想一下我们现在已经有 `Person` 指针，为了拿到 `Person name`  的属性指针，我们需要一种方式来对 name 进行读写操作。而 `WritableKeyPath` 恰好可以做到。因此，我们可以在 `Ref` 中添加一个`subscript` 来创建可以指向 `Person` 某一部分的指针：
...
上面的代码有一点难于理解，但如果只是为了理解这个库，我们不需要真的弄明白它是怎么实现的。

也许某一天，Swift 中的 keypath 也会支持下标，但至少现在没有，接下来我们必须为集合添加另外一个下标。除了使用索引而不是关键字，它的实现几乎就跟上面的一样。
...
这就是全部实现了。上面代码使用了 Swift 大量先进的功能，但它仍保持在 100 行代码以下。如果没有 Swift 4 最新功能，这也基本不可能实现。它依赖于密钥，通用下标，开放范围以及以前在 Swift 中提供的许多功能。

### 讨论

就如之前所提到的那样，这些仍处于研究中而不是生产级的代码。一旦我开始在一个真正的应用程序中使用它，我非常有兴趣了解它将会在哪里，会怎么遇到问题。 下面就是其中一个让我有一些非常反直觉行为的代码段：
...
我非常愿意致力于它的研究。我甚至可以想象的到，如果我为他添加队列支持，你就可以像下面那样使用：
...
我期待着听到您的评论和反馈，如果你徐昂要更深入的理解他是如何工作的，试着自己去实现它（即便你已经看了代码）。顺便提一下，我们将会以它为主题开展两场 [Swift Talk](http://talk.objc.io/)。如果你，如果你对 Florian 和我从头开始构建这个项目感兴趣，就订阅它吧。
