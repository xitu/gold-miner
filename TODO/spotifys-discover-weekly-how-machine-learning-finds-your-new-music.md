...
我是 Spotify 的忠实粉丝，尤其喜欢它的每周推荐功能。因为，它让我感觉到我被**重视**着。它能了解我与别人不同的音乐品味，并在每周都能给出令我满足的推荐。如果没有它，我可能一辈子都找不到一些我非常喜欢的歌曲。
...
此外，Last.fm 采用了一种名为**协同过滤**的不同的方法，至今仍在使用。它可以识别用户可能喜欢的音乐。稍后会详细提到它。
...
实际上 Spotify 并没有使用某个革命性的推荐模型 —— 与此相反，**他们是将一些其它服务中使用的最佳策略混合起来，创建了自己独特、强大的发现引擎。**

Spotify 每周推荐的开发者主要采用了如下三种类型的推荐：

1. **协同过滤**模型（类似于 Last.fm 最开始使用的模型），通过分析**你的**行为与**他人**的行为进行运作。
...
**”Emmmmm，你们都喜欢 Q、R、S 三首歌，所以你们可能是类似的用户。所以，你应该会喜欢另外一个人喜欢听而你还没听过的歌。“**
...
现在，我们有了 1.4 亿条用户向量以及 300 万条歌曲向量。这些向量的内容实质上就是一堆数字，本身没有任何意义。但是对它们进行对比就能起到巨大的作用。
...
接下来与协同过滤一样，NLP 模型会使用这些短语和权重为每首歌构建一个表示向量，这样就能判断两首歌是否相似了。
...
卷积神经网络正式人脸识别背后使用的技术。在 Spotify 这个场景中，工程师们使用音频数据来代替像素。下面是神经网络一中结构的实例：
...
最终，这些由一首歌理解到的各种关键的信息可以让 Spotify 理解不同的歌中的一些基本的相似之处，由此基于用户的听歌历史推断出此用户可能会喜欢这首新歌。
...
我希望本文的信息能满足你的好奇心（就像我的好奇心被满足了一样）。现在我正在通过我个性化的每周推荐找到我喜欢的音乐，了解以及学习它背后的各种机器学习知识。🎶
