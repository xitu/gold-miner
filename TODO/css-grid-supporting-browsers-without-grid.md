...
在阐明如何在去支持那些本身不支持网格的浏览器之前，很有必要搞明白 _支持_ 的含义。支持可能意味着站点必须在列表中的浏览器上看起来完全相同。这可能意味着对于所有的浏览器，你都可以不用去做一些收尾工作。这可能意味着你在测试这些浏览器的时候对他们能获得简化的体验而感到十分高兴。

一个相关的问题就是 **你怎么拿出支持的浏览器的列表？** 即使是一个全新的网站，这也不应该是一个猜测。对于今天的大多数的企业来说，一个新网站不会成为他们建设的第一个网站。你可能会看到一些可用于查看正在使用的浏览器的分析，但是需要注意的是，它们不会对一个完全对移动端不友好的网站倾斜（这句我不太懂）。人们不会去访问一个在小屏幕上没法用的网站！

如果没有任何的相关分析，你可以在 [Can I Use](https://caniuse.com/) 上面导入你所在位置的数据。
...
在编写的时候，Edge，Chrome，Firefox，Opera，Safari，iOS Safari 都支持了网格布局。

IE10 和 IE11 支持带有 `-ms` 前缀的原始规格。对于你正在浏览的 _旧_ 浏览器来说：
...
UC 浏览器不支持网格布局。他不但针对低功耗设备进行了优化，也适用于昂贵且经常计量数据的地区的用户。这是我们在开始规划支持的一个重要考虑因素。

### 有一个 CSS Grid 的 Polyfill（垫片）？


在第一次遇到 CSS Grid 的时候，一个显而易见的问题是：“我可以使用 polyfill 吗？”不幸的是，即使有这样的好事，对于整个布局来说，一个神奇的 polyfill 既不太可能出现，也不是一个好主意。

使用旧的布局方式，网格几乎做不到这一点。所以，为了在没有支持的浏览器中复制网格，需要在 JavaScript 中做很多的工作。即使在资源充足的计算机上，使用了快速渲染引擎，在计算高度和元素的定位方面还是可能会带来一些令人生厌的体验。我们已经知道，**不支持网格的浏览器**是新兴市场上低功耗设备上最常见的 **较老**，或者较慢的浏览器。为什么硬要在这些设备上放一堆 JavaScript 呢？

不要搜索一个 polyfill，而是要考虑如何使用网格布局为那些不支持的浏览器提供更好的体验。使用网格可以为最小 CSS 支持的浏览器创造出复杂的布局，同时仍然可以为那些不支持的浏览器提供良好的体验。这样会比仅仅在这个问题上抛出一个 polyfill 多一些工作，但是这样做的话，你可以保证能提供良好的体验，反而让网站在所有的地方显示相同不是最重要的目标。

### 为原本不支持网格布局的浏览器提供支持

那么，我们如何为正在使用的设备和浏览器提供定制的支持？事实证明，CSS 中有你的答案。
...
使用了浮动（float）或者清除（clear）属性的将会成为网格元素，就好像没有应用浮动或者清除一样。在下一个 CodePen 中删除应用了 `.grid` 类的所有属性，你可以看到我们所有的项目是如何浮动的，第三个项目是如何清除浮动的。但是在网格布局中，这将被忽略。
...
当你使用 `display: table-cell` 创建列，CSS 将创建所谓的 _匿名框_ 。这些事表格的缺失部分 —— 真正的 HTML 表格中的单元格将在 `table` 元素里边的 `tr` 元素内。匿名框基本上解决了这些失踪的父元素。如果你的 `table-cell` 元素变成了一个网格元素。这样这个元素的 table 显示同样会失效，就像什么也没有发生。

`vertical-align` 属性在网格布局中再一次不适用。因此如果你可以在 CSS 表格布局或 `inline-block`中使用它，则可以安全的忽略该属性，尽情使用网格布局的框对齐方式。你可以在下一个CodePen中看到一个使用 CSS Grid 覆盖 `display：table-cell` 和 `vertical-align` 的布局。
...
你同样可以使用 Flexbox 作为一个回退方案，一旦你在一个网格元素上使用 `flex` 属性或者独立的 `flex-grow`，`flex-shrink` 或者 `flex-basis` 属性，同样会失效。

最后，请不要忘记多列布局在某种情况下可以作为一个回退方案。例如当列出卡片或者图像时，它将显示列中的数目，而不是整行。但是在某些情况下可能是有用的。在容器上应用 `column-count` 或者 `column-width` 使其成为多列容器。然后应用 `display:grid` 将忽略 `column-*` 行为。

### 特征查询

其他的大多数布局方式中，大多都只是针对单个项目而不是其容器。例如在浮动布局中，我们有一堆给定了百分比宽度的项目，为其设置左浮动（float: left）。这将让他们排列在一起。只要总数不超过父容器宽度的 100%，我们就可以实现类似网格的效果。
...
你需要做多少用于不支持浏览器的 CSS 覆盖，取决于你决定为这些较旧的浏览器创建多少不同的布局。

### IE10和11版本的网格布局

虽然 Edge 浏览器现在已经升级到支持现代网格布局，但是 IE10 和 IE11 只支持早起版本在这些浏览器加 `-ms` 前缀的写法。我们今天所知道的网格规范最初来自于微软。对这个老的实现方案，我们不是不高兴。我们应该很高兴他们开始了这个过程，首先是给了我们网格。你可以从他创作者的文章 [CSS 网格的故事，来自它的创作者](https://alistapart.com/article/the-story-of-css-grid-from-its-creators)。
...
如果你的列表中有不支持的浏览器，那么你 _必须_ 为他们提供和那些已经被支持的浏览器相同的体验。然后我们就会怀疑是不是应该用网格布局，或者任何新的 CSS。使用可用的方法，这种方法仍然是非常高效的。（这里我没懂。。。qwq）

你可能还在考虑使用网格布局是不是有一个优良的回退方案，如果你知道，短期内很可能你会从“必须是相同的”列表中抛弃一堆不兼容的浏览器。特别是如果你知道现在做的开发会有很长的保质期。然后，你可以在晚一点的时候，只使用网格版本，丢掉回退方案。

但是，对于您的支持可能意味着有可能会失去对一些浏览器的兼容来简化一些开发工作，你要做的事情非网格布局不可，这是对网格布局和设计支持良好的浏览器的体验时间。

### 回退测试

测试回退是最后一步。测试你的回退方案是否奏效的唯一方法就会使用不支持 CSS 网格的浏览器访问。使用[下载微软提供的虚拟机](https://developer.microsoft.com/en-us/microsoft-edge/tools/vms/)的这种方式，你可以不必购买其他电脑。然后，就可以测试使用 Internet Explorer 版本不支持。

你可以在手机上下载 UC 浏览器，或[使用桌面版]()的 Windows 或者虚拟机。

还有比如说可以访问整个运行范围内浏览器的远程虚拟机工具 [BrowserStack](https://www.browserstack.com)。这些服务不是免费的，但是他们而已为你节省大量测试设置虚拟机的时间。
...
我看到过有人推荐你换一下特征查询测试的内容 —— 根本就是扯淡。比如测试 `display: gridx`。这是能正常工作，但是你需要把素偶有的网格代码放到特征查询的代码块里边，而忽略了浏览器会忽略不支持的 CSS 代码的事实。如果你不知道有些网格代码可能会结束在特征查询之外，那么你很容易会得到一个假阳性结果。即使你在使用这个方法进行快速检查，我仍然强烈建议你做一些真机测试。

#### 延伸阅读

我已经列出了这篇文章提到的网址，还有一些额外的志愿可以帮助你用自己的方式来支持浏览器，同时还能利用到新的布局优势。如果你遇到了任何好的资源，或者特别棘手的问题，都可以将他们添加到这个问题下面。网格布局对于我们所有人都是新生的东西，我们可以在生产环境中使用，但是不可避免会出现一些悬而未决的问题，让我们一起看看。
