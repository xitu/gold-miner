...
- 设计。即使是在收件箱中，一个设计良好的HTML邮件能让你突出你的品牌。
...
并不是每一个图片都需要是弹性布局。例如logos和社交网站的图标无论设备的大小如何都要保持相同的大小, 这就是我们用类名来标记需要弹性布局图片的原因。

由于我们经常覆盖我们写的内联样式和HTML属性，所以使用important声明使文档渲染的时候确保我们的响应式样式会被优先表现。
