...
即使你自认为是 CSS 专家，很可能在某一个大型项目中，你必须处理一个越来越错综复杂的样式表。它们中的一些看起来像是互相有关联并且缠绕在一起的网。

![意大利面怪物](https://cdn.css-tricks.com/wp-content/uploads/2017/11/spaghetti-monster.jpg)

层叠的作用非常强大。微小的改变可能会有很大的改变。很难知道下一秒会发生什么。重构、更改和移除 CSS 都是高位动作，因为很难知道这个 CSS 在哪里被引用。

> 一个很难使用新工具表达的事情是 **你什么时候开始想要达到这个目标？** 答案是无论在何种情况下，你都很少有这种想法。
>
> 其中的一种情况是，在我有限的经验中，在大型代码库的大团队中，**给他们的感觉是 CSS 太大了以至于团队的成员开始对 CSS 很敏感并且对 CSS 感到害怕，但是实际上只是让你仅仅增加 CSS。**
> 
> 由此产生一个工具，它能做的事情远远少于 CSS，但是在某种程度上（在你学会之后），没有人在对其感到害怕，我可以感受到它的吸引力。
...
[原子 CSS](https://css-tricks.com/lets-define-exactly-atomic-css/) 提供了一套直接、明显并且简单的方法论。类是不可变的，你不可以改变类名。这使得 CSS 的使用是可预见性和可靠的，因为类总是做**完全**相同的事情。在 HTML 文件中添加或者移除一个有作用域范围的共用类是明确的，它让你确信你不会破坏其他任何东西。这可以减少认知负荷和精神负担。

给组件命名是出了的困难。想出一个既有意义也足够通用的类名费时又费力。
...
```
原子的类从名字就可以知道它们的功能。意图和效果显而易见。而包含无数类名的 HTML 会显得很乱，HTML 比 一个庞大并且错综复杂的样式要容易一些。
...
### 样式差异处理
[工具类](https://css-tricks.com/need-css-utility-library/) 非常适合处理小的样式差异处理。虽然设计系统和模式库现在可能风靡一时，但是你要意识到这将会不断有新的需求和变化。通过你的方式，所有组件的可重用性的谈话往往不是体现在设计模拟（这句话没理解什么意思）。虽然实现和效果一致那么是很好的，但是一个大型网站繁多的上下环境一定会有很多的不可避免的不同。
...
如果希望我们的组件通过简单的方式和另一个组件只有细微的差别，该怎么去做呢？如果你使用的 BEM 的命名方式，修饰符类很可能会不起作用。无数的修饰符往只有一个效果。我们以边距（`margin`）为例。不同组件的边框大部分都不相同，让所有组件的边框保持一致也不太可能。这个距离不仅取决于组件，还取决于组件在页面中的位置和它相对于其他元素的相对位置。大部分的设计都包含相似但是 **不完全相同** 的 UI 元素，使用传统的 CSS 很难处理。

### 很多人都不喜欢它

![Aaron Gustafson，《A List Apart》的总编辑，前微软员工，Web Standards Project 的项目基本管理](https://cdn.css-tricks.com/wp-content/uploads/2017/11/twitter.com_AaronGustafson_status_743073596789133312_ref_srctwsrc5Etfwref_urlhttp3A2F2Fcssmojo.com2Fopinions_of_leaders_considered_harmful2F.png)
...
### 原子 CSS 和行内样式有什么不同？
这是质疑原子 CSS 的人经常会问到的问题。长期以来大家都认为行内样式不利于实践，自 Web 时代初期就很少有人使用了。**那些批评者将原子 CSS 与行内样式等同也并不是完全不对的，因为行内元素和原子 CSS 有相同的弊端。**举个例子，如果我们想要将所有的 `.block` 类中的 `color` 改变为 `navy` 会怎样？我们能这样做
```
.black {
  color: navy;
}
...
虽然原子 CSS 和内联样式一样有很大的缺陷，这不是一种退后。工具类以各种方式优于内联样式。
...
CSS 社区已经创建了很多用于行内样式的有用的工具例如：Sass， Less， PostCSS， Autoprefixer 等。

#### 更加简洁

与其写出冗余的行内样式，倒不如像原子 CSS 一样写出简洁的声明缩写。相比之下少打了一些字：`mt0` 和 `margin-top: 0`，`flex` 和 `display: flex`，等等。

#### 差异性

这是一个有争议的话题。如果一个类或者行内样式仅仅只做一件事情，**那么你是否希望它只做一件事情**，很多人提倡使用 `!importent` 来保证不被其他的除了 `!impirtent` 的样式重写，这也就意味着这个样式肯定会被应用。但是，一个类本身是足够具体的，可以覆盖其他的基本类。和行内样式相比，原子类特异性较低是一件好事。它允许更多的通用性。都可以使用 JavaScript 来改变样式。如果是行内样式的话就比较困难。
...
* 如果你需要在媒体查询中改变特定组件的很多样式。
* 如果你想要使用 JavaScript 改变很多类，将其抽象为一个单独的类更容易。

原子类可以和其他样式方法共存。我们应该将设置一些基础类和稳健的全局样式。如果你继续复制工具类的相似字符串，这些样式很可能被抽象为一个类。你可以在组件类中将其合并，但是你只能在知道它们不会被重用时才可以这样。


> 对 CSS 使用组件第一的方法意味着你你必须创建一个组件即使它以后再也不会被使用。这种过早的抽象就是使样式表变得冗余和复杂的原因。
> 
...
看一下 Bootstrap 的最新版本，现在提供了一整套的工具类，仍然包括其传统的组件。越来越多的流行框架采用这种混合方法。
