...
即使你自认为是 CSS 专家，也很可能在某一大型项目中，处理一个错综复杂越来越庞大的样式表，它们中一些样式表看起来就像一张相互继承混乱缠绕的网。

![意大利面怪物](https://cdn.css-tricks.com/wp-content/uploads/2017/11/spaghetti-monster.jpg)

级联的作用非常强大。微小的改变可能会有很大的改变。这就导致了很难知道下一秒会发生什么。重构、更改和移除 CSS 都是高危动作，因为很难知道这个 CSS 在哪里被引用。
...
### 样式差异处理
[工具类](https://css-tricks.com/need-css-utility-library/) 非常适合处理小的样式差异。虽然设计系统和模式库现在可能风靡一时，但是你要意识到这将会不断有新的需求和变化。通过你的方式，所有组件的可重用性的谈话往往不是体现在设计模拟（这句话没理解什么意思）。虽然实现和效果一致那么是很好的，但是一个大型网站繁多的上下环境一定会有很多的不可避免的不同。
