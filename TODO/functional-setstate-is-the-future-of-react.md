...
过去几年，setting-state 的逻辑（即传递给 `setState()` 的对象或函数）一直都存在于组件类内部，这更像是 imperative 而不是 declarative。（译者注：imperative 和 declarative 的区别参见 [stackoverflow上的问答](http://stackoverflow.com/questions/1784664/what-is-the-difference-between-declarative-and-imperative-programming)）

不过，今天我将向你展示新型内部法宝 —— **React 最为深藏不露的秘密**：
