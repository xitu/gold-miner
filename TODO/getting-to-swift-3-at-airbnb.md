...
我们有几十个模块和几个三方库，都是用 Swift 编写的，包括了几千个文件和几十万代码。就好像这个代码量还不足够具有挑战性一样，Swift 2 和 Swift 3 模块之间无法互相导入的事实，更加剧了迁移过程的复杂度。由于 Swift ABI 在版本 2 和 3 之间的改变，即使是正确的 Swift 3 代码引入 Swift 2 的库也不能编译。这个不兼容性，导致了代码并行优化变得异常困难。
...
新迁移的应用通过首次QA时有很多明显的问题。通过应用本文后面讨论的几种技术，大部分的问题都被三人小队快速地解决了（在几个小时内）。经过初步的消除容易的问题，高可见度的回归分析，我们的 iOS 团队在大型项目里留下了 15 个潜在回归，其中 3 个崩溃，这是我们在发布下一版本应用前需要解决的。
