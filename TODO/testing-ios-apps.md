...
大公司里会有专业的 QA 团队，但如果你是小团队的独立开发，那也就只剩下你去对自己的代码质量以及功能的可用性去做保证了。这会给你带来相当大得压力，因为你写的每一个功能都有可能对你应用的其他部分造成破坏。
...
测试项的放置和特定框架没有很好的结束，在这些类型测试失败时，大多数人会根据预期数字做更新然后继续。但不要这样做。
...
**注意：**我建议在你的项目中在改变截图时应该添加明确的权限，否则其他人会使用布局去做同样的事（只更新截图而不考虑其他）。当截图被修改的时候，你可以使用danger去通知用户。
...
通常 `beforeEach` 扮演着 **Arrange** 跟 **Act** 的角色，留下 **Assert** 去扮演剩下的角色特定的`it` 测试。
...
测试的回报会变高，项目的生命周期会越来越长，你越会感激自己在早期对测试的投入。
