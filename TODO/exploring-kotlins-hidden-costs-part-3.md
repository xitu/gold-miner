...
## 委托属性（Delegated propertie）和范围（range）
...
[委托属性](https://kotlinlang.org/docs/reference/delegated-properties.html) 是一种通过**委托**实现拥有 getter 和可选 setter 的 [属性](https://kotlinlang.org/docs/reference/properties.html)，并允许复用特定的属性实现。
...
委托对象必须实现一个拥有 `getValue()` 方法的操作符，以及 `setValue()` 方法来实现读/写属性。这些方法将会以额外的声明形式，如**包含对象实例**或**属性元数据**（正如其名字一样）。
...
但也有一些情况是**只需要一个委托势力来实现任何属性的**：当委托是无状态的，并且其只需要保持一个变量来完成保存已存在的对象和属性名的工作时，可以通过将其声明为 `**object**` 来替代 `**class**` 实现一个**单例**委托。

举个例子，下面的单例委托取回了在一个 Android `Activity` 中的标志与给定值相匹配的 `Fragment`：
...
如果你选择使用相同内部委托实例来在一个类中声明不同属性，你需要在构造函数中初始化实例。

**注意**：从 Kotlin 1.1 开始，也可以通过声明 [方法内部变量作为委托实例](https://kotlinlang.org/docs/reference/delegated-properties.html#local-delegated-properties-since-11)。在这种情况下，委托可以直到第一次使用方法时再初始化，而不必在构造函数中就执行初始化。

> 类中声明的每一个委托属性都会保护**与之关联委托类的开销**，并会在类中增加一些元数据。
...
针对常见情形，Kotlin 提供了一些基本委托，如 `[Delegates.notNull()](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.properties/-delegates/not-null.html)`、 `[Delegates.observable()](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.properties/-delegates/observable.html)` 和 `[*lazy()*](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/lazy.html)`。
...
默认模式 `LazyThreadSafetyMode.**SYNCHRONIZED**` 将提供相对耗费昂贵的 **双重检查锁** 来保证属性在**多线程**读取时也可以安全运行。
...
> 使用 `lazy()` 委托来避免初始化时的大量开销以及指定模式来避免不必要的锁。

---

### 范围

[范围](https://kotlinlang.org/docs/reference/ranges.html) 是 Kotlin 中用来代表一系列连续的值的特殊表达式。值可以是任何 `Comparable` 类型。 这些表达式的形式都是创建声明了 `[ClosedRange](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.ranges/-closed-range/)` 接口的方法。创建范围的主要方法是 `..` 操作符方法。

#### 包含（这边的test我都不知道啥意思，全当不存在test处理了）

范围表达式的主要作用是使用 `**in**` 和 `**!in**` 操作符实现包含和不包含。
...
该实现针对**非空基本类型的范围**（包括 `Int`、`Long`、`Byte`、`Short`、`Float`、`Double` 以及 `Char` 的值）实现了优化，所以上面的代码可以被优化成这样：
...
**零额外支出**并且没有额外对象开销。范围也可以被包含在 `**when**` 表达式中：
...
将属性的 getter 声明为 `**inline**` 的方法也无法避免这个对象的创建。**这是 Kotlin 1.1 编译器可以优化的一个点。**至少通过这些特定的范围类避免了装箱操作。

> 尽量**在使用时直接**声明非空基本类型的范围，不要间接调用，来避免额外范围类的创建。
> 或者直接声明为**常量**来复用。

范围也可以用在其他非基本类型的实现了 `Comparable` 的类型。
...
> 如果你需要通过调用实现了 `Comparable` 的非基本类型的范围来实现频繁的包含的话，考虑将这个范围声明为常量来避免重复创建范围类吧。

#### 迭代：for 循环

**整型范围** （除了 `Float` 和 `Double`之外其他的基本类型）也是 *progressions*：**它们可以被迭代**。这就可以将经典 Java 的 `**for**` 循环用一个更短的表达式替代。
...
反向迭代还有一种结果相同的方式，使用 `[reversed()](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.ranges/reversed.html)` 方法结合范围：
...
会创建一个临时的 `IntRange` 对象来代表范围，然后创建另一个 `IntProgression` 对象来反转前者的值。
...
一个次要提示，当生成的代码读取 `IntProgression` 的 `**last**` 属性时会通过对边界和步骤的小小计算来决定准确的最后值。在上面的代码中，最终值是 9。
...
遗憾的是，**编译器并没有针对这个经典的包含范围优化**，迭代器依然会创建范围对象：
...
> `**for**` 循环内部的迭代，最好只用范围表达式的**一个单独方法来调用 **`**..**`** 或 **`**downTo()**` 来避免额外临时递进对象的创建。

#### 迭代：forEach()

作为 `**for**` 循环的替代，可以临时使用 `forEach()` 在范围上的内联扩展来实现相似的结果。
...
但如果仔细观察这里对 `forEach()` 方法的调用的话，你就会注意到并没有优化范围，而只是优化了 `Iterable`，所以需要创建一个 iterator。下面是编译后代码的 Java 形式：
...
> 迭代范围时，最好只使用 `**for**` 循环而不是范围上的 `forEach()` 方法来避免额外创建一个迭代器。

#### 迭代：集合

Kotlin 标准库提供了内置的 `[*indices*](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.collections/indices.html)` 扩展属性来生成数组和 `Collection` 的范围。
...
但编译之后，我们可以发现**这并没有那么高效率**，因为编译器无法足够聪明地避免范围对象的产生：
...
> 当迭代没有声明 `Collection` 的**自定义集合** 时，**直接在 **`**for**`** 循环中写自己的序列范围**而不是依赖方法或属性来生成范围，从而避免范围对象的创建。
