...
**Prefetch 是提示浏览器这个资源将来能需要**，但是把决定是否和什么时间加载最佳这个资源的权利委托给浏览器。

<img src="https://cdn-images-1.medium.com/max/2000/1*PSMeFcC3AXDUmdNf5l19Ug.jpeg">

Preload 将 load 事件与脚本解析事件解耦，如果你还没有用过它，看看 Yoav Weiss 的文章 [Preload: What is it Good For?](https://www.smashingmagazine.com/2016/02/preload-what-is-it-good-for/)。

### Preload 在生产环境的成功案例

在我们深入细节之前，下面是上一年发现的使用 proload 并对加载产生积极影响的案例总结：

Housing.com 在对他们的仿原生 Web App 的脚本转用 proload 看到[**大约缩短了10%的可交互时间**](https://twitter.com/HousingEngg/status/844169796891508737)。
...
Treebo，印度最大的旅馆网站之一，在 3G 网络下对其桌面版试验，在对其顶部图片和主要的 Webpack 打包文件使用 preload 之后，在**首屏绘制和可交互时间分别节约了大约** [**1s**](https://twitter.com/__lakshya/status/844429211867791361)。

<img src="https://cdn-images-1.medium.com/max/2000/1*SKYdHNpGldFFUPZBDZQgSQ.png">

同样的，在对自己的仿原生 Web App 主要打包文件使用 preload 之后，Flipkart 在路由解析之前 **节省了大量的主线程空闲时间**（在 3G 网络下的低性能手机下）。

<img src="https://cdn-images-1.medium.com/max/1000/0*QL0ztXPZ1wUXpRKX.">

上面：未使用 preload，下面：使用 preload

Chrome 数据保护小组声明在对脚本和 CSS 样式表使用 preload 之后，对页面第一次绘制获得[**平均 12%**](https://medium.com/reloading/a-link-rel-preload-analysis-from-the-chrome-data-saver-team-5edf54b08715#.bgj9qkqfr) 的性能提升。

对于 prefetch ，它被广泛使用，在 Google 我们仍用它来获取可以加快 [搜索结果页面](https://plus.google.com/+IlyaGrigorik/posts/ahSpGgohSDo) 的喧染的关键资源。
...
下面是在 Chrome 46 版本中不同资源的加载优先级情况（ [Pat Meenan](https://docs.google.com/document/d/1bCDuq9H1ih9iNjgzyAL0gpwNFiEP4TZS-YLRp_RuMlc/edit#)）

<img src="https://cdn-images-1.medium.com/max/1000/1*BTi3YhvCAYiJYRpjNQft9Q.jpeg">

preload 用 “as” 或者用 “type” 属性来表示他们请求资源的优先级（比如说 preload 使用 as="type" 属性将获得最高的优先级）。没有 “as” 属性的将被看作异步请求，感谢 Paul Irish 更新这张关于开发者工具以及网络层上各种请求优先级的表。

我们来谈一下这张表。

**脚本根据它们在文件中的位置是否异步、延迟和阻塞获得不同的优先级**：

* 网络在第一个图片资源之前阻塞的脚本：中级
* 网络在第一个图片资源之后阻塞的脚本：低级
* 异步／延迟／插入的脚本（无论在什么位置）：很低
...
如果你想了解各种资源加载时的优先级属性，用开发者工具的 Timeline/Performance 的 Network 看到的信息如下：
...
**用“preload”和“prefetch”情况下，如果字眼不能被缓存，那么都有可能浪费一部分带宽。

没有用到的 preload 资源在 Chrome 的 console 里会在 3s 后发生警告。

<img src="https://cdn-images-1.medium.com/max/1000/0*Um55iV_tEBO3eXEs.">

原因是你可能为乐改善性能使用 preload 来缓存一定的资源，但是如果没有用到，你就做了无用功。在手机上，这相当于浪费了用户的流量，所以明确你要 preload 对象。
...
不要用 “prefetch” 作为 “preload” 的后备，它们是为了不同的场景很容易导致不符合预期的二次获取。使用 preload 来获取当前需要任务否则使用 prefetch 来获取将来的任务，不要一起用。

<img src="https://cdn-images-1.medium.com/max/1000/0*KKong0kz69LOteD3.">

不要在 preload 中使用 fetch()，在 Chrome 中这样使用将会导致二次的下载。这并不只发生在异步请求的情况我们有一个关于这个问题公开的 [bug](https://bugs.chromium.org/p/chromium/issues/detail?id=652228)。
...
**preload 字体不带 crossorigin 也将会二次获取！** 确保你对 preload的字体添加 [crossorigin](https://developer.mozilla.org/en-US/docs/Web/HTML/CORS_settings_attributes) 属性，否则他会被下载两次，这个请求使用匿名的跨域模式。这个建议也适用于字体文件在相同域名下，也适用于其他域名的获取(比如说默认的异步获取)。
...
当你知道资源加载的正确顺序时使用推送，用 service worker 来拦截哪些可能需要会导致二次获取的资源请求，用 preload 来加快第一个请求的开始时间 —— 这对所有的资源获取都有用。
...
preload 利于下载与执行的解耦，感谢其对文档 onload 事件的支持我们现在可以控制其加载完毕后的事件，获取 JS 包文件在空闲快执行或者获取 CSS 模块在正确的时间点执行，可以说是非常强大的。
...
FilamentGroup 也有一个作为他们的异步 CSS 加载库 [loadCSS](https://github.com/filamentgroup/loadCSS) [检测 preload](https://github.com/filamentgroup/loadCSS/blob/master/src/cssrelpreload.js#L8-L14) 的部分。
...
**有越来越多的 仿原生 Web App（比如 Twitter.com 移动端， Flipkart 和 Housing）使用它来加载当前链接需要的脚本：**
