...
前端性能可以从很多角度来考量。但是从历史角度来看，我们更注重于页面加载后的一些点：比如确保滚动的连贯性，以及动画的流畅性。
...
我们测试的页面是主登录页的摘要部分，这是数据量最大的页面，因此它也有最大的优化空间
...
代码体积从 526kb 压缩到只有 156kb，并且它对页面加载时间造成了巨大的变化。
...
虽然这并没有影响到实际的加载时间（我们任然需要调用哪些 API，并且在这上面耗时），但是用户可以直接拿到数据，所以感知速度的提升非常明显。

在内容已经出现的情况下，页面仍在继续变化，这是一种非常流行的技术，可以让页面更快的展现，并且当新的内容可用时，页面发生更新。可是我更喜欢立即呈现最终的 UI。
