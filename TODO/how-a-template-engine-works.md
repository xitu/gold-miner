...
我在工作经常使用各种模板引擎，因此，我想花点时间来研究下一个模板引擎的工作机制。

### 前言

简单的说，模板引擎就是一种可以帮助你自由的组织、控制一大堆文本数据的工具。一般而言，我们经常在一个 **web** 应用中利用模板引擎来生成 **HTML** 。在 **Python** 中，当你想使用模板引擎的时候，你会发现你有不少的选择，比如[jinja](http://jinja.pocoo.org/) 或者是[mako](http://www.makotemplates.org/)。从现在开始，我们将利用 [**tornado**](https://github.com/tornadoweb/tornado) 中的模板引擎来讲解模板引擎的工作原理，在 **tornado** 中，自带的模板引擎相对的简单，能方便我们去深入的剖析其原理。

在我们讲解原理之前，让我们先来看一个使用 **tornado** 模板引擎的例子：
...
在整个工作过程中，模板引擎将会分为如下两个部分对我们的字符串进行操作：

* 解析
* 渲染

在解析阶段，我们将我们准备好的字符串进行解析，然后格式化成可被渲染的格式，其可能是能被 `rendered.Consider` 所解析的字符串，解析器可能是一个语言的解释器或是一个语言的编译器。如果解析器是一种基于解释器的工具的话，在解析过程中将会生成一种特殊的数据结构来存放数据，然后渲染器会遍历整个数据结构来进行渲染。例如 **Django** 的模板引擎中的解析器就是一种基于解释器的工具。除此之外，解析器可能会生成一些可执行代码，渲染器将会执行这些代码，然后生成对应的结果。在 **Jinja2** ， **Mako** ，**Tornado** 中，模板引擎利用编译器来作为解析器。

### 编译

如同上面所说的一样，我们需要解析我们所编写的模板字符串，然后 **tornado** 中的模板解析器将会将我们所编写的模板字符串编译成可执行的 **Python** 代码。我们的解析工具由单个 **Python** 函数构成：
...
现在我们在 `_execute` 函数里处理我们的模板，在这个函数里，可以对在作用域里的有效变量进行处理。这个函数将创建一个 `string` 的列表，然后把它们连接起来并作为结果返回。`username` 的值将会被放在一个局部变量里，这是因为这看起来对于一个局部变量的操作的效率要高于对全局变量的操作。同时，我们对于其余代码的优化也在这个阶段完成，比如：
...
在 `{{ ... }}` 中的表达式将会被提取出来，然后添加进 `string` 列表中。在 `tornado` 模板模块中，在 `{{ ... }}` 所编写的表达式没有任何的限制，在最后，你所编写的表达式将会被转化成 `Python` 可执行代码。

### 让我们来看看具体的代码实现吧

让我们来看看模板引擎的具体实现吧。我们在 `Template` 类中编声明核心变量，当我们创建一个 `Template` 对象后，我们便可以编译我们所编写的模板字符串，随后我们便可以根据编译的结果来对其进行渲染。我们只需要对我们所编写的模板字符串进行一次编译，然后我们可以缓存我们的编译结果，下面是 `Template` 类的构造函数：
~~~Python
...
为了生成 **Python** 代码，我们需要去看看 `_CodeWriter` 这个类的源码，这个类用于管理我们所编写的表达式缩进，同时它也是一个 **Python** 上下文管理器：
...
我们在这里会遍历整个文件来查找我们所规定的特殊标记符号，在遍历完成后，我们将节点添加至列表中然后退出循环。
...
在 `exec` 函数中将会执行所给与全局命名空间内的编译后的代码对象，然后我们就可以调用 `_execute` 函数了。
