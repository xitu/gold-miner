...
接下来，你可以看看这三篇文章来了解如何通过 React 设置 Meteor 1.3 并用它来处理数据。第二篇会向你介绍容器组件，这是使用 Mantra 开发的一个主要方面的概念。
...
现在我们安装完毕样板项目，它完全包含了遵循 Mantra 规范的 Meteor 项目中所有你需要的核心文件和目录。

你可以通过以下命令 clone 项目安装
...
它完整的包含布局，路由系统以及具有注册，登录登出功能的用户系统。
...
在用户模块中，看看 containers 和 components 文件夹中的 NewUser 文件，在 container 文件夹中的如下所示：
...
对于这个日志程序我们准备使用 React-Bootstrap 。它可以很方便的使用 Bootstrap 来创建 React 应用。这样我们也可以方便的用我们喜爱的模块化来进行工作。
...
React-Bootstrap 并不依赖任何特定的 Bootstrap 库，所以只添加我们需要的就可以，现在让我们从 Twitter 添加官方的 Meteor 包。
...
这个表单十分简单，它仅仅负责显示自身并调用 create 方法，让我们来简单谈谈这里。

在我们的方法文件中，它们负责处理我们应用的逻辑，下面这一行
...
React-Boostrap 将会进行很简单的工作，我们只需要安装好项目，使用 import 方法引入每个我们想要引用的组件，就像其他类型一样渲染这些组件。

我们处理使用数据的方法则有一些不同，尽管它是组件，但不一定是我们实际需要处理的输入内容，我们需要使用特殊的 React-Bootstrap 方法 getValue() 来简单的获取我们需要的值。
...
首先，让我们再来看一下我们创建的应用结构。这里我们制造了一个简单的 Mantra 模块。我们通过这些目录文件来看看他们是怎么做到交互的。通过这些将会让你很好的理解如何使用 Meteor 1.3 和 Mantra 进行开发工作。

**索引**

Mantra 设置一个单入口索引。这个索引文件负责导入内容以及导出路由和方法，使我们导入模块变得可行。通过这种方式我们不用担心再单独导入每个文件。
...
方法文件夹负责我们应用的所有逻辑。你可以看到我们在这里创建了两个文件。首先是一个索引文件。这是一个类似目的模块的索引文件。我们向里面添加下面的内容
...
上面所做的就是导入条目文件，在条目文件中有我们的方法逻辑。这只是为了更容易的从其他文件导入我们的逻辑
...
当我们填写表格来创建一个新条目时，这些将是方法中会被执行的部分，我们将立刻再设置一个组件，首先让我们来获取服务器内容创建并为我们的条目来创建集合和方法
...
我们也要在 methods 文件夹中添加以下内容在 index.js 文件中
...
这个 {entry} 对象是我们将要获取的属性，我们的容器组件会通过他来包含我们所需要的数据
...
这里我们使用了更多的 React-Bootstrap 组件，你会留意到为了获取输入的值，我们用了一个特别的 getValue() 方法。这是因为我们正进行渲染的组件并不是实际输入字段，实际的输入区域是在渲染组件内的。所以我们需要使用这种功能来获取渲染组件。
...
这里你应该已经对 react-komposer 较为熟悉了，我们将用它来创建这一容器组件。它负责创建一个处理错误，调取合适方法的容器组件。在大多数情况下，获取数据并将它们通过属性传给UI组件。

depsMapper 通过react-komposer 中的 useDeps 方法检索方法及上下文内容并将它们传递给 UI 组件。

clearErrors 方法负责清除组件卸载时发生的任何错误的报错
...
现在我们将要创建 EntryList 容器的容器组件。这个稍许有些复杂，因为我们会实际上获取一些数据。
...
这也确实与其他容器组件较为相似，但一个重要的区别在于，我们会检查我们的入口集合，并将它们分配给一个变量。最终我们通过 onData 方法传递这个变量到UI组件中。

让我们通过 entries.js 文件在 publications 目录中设置发布。
...
我们需要在 server 目录中打开 main.js 文件，取消注释行并在发布中拉取方法，所以文件就像这样：
...
现在我们设置了我们的所有路由并且应用已经准备好运行，让我们切换目录到根目录并执行应用程序。

    meteor

你应当可以看到应用程序在Mantra的默认加载屏幕中启动了，让我们添加一个条目，这样我们应该可以在屏幕上看到效果了。

访问 localhost:3000/new-entry ，填写并提交表单来添加一个条目

然后访问根目录，你应该可以看到一个可以逐个查看链接的的条目列表。

希望这个简单的 Mantra 引导以及目前的 Meteor 1.3 beta 版本有助于让你更加了解如何运用他们来构建一个应用。
