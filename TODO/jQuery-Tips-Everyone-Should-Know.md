...
1. [自动修复失效图片](#自动修复失效图片)
1. [鼠标悬停切换 class](#鼠标悬停切换_class)
1. [禁用输入字段](#禁用输入字段)
1. [阻止链接加载](#阻止链接加载)
1. [缓存 jQuery 选择器](#缓存_jQuery_选择器)
1. [切换淡出 / 滑动](#切换淡出_/_滑动)
1. [简单的手风琴效果](#简单的手风琴效果)
1. [使两个 div 等高](#使两个_div_等高)
1. [在新标签页 / 新窗口打开外部链接](#在新标签页_/_新窗口打开外部链接)
1. [通过文本查找元素](#通过文本查找元素)
1. [在 visibility 属性变化时触发](#在_visibility_属性变化时触发)
1. [Ajax 调用错误处理](#Ajax_调用错误处理)
1. [链式插件调用](#链式插件调用)
