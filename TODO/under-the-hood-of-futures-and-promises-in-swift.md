...
正因为如此，许多解决方案都是为了解决上述问题而发明的 - 主要是围绕异步编程创建抽象，使其更易于理解和推理。对于大多数的解决方案来说，它们都是在"回调地狱"中提供帮助的，也就是当您有多个嵌套的闭包处理异步操作的不同部分时。

这周，让我们来看一个这样的解决方案 - *Futures & Promises* - 让我们打开"引擎盖"，看看它们是如何工作的。

## 一个对未来的承诺

但介绍 Futures & Promises 的概念时，大部分人第一个问题就是 *Feture 和 Promise 有什么区别？*。在我看来，最简单易懂的理解是这样的：

- **Promise** 是你对别人所作的承诺。
- 在 **Future** 中，你可能会选择兑现（解决）这个 promise，或者拒绝它。

如果我们使用上面的定义，Futures & Promises 变成了一枚硬币的正反面。一个承诺被构造，然后返回一个未来，在那里它可以被用来在稍后提取信息。
...
接下来，让我们看一下如何将多个 future 组合在一起，形成一条链 — 就像我们用来加载数据、将其解包并将实例保存到 UserLoader 中的数据库一样。

链接涉及到提供一个闭包，该闭包可以返回一个新值的 future。这将使我们能够从一个操作获得结果，将其传递给下一个操作，并从该操作返回一个新值。让我们来看一看：
...
使用上面的方法，我们现在可以给 *`Savable` 类型的 future* 添加一个扩张，来确保数据一旦可用时，能够轻松地保存到数据库。
...
虽然链接提供了一个强大的方式来有序地执行异步操作，但有时你只是想要对值进行简单的同步转换 - 为此，我们将添加对转换的支持。
...
现在，我们有了升级我们的 `UserLoader` 来 Futures & Promises 的所有部分。我将把操作分解为每一行，这样就更容易看到每一步发生了什么：
...
我的建议是，如果你在使用 Futures & Promises，那就是尽量让你的链条尽可能短和简单。记住，好的文档和可靠的单元测试可以帮助你避免很多麻烦和棘手的调试。
...
你也可以在 [GitHub](https://github.com/JohnSundell/SwiftBySundell/blob/master/Blog/Under-the-hood-of-Futures-and-Promises.swift) 上找到该篇文章对于的所有代码。

如果有问题，欢迎留言。我非常希望听到你的建议！👍你可以在下面留言，或者在 Twitter [@johnsundell](https://twitter.com/johnsundell) 联系我。

另外，检查新的 [Sundell 的 Swift 播客](https:swiftbysundell.compodcast)，我和来自社区的游客都会在上面回答你关于 Swift 开发的问题。
