...
React-Native 是一个混合的移动框架，可以让你使用 JavaScript 构建应用。然而，与其他混合移动开发技术不同的是，你构建的并不是一个 “移动网页应用”（把网页应用封装到一个原生的容器里）。在最后会得到真正的东西。你的 JavaScript 代码库在被编译为一个移动应用，与使用 OC 构建的 iOS 应用和 Java 构建的 Android 应用没有分别。这意味着 React-Native 永有了原生应用和混合应用的好处，而没有任何缺点。

我的目标是找出他们是否能够准确地履行他们的承诺。要实现目标的话，我就需要用 Swift 和 React-Native 构建相同的应用。它需要足够简单，以便我可以学习两种语言并及时完成应用程序，但也需要足够的复杂，才能比较每个应用的 CPU、GPU、内存的使用情况和功耗。应用会有四个 tab。第一个叫做 “Profile”，用来提示用户登录 Facebook 来获得用户个人资料里的照片和邮箱，并展示在页面上。第二个 tab 叫做 “To Do List”，是用 NSUserDefaults 来做的一个简单的待办事项表。第三个 tab 叫做 “Page Viewer”，由一个 PageViewController 组成。PageViewController 有三屏，用户可以来回切换（红、绿、蓝三屏）。最后一个 tab 叫 “Maps”，由一个 MapView 组成，放大用户的当前位置，然后在地图上的用蓝点表示。

### Swift 的历程 ###

第一步是 iOS 和 Swift。学习 Swift 相对比较容易，因为它很像我知道的其他语言（Java、C艹）。然而，学习 Cocoa Touch（iOS 框架）才是更难的任务。我看了 **Udemy.com** 上 Rob Percival 的一系列视频，这让我从初识 Swift 阶段过渡到完成了几个应用。虽然我在看完介绍视频后还是在 Cocoa Touch 上有很多问题。视频里大多数的“学习”只是调用复制/粘贴代码，但是我们不是很清楚它做了什么。我感觉可能老师也不知道这是啥，只是记住了它。我不喜欢对我的代码一无所知。

Apple 的 IDE（Xcode）无疑是一个很先进的并且对用友好的。你可以点击叫做 Storyboard 的东西，按你想要的顺序来设置你应用的屏幕，在程序的首屏上放一个箭头。在第一个 tab（“Profile”）里，我要拖一个图片视图、姓名标签和邮箱标签。然后，我拖住它跟代码做一个连接，在代码里撞见一个新变量。再然后，以编程的方式，一旦用户登录了 Facebook，我就把变量的值改变成 Facebook 里的值。这些我通过视频和适应 Swift/iOS 的代码，花了三周的时间完成。

你可以在 Github 上看一下这个应用的 Swift 版本的代码，连接在这里：[https://github.com/jcalderaio/swift-honors-app](https://github.com/jcalderaio/swift-honors-app)
...
第二部是 React-Native。学习 JavaScript 比 Swift 要难上一点，但也不是很困难。我试着利用我从网上学到的一些松散的 React-Native 知识来编写应用，但是还不够。我需要一些视频讲座。回到 ** Udemy.com**，我看了 Stephen Grider 介绍 React-Native 的精彩演讲。起初，我感到特别艰难，React-Native 的结构对我一点用也没有。不过在看了 Stephen Grider 的演讲之后的一周，我已经可以自己编码了。

我对 React-Native 感到真正喜欢的地方是，你写的每一行代码都很说得通，你知道每一行代码的作用。另外，不像在 iOS 里（需要调整每个页面，让他们在横屏或者竖屏时显示正确的尺寸），在 React-Native 里，所有的都调整好了。不需要任何设置，我就能让我的应用看上去很完美。我在一些不同尺寸的 iphone 上运行我的程序也跑得很好。因为 React-Native 使用的是 flexbox（有点像 HTML 中的 CSS），它能敏感的响应当前屏幕的尺寸。
...
每个应用有 4 个 tab，每个 tab 都有一个“任务”，我在每个类别里测试。首先是 “Profile”，它的功能是登陆 Facebook。在代码里的表现形式是请求 Facebook 服务器，返回个人信息图片、邮箱以及姓名。第二个（“To Do List”）任务是从列表里添加或删除一个“代办项”。第三个（“Page View”）任务是在三个页面间来回滑动。第四个(“Maps”)任务是点击 tab 后，代码会让 GPS 来放大我当前的位置，然后用一个蓝色的辐射信号来表示我。
...
***Profile:*** React-Native 在这里略胜一筹，它比 Swift 更有效的利用了 1.86% 的 CPU。在执行任务和录制测试的过程中，CPU 用量被监测的精确时间是：当我按下 “Log in with Facebook” 按钮的时候。
...
***Profile:*** Swift 以比 React Native 高出 1.7 帧/秒的帧率，微弱赢得了这个 tab 的胜利。执行任务并记录数据峰值的准确时间是：在我按下 “Log in with Facebook” 按钮的时候。

***To Do List:*** React-Native 以比 Swift 高出 6.25 帧/秒的帧率赢得了这个类别的胜利。执行任务并记录数据峰值的准确时间是：在我向列表**添加完（added）**一项以及**删除完（deleted）**一项的时候。

***Page View:*** Swift 在这个 tab 上以 3.6 帧/秒的帧率击败了 React-Native。在执行任务并记录测量数据的时候，我观察到，如果我快速滑动两个页面，帧率会急升 50。如果我停留在一个页面，那帧率会下降，但是如果我重新再页面之间滑动，帧数又会急升。
...
Swift 和 React-Native 再一次的各自赢得了两个 tab 的胜利。但是，React-Native 以 0.95 帧/秒在整体上胜出。这真让人吃惊，Facebook 到底从 React-Native 的代码里榨出多少果汁？因为至今为止，它看上去似乎坚持的抵制着原生的 iOS（Swift）应用。

### 内存测试 ###

我们要绘制的第三个数据表是内存的使用情况。我将为 Swift 和 React Native 的项目中的每个 tab 执行一个任务并记下测量结果。Y 轴（内存）的高度是我测量数据的最高值。CPU 的使用率采集间隔是 1 毫秒。在每毫秒，我执行每个 tab 的任务的时候，一个测量就会被 “Allocations” 工具记录下来。我会取这些数据的平均值，然后绘制成下面的图表。
...
我并没有考虑原生的 Android 应用。iOS 是我优先选择的平台，所以这是我最关心的。尽管，我也会尽快的在 Android 上完成同样的实验。我很好奇结果会是什么，但是我敢打赌，如果 React-Native 能比原生的 iOS 性能好，那它也一定比原生的 Android 的性能要好。
