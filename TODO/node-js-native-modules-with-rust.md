...
> 梗概 - 使用 Rust 代替 C++ 开发原生 Node.js 模块！

[RisingStack](https://risingstack.com/) 去年面临一件棘手的事：我们已经尽可能让 Node.js 发挥出最高的性能，然而我们的服务器开销还是达到的最高限度。为了提高我们应用的性能（并且降低成本），我们决定彻底重写它，并将系统迁移到其他的基础设施上 - 这其中有许多工作，本文不展开谈了。

后来我发现，**我们只要写一个原生模块就行了！**

那时候我们还没意识到有更好的方法来解决我们的性能问题。就在几周前，我发现有另外一个方案可行 **那就是使用 Rust 来实现一个原生模块，而不是 C++。** 得益于它的安全性以及易用性，我意识到这是一个很好的选择。

> 在这个Rust教程中，我将手把手教你写一个先进、快速、安全的原生模块。

## 我们 Node.js 服务器的性能问题

我们的问题在 2016 年末的时候暴露出来，那时我们使用 Node.js 监视追踪产品，这个产品已于 2017 年十月合并到 [Keymetrics](https://blog.risingstack.com/trace-becomes-keymetrics-by-october-31/)。

像那时的其他科技创业公司一样，我们将服务部署到 [Heroku](https://www.heroku.com/) 上以节省一些基础设施和运维的成本。我们一直在构建微服务架构应用程序，这意味着我们的很多服务都是通过 HTTP(S) 进行通信的。

**棘手的问题来了：** 我们想让各服务之间进行安全的通信，但是 Heroku 不支持私有网络，所以我们不得不实现一个自己的方案。因此，我们查阅了一些安全认证方案，最终选定了 HTTP 签名。

> 简要的解释一下：HTTP 签名基于非对称密码体系。为了创建一个 HTTP 签名，你需要获取一个请求的所有部分：URL、请求头、请求体，使用你的私钥对其签名。然后，你可以把你的公钥给接收方，让接收方去验证他们。

随时间流逝，我们发现在大多数 HTTP 服务器进程中，CPU 使用率已经达到了极限。显然，一个原因引起我们怀疑 - 如果你想加密，那就会发生这样的问题。

然而，在对 [v8-profiler](https://github.com/node-inspector/v8-profiler) 进行了严格分析之后，我们发现问题不是由加密计算引起的！是 [URL 解析](https://node.js.org/docs/latest/api/url.html)使用的大部分 CPU 时间。为什么？因为想要认证，就必须解析 URL 来验证请求签名。

为了解决这个问题，我们决定放弃 Heroku（这其中也有其他因素），我们创建了一个包含 Kubernetes 和内部网络的 Google 云基础设施，这样我们就不必优化 URL 解析器。

是什么原因促使我写这个故事（教程）呢？就在几周前，我意识到我们可以用另一种方法优化 URL 解析器 —— 使用 Rust 写一个原生库。

## 编写原生模块 - 需要一个 Rust 模块

**编写本机代码应该不那么难，对吧？**

在 RisingStack，我们奉行工欲善其事，必先利其器的宗旨。我们经常对更好的软件构件方式做调查，在必要的时候，也使用C++ 来编写原生模块。

> 恬不知耻的说一句：我也在博客上写了我的学习历程 [遨游于本地 Node.js 模块](https://blog.risingstack.com/writing-native-node-js-modules/)。去看一看！
...
我确信我不是唯一一个想要解析 URL 的人。浏览器可能已经解决了这个问题，所以我查看了 Chromium 的解决方案：[谷歌链接](https://src.chromium.org/viewvc/chrome/trunk/src/url/)。尽管通过 N-API 可以轻易的从 Node.js 调用这个实现，但是有几个原因让我不这样做：

*   **更新：** 当我只是从互联网上复制粘贴代码的时候，我会立即感到不安。长久以来，人们一直这样做，但是总有许多原因使它们不能很好的工作。没有什么好的方法去更新代码库中的大段代码。
*   **安全性：** 一个没有丰富 C++ 编程经验的人是无法验证代码是否正确的，但是我们又不得不将它运行在我们服务器上。C++ 学习曲线过于陡峭，人们需要花费很长时间掌握它。
*   **私密性：** 我们都听说过可用的 C++ 代码是存在的，然而我宁愿避免他，因为我没办法独自审计它。使用维护良好的开源模块能给我足够的信心，我不必担心它的私密性。

**所以我倾向于一个拥有更易用的更新机制和现在化工具的语言：Rust！**

## 关于 Rust 简单说两句

Rust 允许我们编写快速有效的代码。

所有的 Rust 工程由 `cargo` 管理 —— 就是 Rust 界的 `npm`。`cargo` 可以安装工程依赖，你所需的所有依赖都注册在依赖列表中。
...
我们已经有了一个应该会工作的库（rust-url），所以我们来尝试编译它！

### 如何编译一个 Rust 应用？

按照 [https://rustup.rs](https://rustup.rs)的指示，我们可以用 `rustc` 编译器，但是我们现在更应该关心的是 `cargo`。我不想深入叙述它是如何工作的，如果你感兴趣，请移步至我们[以前的 Rust 博文](https://blog.risingstack.com/how-to-use-rust-with-node-when-performance-matters/)。
...
为了从 Node.js 中调用 Rust，我们可以使用 Rust 提供的 FFI。FFI 是外部函数接口（Foreign Function Interface）的缩写。外部函数接口（FFI）是一种由某种语言编写的程序调用另一种语言的例程或服务的实现机制。
...
我们还有为工程添加 `libc`依赖，`libc` 是遵从 ANSI C 标准的 C 语言标准库。

`libc` crate 是一个用作把原生类型和函数绑定到常见各种系统的库，包括 libc。这允许我们在 Rust 代码中使用 C 语言类型，我们想在 Rust 函数中接收或返回任何 C 类型数据，我们都必须使用它。
...
我们需要把类型转换标记为 `unsafe`。被标记了 `unsafe` 关键字的代码块可以访问非安全的函数或者引用在安全函数中的裸指针（raw pointer）。

Rust 使用 `Option<T>` 类型来表示一个可为空的值。就像 JavaScript 中一个值可以为 `null` 或者 `undefined` 一样。每当你访问可空值的时候，你应该做显式的检查。在Rust中，有几种方式可以访问它，但是在这里，我将使用最简单的方式：如果值为空，则将会抛出一个错误（panic in Rust terms）[`unwrap`](https://doc.rust-lang.org/std/option/enum.Option.html#method.unwrap)。
...
要编译这些 Rust 代码，你可以使用 `cargo build --release` 命令。在编译之前，一定要确认你把 `url` 库添加到本项目 `Cargo.toml` 的依赖中了！
...
美滋滋：我们已经编写了一个可以被 Node.js 调用的 Rust 代码！它能工作，但是你应该已经发现了，我们不得不做一大堆类型转换，这将增加我们函数的调用开销。一定有更好的办法将我们的代码与 JavaScript 做整合。
...
执行 `neon new <projectname>` 将会创建一个新的零配置 Neon 工程。
...
上述代码中，新类型 `JsString`、`Call` 和 `JsResult` 是对 JavaScript 类型的封装，这让我们就可以接入 JavaScript VM ，执行上面的代码。`Scope` 将我们的新变量绑定到当前的 JavaScript 域中，这让我们的变量就可以被垃圾收集器管理。

这与我之前写的博文 [使用 C++ 编写原生 Node.js 模块](https://blog.risingstack.com/writing-native-node-js-modules/) 非常类似。
...
我们得接受所有类型的参数（就像一个 JavaScript 函数一样），所以我们没办法确定参数的数量，这就是我们必须要检查第一个元素是否存在的原因。
...
这只是一个简单的例子，当然，我们还有许多东西要学习，

在未来，我们可以进一步优化计算，尽可能的利用并发计算提高性能，一些类似 [`rayon`](https://crates.io/crates/rayon) 的 crates 提供给我们类似的功能。

## 使用 Rust 编写 Node.js 模块

希望你今天跟我学到了使用 Rust 编写 Node.js 模块的方法，从此你扩充了自己的工具链。我想说的是，没有能解决所有问题的银弹，只有各种方法的综合运用，才是解决问题之道。

**要记住，在某些场景下，Rust 可能是很便利的解决方案**
