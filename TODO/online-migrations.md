...
要在一个实时的数据库上回填一个新表其中最重要的是简单找到所有需要迁移的对象。通过查询数据库来查找所有对象将需要在生产数据库执行大量查询，这将需要很多时间。幸运的是，我们可以将这个过程分流成对我们生产数据库没影响的离线过程。我们为我们的 Hadoop 集群提供了我们的数据库快照,这使我们能够使用 [MapReduce](https://en.wikipedia.org/wiki/MapReduce) 通过离线，分布式的方式快速处理我们的数据。
