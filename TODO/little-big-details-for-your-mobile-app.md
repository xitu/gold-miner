...
通过本文你可以看到这些 _小中见大的细节_ 和设计中那些更明显的元素同样重要，以及它们如何决定你的 app 的成功。

## 启动页

当用户打开你的 app 时，最不要做的事情就是让他们等。但是如果 app 的初始设置非常耗时，又 _不可能_ 优化该怎么办？你 _不得不_ 让用户等。如果他们愿意等，你得知道如何 _吸引他们_。启动页解决了等待的问题，让你有一个简洁有力的窗口来吸引用户。
...
*   [Google](https://developer.android.com/training/articles/perf-anr.html) 和 [Apple](https://developer.apple.com/ios/human-interface-guidelines/graphics/launch-screen/) 都建议用启动页 _模拟更快的加载_ 来提高用户体验。启动页给到用户即时反馈，表示 app 已经启动并正在加载。 为了保证人们等待的时候不厌倦，给他们一些 _娱乐_：有意思的，意想不到的，或者任何可以抓住用户的注意的东西，时间长到可以够你的 app 启动就好。

    ![](http://babich.biz/content/images/2016/08/1-88tQ_gtQrWY7LQXUMglNzg.gif)

    图片来源: Cuberto

*   如果你的 app 的初始设置超过 10 秒钟，考虑使用 [进度条](http://babich.biz/progress-indicators/) 来表示正在加载。_记得不确定的等待长于确定的等待_。所以，你要给用户一个清晰的标识，他们需要等多长时间。

    ![](http://babich.biz/content/images/2016/08/1-Qq7rzaTpyd2OndF3zgyZtA.png)

    用进度条使得加载过程更自然. 图片来源: de_martin

## 空状态

我们通常会设计一个弹出界面，布局中的所有元素都完美的放置，看上去很美。但是如果界面正在等待用户操作，该怎么设计？我要说的就是空状态。设计空状态是非常重要的，因为即使它是一个临时状态，它也会是 _你的 app 中的一份子_，并且对你的用户 _有用_。

空状态的意义不仅是一个装饰。除了向用户提示界面上期望看到的内容，它还可以作为一种 _导引_ （介绍 app，展示为用户做的事情），或者 _助手_ （出错时的屏幕）。这两周情况下，你都希望用户尽快做点事情，让界面不要空着。
...
*   _给新手用户设计空状态_。记住新用户的体验很 _重要_。给他们设计空状态的时候要尽量简单。瞄准用户的主要目标，设计最大的交互：清楚的信息，合适的图像，一个按钮，这就够了。
...
_用心设计_。你的 app 的 UI 里面，每个微小的细节都值得密切注意，因为 UX 就是所有细节和谐工作的总和。所以，请从一而终，持之以恒的打磨你的 UI，创造真正无与伦比的用户体验。
