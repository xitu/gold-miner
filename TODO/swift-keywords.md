...
有句话以前说过，现在我要再次提一下，一个优秀的匠人，他（她）的工具同样优秀。我们默守陈规的使用着这些工具，它们就会带我们到想去的地方，或者完成我们的梦寐以求的作品。
...
{
    typealias MediaType = String // 可以是任何需要的类型
}
```
...
**enum**：为一组相关值定义通用类型，并使你能够在代码中以类型安全的方式使用这些值。在Swift中，它们属于第一类类型，并且可以使用一些特性，这些特性在其他语言里往往只有类才支持。
...
```
// 现在 potentialError 被初始化了，不在是 nil 了
```
...
**operator**：一个用来检查、联合或更改者值的特殊符号或短语。
...
```
public var foo:String? // 在程序的任何地方都可以被覆盖或重写，但是外部不行。
```

**static**：定义了类自己的调用方法。也用于定义静态成员。
...
**rethrows**：表示只有在函数的参数抛出一个错误时，函数才抛出一个错误。
...
**try**：表示接下累的函数可能会抛出一个错误。有三种不同的用法：try、try? 和 try!。
...
**get**：返回成员给定的值。也用于计算属性，可以间接的获取其他属性和值。
...
**set**：获取成员的值来作为它的新值。也可用于计算属性，间接的设置其他属性和值。如果一个计算属性的 setter 没有定义一个名字来代表要设置的新值，那么默认新值的名字为 `newValue`。 
