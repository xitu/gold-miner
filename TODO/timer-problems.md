...
> **注意**：本文的代码会使用Swift 3版本的 Dispatch API 演示单次计时器。但许多共通原则适用于其他各种周期计时器和异步计时器API。
...
重设了的计时器是指我们需要延长计时器的到期时间。例如等待计时器（sleep或者timeout计时器）。对于等待计时器，每个新的 activity 都会把它重置到完整的时长。
