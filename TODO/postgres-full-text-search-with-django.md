...
Django 在 1.10 版本已经增加了对 Postgres 内建全文检索的支持。当我们想要增加 django 检索能力又不想去建立和维护其它服务时，相较于其它更重型的像 [elasticsearch](https://www.elastic.co/products/elasticsearch)  或者  [SOLR](http://lucene.apache.org/solr/) 搜索系统， Posgres 会是一个很好的选择。

在这个简明攻略中，我将会展示如何为 Django 应用添加全文检索功能。在 Django [文档](https://docs.djangoproject.com/en/1.11/ref/contrib/postgres/search/) 中简单的使用案例包含的已经很全面了，所以我会直接关注于更加进阶的例子，这些例子将允许在不同的字段间查询，包含字段间对应关系的数据，为不同的字段打分，添加索引来加快查询速度，已经保证查询结果是实时的方法。
...
首先第一步是为我们的帖子创建*文档*。每一份文档在逻辑上都将代表一个帖子，包括
...
这包括了我们每个帖子的所有数据，字段数据间通过空格来分割。

## 查询向量 ##

那么现在我们已经有了我们的文档，我们需要把他们转换成 Postgres 可以索引和查询的格式。 Postgres 把这种形式叫做 [向量](https://www.postgresql.org/docs/9.6/static/textsearch-controls.html#TEXTSEARCH-PARSING-DOCUMENTS)。Django 提供了一个该功能的封装类叫做 [查询向量]。一个`查询向量`也可以增加关联性比重，接下来我们会重现查询语句来创建向量。
...
每个文档都被统一到一组常用的*词根*。其中包括所有字母都切换到小写，去除通用的前缀和后缀（比如像英语中的 's' 和 'es'），并且移除掉像 'a'，'an' 和 'the' 这样的通用词汇。这个数据表示可以在文档的哪里找到这个词根，后面的字母表示这个词根的比重。如果我们想要覆盖 Postgres 处理这些词汇的配置，比如说使用不同的语言，我们需要向查询向量传递一个额外的参数 `config`。如果没有声明这个配置， Postgres 将会使用数据库默认的配置，这样很可能基于 locale 配置。
...
在默认情况下，django 将会使用 Postgres 的 `plainto_tsquery()`[function](https://www.postgresql.org/docs/9.6/static/textsearch-controls.html#TEXTSEARCH-PARSING-QUERIES) 来解析这个查询。这种方式的缺点在于它将会搜索与所有单词都匹配的文档。所以，我们可以传递一个 [SearchQuery()](https://docs.djangoproject.com/en/1.11/ref/contrib/postgres/search/#searchquery) 的实例而不是字符串，这样查询条件就可以使用布尔操作符组合起来了。
...
如果我们在 SearchVector() 中使用自定义的 `config`，我们就应该使用和 SearchQuery() 中 相同的 `config`。
...
Django 并不允许我们使用带有 update 子句的函数，但是 Postgres 允许，所以如果我们真的想那么做的话，我们可以调用一次像这样的查询来一次性更新所有文档。
...
现在我们的文档是储存在一个字段中的，我们可以创建一个 GIN 索引来加快查询速度。在 Django1.11 中，这简单的只需要为我们的模型添加一个 `index` Meta 选项，然后创建并执行 migrate 。
...
对于其它有着缓慢更新流程的应用，每次数据改变都更新文档是更合适的。这样做的优点是查询的数据将是实时的。这样做的缺点是每次更新都会计算 search_vector 从而增加了额外的开销。
...
现在我们更新我们的 Post 模型并且更新我们的自定义模型和自定义 save 方法。这里的想法时将数据保存到数据库，然后执行一个 SELECT 查询来将所有的数据连接到一起，之后在创建新的 search_vector。所以每次保存都会导致一次 UPATE，SELET 和另一个 UPDATE 的操作。
...
另外，更新 authors 和 tags 并不会触发这个 `save()`，所以我们也为这些添加信号来强制执行 post 模型的 `save()`，从而更新 search_vector 字段。
...
现在所有对 Post，Author 或增删改 tags 的操作都会触发查询数据的更新。如果一个 tag 被重命名了，那么我们不会在没有创建另一个信号处理程序的情况下接收它。（这句不是很理解，就直译了。。接受改为保存？）
...
现在我们已经有了一个运行中的应用了，该应用使用了 Postgres 的全文搜索，一旦它运行起来，大部分就不需要你管了。相较于搭一个  [elasticsearch](https://www.elastic.co/products/elasticsearch) 或者 [SOLR](http://lucene.apache.org/solr/) (even with [Haystack](http://haystacksearch.org/))。这简直是一股清流，而且这结果对于大多数应用来熟已经足够了。
