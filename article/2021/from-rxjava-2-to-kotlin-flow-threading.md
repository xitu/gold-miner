...
Kotlin Flow 真的不错，可以与 RxJava Observable 相比。它们具有相似的使用，相似的运算符，并且都在其链内处理线程。他们在使用 `Observable.just` 或 `flowOf` 时也有类似的技巧。但是就并发性和并行性而言，Kotlin Flow 似乎更简单。同样，Kotlin Flow 也没有诸如带有 `subscribeOn` 的 RxJava 这样的问题，因为在流程中，我们用作用域声明了链的末尾，从技术上讲，不可能将它们放多个。

在处理线程的方法上，Kotlin Flow 和 RxJava 有相反的概念：在 RxJava 中，我们认为是从上到下，而在 Kotlin Flow 中则是从下到上。但是无论如何，如果需要迁移代码，反之亦然，而不会破坏很多功能。
