...
这种演变大多是有目的，特别是在最新的迭代中，开发者社区已经设法影响了其中的一些变化，使 JavaScript 成为一种非常灵活和有用的语言。

然而，在所有这些年的演变过程中，可以说有一些过时功能的残余，这些功能还没有被拿掉，但确实没有任何作用（或者在这方面不是很有效）。

以下是 JavaScript 的三个功能，即使它们在运行时仍然可用，仍然要避免去使用。

## void 操作符

你可能在某一时刻看到过 void 操作符的存在。在过去，每当你点击一个链接时，将启动一个 JavaScript 函数，你将添加 `href="javascript:void(0)"` 以确保默认行为不会启动。

但这究竟是什么意思呢？

`void` 操作符是一种在 JavaScript 中生成 `undefined` 值的方法。这是正确的，它接受任何一个表达式，并且每次都返回 `undefined`。

我知道你在想法：为什么不直接使用已经存在的 `undefined` 关键字呢？在 ECMAScript 5 之前，`undefined` 关键字并不是一个常量值。你可以定义 `undefined`，如果你再想一想，这不就是我们曾经想做的事情吗？

当然，这样做是没有意义的，这就是为什么最终它被重新定义为一个常量值，而改变它不再是一个选项。然而，因为你在过去可以改变它，所以 `void` 会允许你访问的 `undefined`，即使这个常量不再起作用。

事实上，一个很好的方法是通过创建你自己的 IIFEs 来重新定义只属于你的命名空间的常量，避免与第三方库的任何问题，其中一个参数确实是 `undefined`。

```JavaScript
(function (window, undefined) {
  // 你的逻辑是可以把 undefined 当作预期
})(window, void(0))
```

当然，今天的 `void` 操作符仍然有它的用途，但它是非必要的。在现在的 JavaScript 中，最好的用例是帮助避免单行箭头函数的非预期返回。

你可能知道，一个单行箭头函数将返回该行的结果，即使你没有特别使用 `return` 语句。
...
这两个函数都会返回一些东西。显然，对于 `double` 函数来说，这是有意为之，但另一个可能不是，你可能只是想调用这个函数，但你对其结果值不感兴趣。在那里你可以这样做。

```JavaScript
const callAfunction = () => void myFunction(); // 返回 myFunction 所返回的结果，即使我不想这样做
```

而这将立即掩盖返回值，并确保你的调用只返回 `undefined`。
...
`with` 语句允许扩展给定语句的范围链。换句话说，它允许你将一个表达式注入给定语句的作用域，最好是简化该语句。
...
你不情愿地覆盖了函数的第二个参数，给对象添加了一个相同命名的属性。我想补充的是，这完全是正常的，因为人们永远不会期望两者处于同一个作用域级别。然而，多亏了 `with`，我们把这两个作用域都混在了一起，结果并不理想。

这都是为了说明要避免使用 `with`，虽然它可能看起来是节省一些按键的好方法，但你很快会创造出非常复杂的代码，，在心理上解析它可能是对别人（或你，两周后的未来）的一个挑战。

## Labels 标签

如果你够老（像我一样），你就经历过其他语言（如 C 语言）中对 `go-to` 语句的憎恨。那很糟糕，那是一个在当年很有意义的功能，但最终随着同一问题的更新的解决方案，变得如此过时和糟糕，变成了一种反模式。
...
当然，这个例子看起来非常像一个 `if..else` 语句。而且你完全可以说，它看起来并不那么糟糕。然而，你打破了代码的正常流程，跳过了语句。如果你的目的是这样做，那么 `if..else` 就更容易被别人在心理上解析了。
...
你能在头脑中解析上述代码并告诉我具体的输出结果吗？这并非不可能，但要花点时间。上面的脚本会打印：
...
`labels` 标签可能是棘手的小家伙，即使你让它们工作，从解释器的角度来看，它们也是很有意义的，但你应该为人类而不是为机器写代码。别人会来读它（甚至是三周后的你），当他们看到标签的那一刻，他们会永远恨你。当然，他们会花更多的时间来理解你代码的基本流程，但这是目前的次要问题。

## 本文总结

我喜欢 JavaScript，不要误会我的意思，自从 `18` 年前我开始从事网络开发工作以来，我一直在以不同的方式与它互动。我见证了这门语言的发展，就像一坛好酒，随着时间的推移而变得更好。然而，如果我说这门语言中没有一些我不喜欢的黑暗角落，那是假的。而这三个功能恰好表明了这一点。

好消息是，在我多年的经验中，我还没有看到 `with` 或 · 被实施并部署到生产中。这并不是说没有这样的情况，但我从未见过这样的情况，这让我觉得没有多少代码审查会让它们通过。
