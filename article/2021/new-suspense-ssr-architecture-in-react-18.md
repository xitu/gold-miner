...
React 18 将包括对 React 服务器端渲染（SSR）性能的架构改进。这些改进是实质性的，是几年来工作的结晶。这些改进大多是在幕后进行的，但有一些选择机制你需要注意，特别是如果你*不使用*框架的话。

主要的新 API 是  `pipeToNodeWritable`，你可以在 [Upgrading to React 18 on the Server](https://github.com/reactwg/react-18/discussions/22) 中了解到。我们计划写更多关于它的细节，它不是最终版本，因为还有一些事情需要解决。
...
- 然后在客户端将 JavaScript 逻辑绑定到服务器生成的整个应用程序的 HTML上（这个过程叫 “hydration”）。

这几个步骤的关键部分是，每一步都必须在下一步开始之前一次性完成整个应用程序。这样做效率不高，如果你的应用程序的某些部分比其他部分慢，就像几乎所有具有一定规模的应用面临的问题一样。

React 18 让你使用  `<Suspense>`  来将你的应用程序分解成较小的独立单元，这些单元将独立完成这些步骤，不会阻碍应用程序的其他部分。因此，你的应用程序的用户将更快地看到内容，并能更快地开始互动。你的应用程序中最慢的部分不会拖累那些快速的部分。这些优化是自动的，你不需要写任何特殊的协调代码来完成这个功能。
...
这里，屏幕中灰色部分还没有完全可交互。你的应用程序的 JavaScript 代码还没有加载完成，所以点击按钮是没有任何响应的。但特别是对于内容繁杂的网站，SSR 非常有用，因为它可以让网络连接较差的用户在 JavaScript 加载时开始阅读或查看内容。
