...
跨站请求伪造，也被称为 CSRF、Sea Surf、XSRF。它是一种攻击方式，攻击者利用它诱骗受害者代表他们自己执行操作。受害者持有的权限级别决定了 CSRF 攻击的影响范围。这种攻击利用了一个事实：一旦用户通过网站服务的身份认证，网站就完全信任该用户（译者注：身份认证指 cookie）。
...
发起跨站请求伪造攻击有两个主要部分。第一部分是诱骗受害者点击某个链接或者加载某个页面。这点一般通过社会工程学或恶意链接实现。第二部分是从受害者的浏览器向网站服务器发送一个精心设计的合法的请求。该请求携带了攻击者精心挑选的值和受害者在目标网站上的所有 cookie。因此，目标网站知道受害者可以在该网站上执行哪些行为。任何携带 HTTP 认证或 cookie 的请求都是合法的，即使这些请求是攻击者发起的。
