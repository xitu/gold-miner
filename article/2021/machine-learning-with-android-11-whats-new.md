...
如你所见，我们在本文中主要关注设备上的 ML。Android 11 对设备上的 ML 做出了许多很酷的更新，但让我们简单地谈一谈为什么我们要对此加以关注。我们还将了解为什么有这么多人宣传设备上的 ML 或终端 ML。
...
![设备上 ML 这个想法的背后](https://cdn-images-1.medium.com/max/3836/1*O1a_Su6P-XggXk9IXTSzMQ.jpeg)

你不会直接将模型用于你的设备，而需要在导入前先对你的模型进行压缩或优化，以便能够在设备上正常的运行它 —— 因为终端设备的计算能力、网络可用性和磁盘空间有限。但是，在本文中，我们将跳过优化过程，直接部署 `.tflite` 模型文件。而你可以进一步了解 [TensorFlow Lite](https://www.tensorflow.org/lite/)和[模型优化过程](https://www.tensorflow.org/lite/performance/model_optimization)。
...
我所能够想到的第一件事就是节能：你本来是需要花费大量的能量，将你的视频数据连续发送或流式传输到服务器。但有时这样做是不可行的 —— 没有数据网络的时候。值得一提的是，如果你对你的数据先做大量的预处理，倒也能节省些能耗。
