...
译者注：React 的函数组件是 React 组件的另一种定义方式，两种方式都可以用于定义组件，但是相比于类组件，函数组件要更简单好用些。函数组件的创建方式就是定义一个函数，这个函数 return React组件。因此，返回一个 React 元素的函数就是组件。<sup>[参考内容](https://www.cnblogs.com/lovevin/p/13231718.html)</sup>
