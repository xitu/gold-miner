...
的时事通讯和社交媒体帐户添加了宣传渠道，以扩大在不参加今年会议的人群中的影响力。
...
在去年的调查中，我们试图了解构成组织设计系统的组件。去年设计系统中最流行的组件包括组件库、样式指南、设计指南和内容指南。

今年，我们将问题扩展到包括9个选项，添加了诸如设计令牌、图标库和可访问性准则之类的内容。
...
2. UI套件（83％）
3. 风格指南（75％）
4. 大多数受访者（74％）还报告说他们的设计系统具有组件代码库。
...
这里有两个值得考虑的关键信息可以获取 —— 首先，有 75％ 的调查受访者是专注于产品级别或设计系统级别的设计工作的。

其次，记录组织的视觉语言是设计系统开发的第一步。组织的视觉语言在样式指南和图标中表示，并嵌入到产品设计阶段使用的 UI 套件中。视觉设计工件（例如样式指南和图标）的百分比很高，与早期设计系统的发展阶段保持一致。
...
首先，"贵公司是否在内部建立了自己的设计系统？" 而且如果可以回答，我们询问了他们"您的公司为什么选择内部构建设计系统而不是重新使用开源设计系统？"
