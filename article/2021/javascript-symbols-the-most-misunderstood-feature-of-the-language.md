...
如果你向一个开发者问：**”Symbol 有什么独特的特性？”**，他也许会回答：**“嗯，他是独一无二的”**。

我知道，这是一个糟糕的回答，但请和我一起来看看，可以吗？Symbol 是 JavaScript 奇怪语法之一，因为很少有人真正理解它，也不去使用它。

但是 Symbol 的存在是有原因的，你觉得呢？就像 `with` 一样有它存在的价值 —— 不管你是否喜欢它 ——，Symbol 也有它存在的价值。我们在这里来分析一下。
...
另一个有趣的点是，在 JavaScript 中原始值是不可变更的。这意味着你永远不能修改数值 `2`。但实际上，你可以为变量重新赋值为其他（不可变更的）原始值。

同样作为原始值的 Symbol 也有以上的特征：

* Symbol 是一种原始值，可以直接赋值给变量。
* Symbol 是不可变更的，不管你如何尝试，都无法修改它。

它还有一个额外的特征：Symbol 在全局是唯一的。

前两个特征对于所有原始类型来说都是常见的，但第三个特征却不是，这就是它们如此特别的原因。
...
以上结果就表明 Symbol 独特的特征：每次创建的 Symbol 都是一个新的值。即使你给他们传递了相同的名称。

如果想调试一下，你可以为 `Symbol` 构造函数提供一个属性，一个字符串。除了调试之外，该值没有任何用处。
...
上一个例子里，在资源消耗上有多昂贵？会有多少内存来抱怨说我们被用来存储相同的字符串？太少了，我甚至不愿去关注它。
...
Symbol 的另一个有趣的用途来自它们是唯一的这样一个特征。它们可用于为对象提供自定义的、唯一的“hooks”。就像你可以在自定义对象上添加的 `toString` 钩子一样，它会在 `console.log` 序列化时被调用。
...
注意，为了简单起见，我向 `Object` 添加了一个属性，这不是必要的。而是创建一个自定义类来正确控制谁继承了这个自定义钩子。

### 作为类的元数据

Symbol 的最后一个有趣的用例是向你的类和对象中添加属性，这些属性实际上不属于其”形状“的一部分。

解释一下：一个对象的“形状”是由它的属性决定的。将数据添加到对象而不影响其形状（其原始属性集）的唯一方法是将它们添加为 Symbol。这是因为这些类型的属性不会作为 `for..in` 循环的一部分出现，也不会作为调用 `Object.keys` 的结果出现。
...
就像上面提到的，Symbol 不是 JavaScript 最方便的特性，但也不是最差的。Symbol 成为标准的一部分是有充分理由的，现在你知道了它们的一些有趣的用例，考虑花时间使用并利用它们吧。
