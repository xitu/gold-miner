...
> **注**：如果你使用的是 PyCharm 并在 `order_up` 下看到了红色的波浪线，这表示找不到此包。你可以重新在 Github 项目的根目录下打开这个项目或右键单击项目文件夹并选择 “Mark Directory as” -> “Sources Root” 来解决此问题。
...
现在，`order_up` 的主要功能是获取订单，删除不在菜单上的项目，但允许重复项目。 因此，让我们添加测试以确保我们在代码中保留这些功能。
...
为此，我们不分配 `return_value` 而是分配 `side_effect` 一个列表。
