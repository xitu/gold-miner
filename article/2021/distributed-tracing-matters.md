...
单体式应用的问题大多是易于调查的。您可以查找执行迹线，并详细调查错误和瓶颈。有时它可以快速查看堆叠追踪，以了解您业务流程中的问题是什么。

对于分布式系统，尤其是无服务器云体系结构，就不是这种情况。单个进程经常调用若干函数或时间表导致事件驱动的处理。调试这种复杂的分布式系统可能很困难。关联请求和操作并不是一项简单的任务，但你通常需要找到错误，不一致或瓶颈。

这就是为什么你需要分布式描绘，总体的团队结构和生态系统。它通过为您详细地提供所有被执行的代码来帮助您对专用请求或触发的业务流程提供帮助。

在本文中，我想向您介绍 OpentRacing API，W3C 的 Trace 上下文规范，以及如何构建您自己的自定义集成，您可以用于为您选择的跟踪工具创建和扩展跟踪上下文。
...
现代软件架构通常是多个系统的融合。在一个系统上的单个请求可能导致生态系统周围的许多操作和业务流程。有时，它们甚至不是通过基于事件的驱动架构的同步而是完全解耦。
...
通过查看 AWS 上一些虚构架构的​​示例，我们看到在不同的系统中有很多代码只是一个外部触发器。

* 数据被写入了 DynamoDB，但在更改后通过 Streams 转发。
* 消息排队到 SQS，稍后被另一个 Lambda 函数处理。
* 进行对外服务的调用，然后进行进一步的传入请求。
...
如果您正在处理一个多团队项目，并且产品是由独立团队开发的，那就更明显了。事实上，每个团队都会监控自己负责的资源和微服务。但是，如果客户抱怨请求缓慢并且没有团队报告任何性能瓶颈怎么办？如果没有任何团队总体的请求相关性，就不可能详细调查这些问题。
...
`Spans` 是分佈式追踪的基本概念。`Span` 表示特定的執行代碼或工作量。看看 OpenTracing 的規範，它包含：

* 操作的名稱
* 開始和結束時間
* 一組標籤（用於查詢）和日誌（用於特定於跨度的消息）
* 前后关系

这意味着，参与的请求在我们的生态系统中的每个组件应有助于至少一个跨度。由于跨度可以引用其他跨度，我们可以建立一个完整的堆栈跟踪信息的多个时间段的覆盖在单个请求中的所有操作。跨度的大小没有限制。这基本上可以在任何地方被使用，从覆盖整个复杂的过程到单个功能甚至操作的跨度。
...
如果生成了一个新的跨度而没有另外指定，当前活动的跨度会自动成为其父跨度。
...
现在我们已经介绍了这些概念。 让我们通过了解如何利用这个概念来初始化跟踪以及如何通过自己打开和关闭跨度来扩展它们来将其付诸实践。

尽管分布式跟踪工具为许多不同的语言和框架提供了代理，但如果您需要手动扩展跟踪，这可能很重要。
...
* 一个跟踪跨度名称的列表，按照跨度打开的顺序
* 一个保存 span 必要细节的对象：打开它的时间戳和跨度的标识符
...
构建分布式系统是一项复杂的任务，但可以通过 AWS、Azure 或 GCP 等云提供商以及利用 CloudFormation 的无服务器框架等高级基础设施作为代码工具来快速完成。 请记住，您需要分布式跟踪来分析您的系统和完整生态系统的运行方式并系统地调试问题。
