...
这些系统的资源非常受限，使得大部分传统的数据库系统难以配置和安装，而且这不是个例。

但有一些小型数据库作为软件产品的一部分被嵌入的使用案例。比如，想象一下通过你的 IDE 在一个大型的代码仓库中完成搜索。IDE 可以嵌入一个反向索引的数据库，允许使用关键字搜索，然后得到相关文件的一个快速查阅。或者在你最喜欢的桌面邮件客户端进行搜索时，这是最有可能包含嵌入式数据库的场景。所有的邮件都被存储且编入了索引，这样你可以简单快速的得到你想要的信息。

另一个使用嵌入式数据库巨大的好处是它并不需要网络通信来交互，相比于其它各类标准的数据库，这是一个巨大的性能提升。通常在一般的开发中，你可能会在你自己的服务器(或者服务器集群)上放你自己的数据库，那么它的资源消耗不会影响到你架构中的组其他件，而对嵌入式数据库，你可能希望它们能尽可能的靠近客户端。这可以减少它们之间的等待时间以及避免依赖于通信渠道(即网络)。
...
LowDB 主要好处之一是它注定在 JavaScript 使用，换句话说：它可以用在后端，桌面端或者其它可以跑浏览器代码的地方。

你可以用在 Node.js 中使用 LowDB 写后端，也可以集成到 Electron 项目中去开发桌面客户端，当然它也能直接在已集成 JS 运行环境的浏览器中运行.

LowDB 提供的 API 也很简单少量，也并不没有任何开箱即用的搜索功能。它仅限于将 JSON 文件的数据加载到一个数组变量中，然后让你自己（用户）去找你想要的数据。
...
高度扩展性以及与 TypeScript 的兼容性，为了数据存储提供了一种类似模式的行为(不允许你添加违背预设置模式的数据)。
...
有趣的是，LevelDB 被 [Chrome’s IndexedDB](https://blog.openreplay.com/getting-started-with-indexeddb-for-big-data-storage) 作为后端数据库使用，显然 Minecraft Bedrock 版使用它作为某些块和实体数据存储(尽管它们使用的是 Google 实现的[轻微修改后的版本](https://minecraft.fandom.com/wiki/Bedrock_Edition_level_format))。

## Raima Database Manager

我有提到过 IoT，是吗？ [Raima](https://raima.com/) 是最快的数据库管理者之一，对运行于资源受限的 IoT 设备而做了特别优化。

我说的资源受限的环境指的是什么？Raima 运行只需要 350kb 的 RAM，这样我可以最简化资源使用。

这个解决方案最主要的且前面几个都没有的特点是，它完全支持 SQL。它提供关系型数据模型以及允许使用 SQL 语言查询。

与 LevelDB 不同，它还能通过客户端-服务端的架构多进程访问数据库(相比其他的，它让你能离源代码稍微更远一点)。如果你决定做一个贴近源代码的嵌入式应用，你也可以使用多线程并发访问多个数据库。

Raima 的灵活性使你可以从传统的客户端-服务端模式转到最有效的(当然也是受限的)单个客户端使用单个内存数据库的用例。但这是嵌入式数据库非常有效的用例。

![图片出自 [Raima 网站](https://raima.com/architecture/)](https://cdn-images-1.medium.com/max/3166/1*Wri490chKY--YYgiIkLUsA.png)

它的灵活性使它成为一个非常通用的解决方案。当然，每一种部署模式都有自己的好处和限制，但也会有最适合的特殊用例。所以确保你选择了最适合自己的，然后将其发挥出最大的价值。
...
最后一个，但绝对相当重要的一个，solidDB 覆盖一个非常有趣的案例，在提供一个内存关系型数据库的同时也可以提升模型的持续性。口气不小，声称自己可以保持两种数据存储选项的实时同步。
