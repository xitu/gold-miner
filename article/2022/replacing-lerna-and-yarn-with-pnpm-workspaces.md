...
从我们现有的选项，一些已经存在一段时间（像 Lerna），但已经不再持续维护了；一些还没渡过方案阶段（像 Bolt），一些在运行，但只适用于某种特定的项目。

不幸的是，我们没有一个好的工具匹配所有 JavaScript/Typescript 类型的项目和所有团队规模，这是可以理解的。

然而，现在有一个（“新”）选项，可能在大多数例子中，对我们有所帮助：**pnpm workspaces**。
...
然后，我决定 fork 一个叫 `mdx-prism` 的库去修复一些小问题和自动化部署。再一次，建立另一个新的包含它的代码和基础配置的 git 仓库。
...
虽然因为它们可能没有一个专注团队（或者几个人）去维护它，规划工具的未来，Lerna 正在慢慢消迹。
...
因为这是可能发生的，我们的依赖和我们的依赖的依赖也能出现这个错误，从 `node_modules`导入一些东西，而不将其声明为 dependency/peerDependency。
...
虽然用这种方式 安装 `node_modules` 更加可靠，但这可能破环一些已经用上面的 flat `node_modules` 构建的应用兼容性。
