...
# 用Isabelle/HOL验证分布式系统 

我们每天都以互联网服务的形式使用分布式系统。这些系统很有用，但实现起来也很有挑战性，因为网络是不可预料的。每当你发送一条网络消息时候，它有可能很快到达，也有可能延迟很久，或者永远不会达到，又或者多次到达。
...
不幸的是，众所周知，分布式系统很难推理。因为无论消息到达的顺序，或者一些消息丢失，或者某些进程崩溃，它们都必须要确保其保证。许多算法非常微妙，非正式的推理不足以确保它们是正确的。此外，并发活动出现的排列和交错，这个可能性的数量很快就会变得太大，以至于模型检查器无法进行详尽的测试。出于这个原因，正式的正确性证明对于分布式算法很有价值。

## 在 Isabelle/HOL 中建模一个分布式系统

在这篇博文里，我们将会探索怎么样使用 Isabelle/HOL 辅助证明，去正式的验证一些分布式算法。Isabelle/HOL 没有对分布式计算有任何内置的支持，不过幸运的是，可以十分简单的使用 Isabelle/HOL 提供的结构：方法，列表和集合，去建模一个分布式系统。

首先我们假设在系统里每一个进程（或**节点**）都有一个唯一的标识符，它可以简单的是一个整数或是一个字符串。根据算法，系统中一组进程 ID 可能是固定的和已知的，或者是未知的和不固定的（后者适用于进程们随时可以加入或离开的系统）。

然后算法的执行以离散的时间步长进行。在每个时间步长中，一个事件发生在其中一个进程上。这个事件可能是这三件事中的之一：接收一个其他进程发送过来的消息，接收用户的输入或者是超时。
...
接下来，我们定义所有可能的事件序列集合。为此，我们在 Isabelle 中使用了一个判断断言：如果是算法属于执行中的有效事件集合，`(execute step init procs events msgs states)`则返回 true ，其中`step`是阶跃函数， `init` 是每个进程的初始状态，`proc`是系统中所有进程的集合（如果我们希望允许任意数量的进程，这可能是无限的）。最后两个参数跟踪执行状态：`msgs` 是迄今为止发送的所有消息的集合，`states` 是从进程 ID 到该进程状态的映射。
...
这个定义的状态是，当系统处于初始状态且未发送任何消息时，空事件列表有效。此外，如果`events`是迄今为止有效的事件序列，并且event在当前状态下是允许的，那么我们可以调用 step 函数，添加它发送到`msgs`的任何消息，更新相应进程的状态，其返回值是另一个有效序列的事件。
...
2. 如果一些执行时，属性 **P** 的值为真，那么我们在执行结束时再添加一个时间步长，那么 **P** 在该时间步长之后仍然成立。

换句话说，我们证明 **P** 在整个系统所有可能的状态中是一个不变量。在Isabelle里，这证明看起来是粗糙的像这个 ( `step`, `init`, 和 `procs` 被适当的定义过):
...
有关如何在此模型中证明简单共识算法的正确性的更多详细信息，我录制了一个[2 小时的视频讲座](https://www.youtube.com/watch?v=Uav5jWHNghY) ，该讲座贯穿了第一原理的演示（无需之前有 Isabelle 的经验）。这个[演示的Isabelle 代码](https://gist.github.com/ept/b6872fc541a68a321a26198b53b3896b) 也是可用的。
