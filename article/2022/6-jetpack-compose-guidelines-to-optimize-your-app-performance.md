...
Jetpack Compose 有一个专门的优化系统，但理解 Jetpack Compose 的渲染机制对于正确提高你 app 的性能至关重要。
...
创建几对类和可组合项，然后生成对应的 Compose 的编译报告。现在，只需要关心这样的影响而不是机制，因此你只需分析结果。
...
> **注意**: 不返回 Unit 的可组合项既不能跳过也不能重新启动。因为它们是值的生产者，应该在发送变化时强行让他们父级元素重新组合，它们是不可重启的。
