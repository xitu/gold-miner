...
CSS 似乎是一种非常简单的语言，很难在这上面犯错误。只需要添加规则以对网站进行样式设置就可以了，对吗？对于只需要几个 CSS 文件的小型站点，可能是这种情况。但是在大型程序中，这样可能会使样式迅速失控。如何让它们更可控？

事实是，就像其他任何语言一样，CSS 有它自身的微妙之处，这可以成就你的设计，也可以毁掉你的设计。这是 CSS 的 10 条技巧 — 可以帮助你从样式中获得最大收益的最佳实践。
...
考虑为你的项目使用一套 CSS 规范。CSS 规范使 CSS 文件具有一致性。它们有助于扩展和维护您的项目。这是我可以推荐的一些流行的 CSS 规范。
...
Object-oriented CSS 或 OOCSS 具有两个主要原理。
...
另一种增强可读性的好方法是嵌套选择器。这是一个简单而强大的功能，但CSS缺少这一功能。
...
不必一遍又一遍地写出相同的内容，而只需在需要时添加 `mixin` 即可。
...
另一个很好的后处理器是 `[autoprefixer](https://www.npmjs.com/package/autoprefixer)`。使用 `autoprefixer`，当您要支持最后四个版本时 — 无需在CSS文件中写入任何前缀就可以完成所有工作！
...
现在让我们进入实际的 CSS 应用。这经常被忽略。通常，通常，你可以简单地通过使用正确的 HTML 标签来减小 CSS 包的大小。假设你的标题包含以下规则：
...
不建议使用 CSS 的标签选择器，因为它会匹配所有的标签。虽然只有几分之一毫秒的差异，但积少成多。更重要的是，减少其他方面的复杂性是一种好习惯。
...
然而，这样的话，只有键盘导航的用户将不会知道他们在你的网站上关注什么。
...
当你必须处理媒体查询时，请始终使用移动设备优先。以移动设备为先的方法意味着你首先要开始为小屏幕设备编写 CSS 并从那里开始构建。这也称为渐进增强。
...
/* 非移动优先媒体查询，低于 600px 的所有内容都会获得以下样式 */
...
最后，压缩你的包以减少它们的大小。压缩删除注释和空白，压缩包只需更少的带宽就能获取。
...
* **创建自己的**: 如果你没有用于项目设置的专用软件包，那么我会提供一个教程，向你展示如何创建 [自己的实现](https://medium.com/swlh/how-i-reduced-my-css-bundle-size-by-more-than-20-76433e7330eb).

## 总结

遵循以下 10 个简单步骤将有助于你编写 CSS 文件：

* 更轻巧
* 易于维护
* 易于扩展

不仅如此，使用一些实用工具，如预定义的调色板或排版规则，将帮助您创建更一致的设计。你的样式也将更加可重用，你就可以为下一个项目节省时间。

你遵循哪些其他 CSS 最佳实践，但本文未提及？让我们在评论中知道！
