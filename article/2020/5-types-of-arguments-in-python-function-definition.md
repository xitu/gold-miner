...
**注意：** 默认值在定义过程中在函数定义处只会计算一次。因此，在把如列表、字典等可变对象作为默认值时会有些许不同。

#### 2. 关键字参数：

也可以使用形如`kwarg=value`的关键字参数来调用函数。

在调用函数时，参数值并不需要与定义的函数的参数顺序相同，关键字参数也是如此。但所有的关键字参数必须与定义的函数中的参数一一对应。
...
2. 在混合使用位置参数和关键字参数时，关键词参数总是在位置参数之后。
...
可变长度参数也称为**任意参数**。如果不能提前了解参数的数量，可以使用任意参数。Variable-length arguments are also known as **arbitary arguments**. If we don’t know the number of arguments needed for the function in advance, we can use arbitary arguments
...
对于任意位置参数，定义的函数的参数前会有一个**星号（`*`）**，该参数可以包含非关键字可变长度参数。这些参数将包含在一个**元组**中。在可变数量的参数之前，可能会出现零个或多个普通参数。
...
对于任意关键字参数，定义的函数的参数前会有**双星号（`*`）**，该参数可以包含非关键字可变长度参数。
