...
首先，让我们快速列出对 API 网关解决方案最低期待。请注意，这不是一个详细的列表，也不是你正在寻找的解决方案与之相匹配的内容。但是，它应该覆盖 API 网关的大部分应用场景。
...
Tyk 拥有丰富的特性集和漂亮的 Web 用户界面，它对于拥有复杂 API 管理方案的项目来说是一个不错的选择。Tyk 的出色之处在于，它包括 API 仪表盘、分析、安全、版本控制、开发者门户、限速和其他免费开箱即用的网关特性，如果你仅用于非商业的目的的话。但是，用于商业目的，你需要购买它们的商业许可证，其中也包括它们的支持。
...
最后，这是一张流程图，我故意将其简化。你应该使用以下图表以及上述特性矩阵来缩小你的选择。
