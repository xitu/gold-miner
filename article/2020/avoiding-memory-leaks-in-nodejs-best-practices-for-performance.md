...
内存泄漏是每一位开发者最终都会面对的事，它在大多数语言中都很常见，即便语言本身能够自动管理内存。内存泄漏会导致一些如应用缓慢、崩溃、高延迟等等问题。

在这篇文章中，我们会了解到什么是内存泄漏并如何在 Node.js 中避免它。虽然这篇文章着重点在 NodeJS，但应该也适用于 JavaScript 和 TypeScript。规避内存泄漏有助于你的应用更高效地利用资源同时也能带来性能提升。
...
* **堆**：V8 存放对象或动态数据。这是内存区域中最大的一块并且这里是 **垃圾回收(GC)** 发挥作用的地方。

> V8 通过垃圾收集管理堆内存。简单地说，它会释放没有被引用的对象。例如，没有在栈中引用到，不管直接引用还是间接引用，都会被释放内存空间用于新对象的创建。
> 
> 为了给自身 V8 进程考虑，V8 中的垃圾收集器会无条件地回收未使用的内存。V8 垃圾收集器是区分新旧生代的（堆中的对象按它们的存放时间分组并会在不同的阶段被清理）。V8 的垃圾回收有两个阶段和三种算法。
...
简单来说，内存泄漏就是堆上的一块孤立内存，这小块内存不再被程序使用并且也没有被垃圾收集器释放回系统，所以，这是一块没有用的内存。这样的内存块持续增加可能会导致应用没有足够内存空间去支撑其继续工作，也可能会导致你的操作系统没有足够的内存可供分配，进而导致系统缓慢或崩溃。

## 什么导致了内存泄漏

自动内存管理（如 V8 中的垃圾收集机制）目的在于避免内存泄漏，像循环引用不再是一个忧虑，然而内存泄漏依然可能发生，也许是因为预料之外的堆中的引用亦或是各种原因。一些常见的原因列举如下。
...
* **闭包**: JavaScript 闭包能够记住它所被包围的上下文，当一个闭包持有一个引用，该引用指向堆中的一个庞大对象时，闭包会令该庞大对象持续在内存中直到闭包不在使用它。这意味着你可能会轻易地陷入这种情况，即持有着一个引用的闭包被不正确地使用从而导致内存泄漏。
...
当你赋值给一个未声明的变量时，在默认情况下 JavaScript 会自动提升它为全局变量。这可能是一种低级错误，但却有可能导致内存泄漏。另一种情况是赋值给 [`this`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/this)（在 JavaScript 中依然很神圣的东西）。
...
当你使用箭头函数，你同样需要小心不要意外的创建全局变量，不幸的是，严格模式对这种情况没有帮助，你可以使用 来自  ESLint 的 `no-invalid-this` 规则来避免这种情况。如果你不适用 ESLint，确保不要在全局箭头函数里向 `this`赋值。
...
在任何现实生活中的应用中是无可避免地使用堆内存的，但我们可以跟随以下几点更高效地利用堆：
