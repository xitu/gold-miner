...
另一个常见的变种是**带约束条件**的背包问题。一些题目对物品个数进行了限制，使得你不能选择重复选择某一种物品。对于每一种物品，程序必须决定将它放在包中或者抛弃它。
...
虽然这是一个有效的解法，但它不是最优解，因为其时间复杂度是指数级的。如果要求使用递归，请使用此解法。对于动态规划解，这也可以是一个很好的起点。

**时间复杂度**：O(2^n)，由于大量重复的子调用
...
然后排除该物品，递归调用相同的函数，并将结果保存在 `profit2` 变量中。在第 21 行，我们返回 `profit1` 和 `profit2` 中较大的一个。
...
这个程序包含许多重复的子问题，但我们并没有把结果存储起来，每次都需要再计算一次。重复计算会大大增加程序运行时间。为了避免重复计算，我们可以改用动态规划来保存子问题的解决方法以供之后使用。
