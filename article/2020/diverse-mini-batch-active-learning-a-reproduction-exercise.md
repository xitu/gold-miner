...
在这第二篇博文中，我们将看看更新的方法，并借此机会复现 2019 年的论文 [**Diverse Mini-Batch Active Learning**](https://arxiv.org/abs/1901.05954) 的结果。为了完整详细地描述所有的方法，我们直接为读者提及了该文章。

## 多样化的重要性

我们之前的博文介绍了基于不确定度的、使用“类别预测概率”来选择模型最为纠结的样本的方法。我们也介绍了基于密度的、利用数据分布来找到最有代表性样本的方法。
...
正如上面所说，一个分割特征空间的自然的想法是使用聚类方法。因为聚类应当由相似的样本组成，一个聚类方法应既能够区分不同特征，又能找到每个聚类中的代表性样本。
...
**注：** 在这篇论文中，作者将他的方法与一个叫 FASS 的框架进行比较。出于简单性的考虑，我们在这篇博文中没有复现这个框架的结果。我们也决定使用 Keras 而不是惯用的 MXNet，来看看用另一个框架能不能做到复现。
...
* 我们从 100 个样本开始，在第 100 个 batch 左右调整到 1,000 个。
...
**结论：** 在这个实验中，我们没能复现原文的结果，但也没有冲突。特别地，随机采样似乎在原实验中在 1,000 个样本上达到了 0.68 的准确率，其他的方法大概在 0.70 左右。最后，由于结果的变异性很强，我们不能断言某个方法就比另一个要好。
...
同时，我们观察到随机法要显著地差于所有的主动学习方法。纯粹基于不确定度的方法在某些地方比基于聚类的方法要差一点，但也不是太多。同时，由于它更容易搭建，可能仍然是一个值得考虑的选择。最后，原文的图形表明聚类的方法在 400 个样本左右时只有细微的差别。我们没能复现并观察到它们的相似表现。
...
**结论：** 我们的两个实验都确认了原论文的发现，尽管有微小的不同。我们看到，基于多样性的方法击败了随机选择和基于不确定度的采样方法。有趣的是，它们甚至似乎一致地大大优于它们所基于的基于信息量的方法。
