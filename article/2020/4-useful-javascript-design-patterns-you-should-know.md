...
您是否像我以前一样对设计模式感到非常困惑呢？就我自身而言，我以前的个人项目，几乎没有考虑过设计模式。因此，存在很多设计缺陷和代码缺陷，给以后的维护和迭代带来了很多的麻烦。
...
这可能有点抽象，但您可以这样去理解：现在假设您正在玩一个电子游戏，您花了五个小时才完成通关。但是在您下一次玩的时候，因为您之前已经玩过了，积累了一些技巧，所以这次只需要两个小时就可以完成通关。这时您的朋友问您玩这个游戏的技巧，于是您告诉了他一些自己的经验，这就是所谓的设计模式。
...
现在我们需要编写判断逻辑，以确保只有正确的用户才能看到内容。您会怎么做？许多初学编程的人可能会简单地选择 `if-else` 并像下面这样编写代码：
...
看了上面的代码之后，您可能会想：代码的数量似乎增加了。

但正如我们之前所说的，设计模式的价值在于它能使您更容易地应对变化的需求。如果您的需求从头到尾都没有改变，那么使用设计模式真的没有多大价值。但是，如果当项目的需求发生了变化，那么设计模式的价值就可以得到体现。
...
我们可以看到，通过使用策略模式，我们的代码变得更易于维护。现在可以考虑将策略模式应用到您自己的项目中了，例如在处理表单验证时。

当您的需求基本上满足以下条件时，您可以考虑使用策略模式来优化代码。
...
* `MessageCenter` 最初是由 Jon 开发的，他后来因为某些原因将 `MessageCenter.fetch` 重新命名为`MessageCenter.request`。这将导致您需要更改 `applySuccess` 函数，否则您的代码报错。
* 订单模块最初是由 Bob 开发的，但是他还没有写 `Order.update` 由于工作量太多。这使得您的代码不可用，并且您只能临时删除该函数。

更糟糕的是，您的项目常常不只依赖于这三个模块。例如，当申请成功时，我们需要提交一个日志。您如何处理这种情况？您可能需要再次修改原始函数。
...
您是否觉得 EventEmitter 很熟悉？没错，在面试中经常会出现？
...
当您负责一个基本上满足以上条件的模块时，您可以考虑使用发布-订阅模式。
...
在上面的例子中，我们设计了组件 yellowHOC 来包装其他组件。这是装饰模式。

您是否会感到困惑呢？让我们看一下装饰器模式的另一个例子。
...
看起来已经满足了要求，但实际上，上面有一个非常大的缺点：我们的采购流程可能会改变，比如增加一个库存检查流程。然后，您必须彻底地更改原始代码，这对于维护代码设计是非常困难的。
...
这样有什么好处呢？我们做的第一件事是解耦，我们的方法是在函数 A 中调用函数 B，然后在函数 B 中调用函数C。但是现在它不同了，每个函数都是相互独立的。
