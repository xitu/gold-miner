...
函数式编程（FP）是通过组合纯函数来构建软件的过程。如今，老板们在寻找能利用多种范式来解决问题的程序员。而函数式编程凭借其解决现代问题的高效性和可扩展性，尤其受到欢迎。
...
**函数式编程** 是一种声明式的编程范式，程序是基于函数序列而非语句构建的。

每个函数接收一个输入，并对相同输入总是返回一致的输出，且不会改变程序的状态，也不会受其影响。

这些函数完成单一的操作，并且能够顺序地组合起来以完成复杂的操作。函数式编程范式使得代码高度模块化，因为函数能够在程序中复用，也能被调用、作为参数传递，或者是被返回。

**纯函数** 没有副作用，并且不依赖于全局的变量或状态。

![Visualizing FP Functions](https://i.loli.net/2020/11/09/gnxakfDrOi23jVZ.png)

当解决方案能够容易地用函数表示，并且几乎没有实体方面的含义时，就可以使用函数式编程。相比面向对象程序基于现实对象的代码建模方法，函数式编程则超越了数学意义上的函数，它的中间或者最终值之间并没有实体关联。
...
* **调试简单**：纯函数和不可变的数据让找到变量值被设定的地方变得简单。纯函数影响变量的因素更少，因此能让你更容易发现有 Bug 的部分。
...
不是所有的编程语言都支持函数式编程。某些语言，比如 Haskell，生来就被设计为函数式编程语言。其他一些语言，比如 JavaScript，既有函数式编程的，也有面向对象编程的能力。剩下的就是那些完全不支持函数式编程的语言。

#### 纯粹的函数式编程语言

* [**Haskell**](https://www.haskell.org/)：这是最受欢迎的函数式编程语言。它内存安全、垃圾收集完善，并且得益于提前的机器码编译，运行速度也快。Haskell 丰富且静态的类型系统让你可以使用代数和多态的类型，使函数式编程更加高效且易于阅读。
...
* [**Clojure**](https://clojure.org/)：这门语言是 Lisp 的函数作为一等公民的方言，运行在 JVM 上。它基本上是一门函数式编程语言，既支持可变的，也支持不可变的数据结构，但并不像这里其他的函数式语言那样严格。如果你喜欢 Lisp，你也会喜欢 Clojure。
...
* **JavaScript**：尽管函数并不是一等公民，JavaScript 凭借其的异步特性，在函数式编程方面也相当突出。JavaScript 也支持基本的函数式编程特性，比如 lambda 表达式和解构。这些特性共同让 JavaScript 成为了多范式语言中最好的支持函数式的编程语言。
...
Python 作为一个多范式语言，部分地支持了函数式编程。借助函数式编程，一些数学问题的 Python 解法可以更容易达成。 

开始使用函数式编程最难的一个改变是要减少类的使用量。Python 中的类有着可变的属性，这使得我们难以创建纯的、不可变的函数。

试着将你的代码尽可能保持在模块以内，仅当必须使用类时才使用。
...
这份代码会造成错误，因为它试图给不可变的元组对象重新赋值。函数式的 Python 程序应该多使用这些不可变的数据结构来实现纯函数。
...
为了在 Python 中组合函数，我们会用 Lambda 表达式。这使得我们能够用一次函数调用来调用多个函数。
...
得益于对“函数是一等公民”的支持，JavaScript 已经提供函数式编程能力很久了。最近，JavaScript 中的函数式编程越来越受欢迎，因为它在 Angular 和 React 等框架中使用时可以提升性能。

让我们看看如何使用 JavaScript 来实现函数式编程。我们会关注如何表达核心概念：纯函数、函数是一等公民，以及函数组合。

#### 纯函数和不可变函数

为了在 JavaScript 中创建纯函数，我们会用函数式的方式替代传统方法，比如 `const`，`concat` 和 `filter()`。
...
如果我们想要组合更多的函数，我们可以使用 `lodash` 来简化组合过程。具体地，我们会使用 `compose` 特性。它接收一个函数作为第一个参数，后面跟着一系列函数执行。
...
这个类同样有一个带参的构造函数、所有属性上的 Getter 方法，并且没有 Setter 方法，这使得这个类是不可变的。

#### 函数是一等公民

Java 可以使用 Lambda 表达式来实现函数作为一等公民。Lambda 表达式像一个方法一样接收一系列的表达式，但自身不需要具名或者预先定义。

我们可以使用 Lambda 表达式来替代普通的函数，因为它也被认为是类对象，可以用来传递或者返回。
...
Java 里有一个 Interface：`java.util.function.Function`。它提供了进行函数组合的方法。`compose` 方法的返回值会首先执行传入的函数（`multiplyByTen`），然后把返回值传递给外部的函数（`square`）。
`andThen` 方法的返回值则首先执行外部的函数，然后才是参数中的函数。
...
然后在 **第 5 行和第 8 行**，我们创建了两个组合函数 `multiplyByTenAndSquare` 和 `squareAndMultiplyByTen`，每一个都接收两个参数（来满足 `square`）。*（译注：这里的接收两个参数是什么意思？请校对一起探讨）*

这两种组合函数最终都完成了原组分函数的功能，但顺序不同。你可以试着使用同样的输入来执行这两个组合函数。

## 接下来学什么

今天，我们讨论了一些函数式编程的基本概念，并探索了这些核心概念在 Python、JavaScript 和 Java 中是如何体现的。

重新火热的函数式编程语言之一是 Scala。许多科技巨头，比如 Twitter 和 Facebook，都使用 Scala，并招聘相关的技术人员。你的下一步是学习 Scala 的基础，作为函数式编程语言的入门。
