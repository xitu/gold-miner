...
一个**模块**是一段可以为其他模块提供函数功能的代码块，同时也可以依赖别的模块里的功能。模块中 **exports** 用以导出， **imports** 用来引用。模块之所以有用，是因为它们允许开发人员复用代码，它们提供许多开发人员可以使用的稳定、一致的接口，并且它们还不会污染全局名称空间。

模块（有时称为 ECMAScript 模块或 ES 模块）现在可以在 JavaScript 中直接使用，在本教程的剩余部分中，您将探索如何在代码中使用和实现它们。
...
为了演示如何使用它，请将 `functions.js` 文件更新为模块并导出其中的函数。需要在每个函数的前面添加 `export`，这可以让这些函数被用于其他模块。
...
在 `script.js` 文件中，您可以使用以下命令将默认函数导入为 `sum`：
...
了解更多有关 JavaScript 中模块的只是，请阅读 [Mozilla Modules](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules)。如果你想探索 Node.js 中的模块，可以阅读 [How To Create a Node.js Module tutorial](https://www.digitalocean.com/community/tutorials/how-to-create-a-node-js-module)。
