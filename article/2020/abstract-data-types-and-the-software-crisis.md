...
> **不要与以下内容混淆：**
>
> **代数数据类型**（有时缩写为 ADT 或 AlgDT）。代数数据类型是指编程语言中的复杂类型（例如 Rust，Haskell，F＃），这些类型显示特定代数结构的某些属性。例如，总数类型和产品类型。
>
> **代数结构。** 代数结构是从抽象代数研究和应用而来的，与 ADT 一样，抽象代数通常也用代数公理来描述指定，但在计算机和代码领域之外仍然适用。可能存在无法在软件中完全建模的代数结构。相比之下，抽象数据类型可以作为规范和指南来正式验证软件运行。

抽象数据类型（ADT）是由公理定义的抽象概念，公理表示一些数据和对该数据的操作。没有根据具体实例定义 ADT ，也没有指定实现中使用的具体数据类型，结构或算法。相反，ADT 仅根据其操作以及这些操作必须遵循的公理来定义数据类型。
...
ADT 可以代表对任何类型的数据的任何一组操作。换句话说，所有可能的 ADT 的穷举列表是无限的，其原因与所有可能的英语句子的穷举列表是无限的类似。ADT 是对未指定数据而不是特定的一组具体数据类型的一组操作的抽象概念。一个常见的误解是，许多大学课程和数据结构教科书中讲述的 ADT 的具体示例就是 ADT。许多这样的将数据结构标记为“ADT”，然后跳过 ADT 并以具体的术语来描述数据结构，而从未使学生接触到数据类型的实际抽象表示。**这就糟糕！**

ADT 可以表达许多有用的代数结构，包括半群，monoid，函子，单子等。Fantasyland 规范是 ADT 描述的有用的代数结构目录，旨在鼓励 JavaScript 中的互操作实现。可以使用提供的公理来验证库构建器的实现。

## 为什么要使用 ADT？

抽象数据类型非常有用，因为它们为我们提供了一种以数学上合理且明确的方式来正式定义可重用模块的方法。这使我们可以共享一种通用语言，以引用大量有用的软件构建块词汇：当我们在不同领域，框架甚至编程语言时，会对我们学习产生有用的想法。

## ADT 的发展历史

在 1960 年代和 1970 年代初，许多程序员和计算机科学研究人员对软件危机感兴趣。正如 Edsger Dijkstra 在他的图灵奖演讲中所说的那样：

> “软件危机的主要原因是机器变得功能强大了几个数量级！坦率地说：只要没有机器，编程就完全没有问题。当我们有一些弱小的计算机时，编程成为一个温和的问题，而现在我们有了庞大的计算机，编程也同样成为一个巨大的问题。

他所指的问题是软件非常复杂。NASA 的阿波罗登月舱和制导系统的印刷版大约是文件柜的高度。如此大量的代码，想象一下试图阅读和理解其中的每一行代码的困难程度。

现代软件要复杂几个数量级。Facebook在2015年大约有 [6200 万行代码](https://www.informationisbeautiful.net/visualizations/million-lines-of-code/)。如果每页打印 50 行，则将填满 124 万页。如果堆叠这些页面，则每英尺或 688 英尺可获得约 1800 页。这比旧金山撰写本文时最高的住宅大楼[千禧塔](https://en.wikipedia.org/wiki/Millennium_Tower_(San_Francisco))高。

管理软件复杂性是几乎每个软件开发人员都面临的主要挑战之一。在 1960 年代和 1970 年代，他们没有我们今天认为理所当然的程序语言，模式或工具。诸如短绒棉，智能感知甚至静态分析工具之类的东西也尚未发明出来。

许多软件工程师指出，他们在大多数情况下构建硬件就可以正常工作。但是，软件通常是复杂，纠结且脆弱的。软件通常是：
...
如果您只考虑模块化的软件，则无需了解整个系统即可知道如何使系统的一部分正常工作。该软件设计原理被称为区块。为了获得局部性，您需要可以独立于系统其余部分理解的模块。您应该能够清楚地描述模块，而无需过多说明其实现。这就是 ADT 解决的问题。

从 1960 年代一直延续到今天，提高软件模块化的状态是一个核心问题。考虑到这些问题，包括 Barbara Liskov（来自 SOLID OO 设计原则的 Liskov 替代原则中引用的 Liskov），Alan Kay，Bertrand Meyer 和其他计算机科学传奇人物一起致力于描述和指定各种工具来实现这些目标。模块化软件，分别包括 ADT，面向对象的程序设计和按合同设计。

ADT 源自 Liskov 和她的学生在 1974 年至 1975 年之间使用 [CLU 编程语言](https://en.wikipedia.org/wiki/CLU_(programming_language))所做的工作。它们极大地促进了软件模块规范的发展（这是我们用来描述允许软件模块进行接口交互的语言）。软件接口正式可验证的合规性使我们与软件模块化和互操作性更加相关联。

Liskov 于 2008 年因其在数据抽象，容错和分布式计算方面的工作而获得了图灵奖。ADT 在这一成就中发挥了重要作用，如今，几乎每所大学计算机科学课程中都包含了 ADT。

从未完全解决软件危机，任何专业开发人员都应该熟悉上述许多问题，但是学习如何使用诸如对象，模块和 ADT 之类的工具肯定会有所帮助。
...
* **正式。** 规格必须是正式的。规范中每个元素的含义必须定义得足够详细，以使目标受众有相当大的机会从规范中构建兼容的实现。对于规范中的每个公理，必须有在代码中实现的代数证明。
* **适用。** ADT 应该广泛适用。ADT 通常应可用于许多不同的具体用例。在代码的特定部分中以特定语言描述特定实现的 ADT 可能会过度指定。相反，ADT 最适合描述公共数据结构，库组件，模块，编程语言功能等的行为。例如，描述堆栈操作的 ADT 或描述承诺行为的 ADT。
* **最小化。** ADT 规范应最小化。规范应该包括行为的有趣且广泛适用的部分，仅此而已。每种行为都应准确无误地加以描述，但应尽可能少地具体描述。大多数 ADT 规范应使用少量公理来证明。
* **可扩展。** ADT 应该是可扩展的。需求的微小变化应该只会导致规范的微小变化。
* **声明式的。** 声明性规范描述的是内容，而不是方法。应该以事物的形式，输入和输出之间的关系映射，而不是创建数据结构的步骤或每个操作必须执行的特定步骤来描述 ADT。

好的 ADT 应该包括：

* **人类可读的描述。** 如果 ADT 没有附带一些易于理解的描述，它们可能会非常简洁。自然语言描述与代数定义相结合，可以相互检查，以清除规范中的任何错误或读者对其理解的歧义。
* **定义。** 明确定义本规范中使用的任何术语，以避免产生歧义。
* **抽象签名。** 描述预期的输入和输出，而不将其链接到具体的类型或数据结构。
* **公理。** 公理定义代数的不变量，用于证明实现已符合规范要求。
...
推动和弹出没有副作用。如果推送到堆栈并立即从同一堆栈弹出，则堆栈应处于推送之前的状态。
